...
if(trim($_POST["concopia"])){
	$emailSend = explode(";",$_POST["concopia"]);
	foreach($emailSend as $mailDir){
...
if(trim($_POST["concopiaOculta"])){
 $emailSend = explode(";",$_POST["concopiaOculta"]);
 foreach($emailSend as $mailDir){
...
?>
<!--
<iframe src="../radicacion/tipificar_documento.php?nurad=<? echo $nurad;?>&dependencia=<? echo $coddepe;?>&krd=<? echo $krd;?>" width=1% height=4></iframe>
<form><p>
