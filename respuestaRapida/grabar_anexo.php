...
$anexo_record['anex_env_email']  = $cierre;
...
if($editar) {
    $transaccion = new Historico($db);
    $comentario = "Modificación anexo $numero_anexo.";
    $transaccion->insertarHistorico(array($radPadre), $dependencia , $codusuario, $dependencia, $codusuario, $comentario, 91);
}
...
$sqlBorrador = "select is_borrador FROM public.radicado where radi_nume_radi = '" . $radPadre  . "'";
$rsBorrador = $db->conn->query($sqlBorrador);
$esBorrador = 1;
...
if($result && substr($radPadre, -1) == MEMORANDO && $esBorrador == 2) {

    $nombreGenR =  '';
    
    $sqlHistFirm = "SELECT depe_codi, usua_codi
    FROM public.hist_eventos 
    where radi_nume_radi= '" . $radPadre  . "' and sgd_ttr_codigo  =40
    ...
    while($rsSqlHistFirm && !$rsSqlHistFirm->EOF){
        ...
        $rsSqlNomFi = $db->conn->query($sqlNomFi);             
...
if ($result) {
    ...
    foreach($destinatarios as $destinatario)
    {
        ...
        if($envio != '' && $esNotificaciones == false)
        {
            ...
            if($envio == 'E-mail' || $envio == 'Ambos')
            {
                ...
                $TX_COMENTARIO = "Anexo $numero_anexo medio de envio: $envio ".(($validez=='true')?' certificado':'').(($cierre==1)?' .Cierre automatico del radicado de entrada':'');
    ...
    $hash = hash_file('md5', $archivo_grabar);
    $db->conn->Execute("UPDATE anexos SET anex_hash = '$hash' WHERE anex_codigo = '$numero_anexo'");
