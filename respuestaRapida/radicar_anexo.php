...
if($numRadicadoPadreInicial >= 3000 &&($tipo_radicado == SALIDA || $tipo_radicado == CIRC_INTERNA || $tipo_radicado == CIRC_EXTERNA ||
    ...
    $datos_numrad = $radAux->getDatosRad($numRadicadoPadreAnt);
    if (!$datos_numrad['fechaRadicacion']) {
      die('Error: El número de radicado no existe');
    }
    ...
    $hist->insertarHistorico($radicadosSelBorr,
        ...
        $_SESSION["dependencia"] * 1,
        $_SESSION["codusuario"],
        'De Borrador ' . $numRadicadoPadreAnt . ' a radicado No ' . $numRadicadoPadre,
        104);    
...
$setAutor       = 'Sistema de Gestion Documental SuperArgo';
$SetTitle       = 'Respuesta a solicitud';
$SetSubject     = 'Respuesta a solicitud de SuperArgo';
...
$i=0;
$nvSegRad = $rssPatth->fields['SGD_SPUB_CODIGO'];
...
if($tipoRadPadre == 2 || $rs1001->fields["NUMERO"] > 0){
    ...
    /*trae el medio de envio del padre*/
    $sql_mrec_padre="SELECT mrec_codi FROM radicado WHERE radi_nume_radi=".$numRadicadoPadre;
    $rs_mrec_padre=$db->conn->Execute($sql_mrec_padre);



    $rad->mrecCodi      = $rs_mrec_padre->fields['MREC_CODI'];       
    ...
    $nurad = $rad->newRadicado($tipo_radicado, $tpDepeRad, $nvSegRad);
...
while(!$rsInfoAdicionalReasignar->EOF){
    ...
    $anexoCreador = $rsSqlAnexCreador->fields["ANEX_CREADOR"];
    $sqlNombreCreador = "select usua_nomb from public.usuario where upper(usua_login) = upper('$anexoCreador')";
    $rsSqlNombreCreador = $db->conn->query($sqlNombreCreador);
    $nombreCreador = $rsSqlNombreCreador->fields["USUA_NOMB"];

    if($tipoRadPadre == MEMORANDO || $tipoRadPadre == SALIDA || $tipoRadPadre == ENTRADA) {

        if($tipoRadPadre == ENTRADA) {
                $sqlHistFirm = "SELECT hist_fech
                FROM public.hist_eventos 
                where radi_nume_radi= '" . $numRadicadoPadre  . "' and sgd_ttr_codigo  =3
            ...
            $sqlHistFirm = "SELECT hist_fech
            FROM public.hist_eventos 
            where radi_nume_radi= '" . $numRadicadoPadre  . "' and sgd_ttr_codigo  =40
            order by hist_fech desc
            limit 1";
        }


        $rsSqlHistFirm = $db->conn->query($sqlHistFirm);
    ...
    $sqlRevisoAprobo = "select usua_codi_dest, depe_codi_dest FROM hist_eventos 
        where radi_nume_radi = " . $numRadicadoPadre . " and  (sgd_ttr_codigo = 16 or sgd_ttr_codigo = 9) and (usua_codi_dest != " . $usua_actu . " or depe_codi_dest != " . $coddepe . ") " . $whereAuxReviso . " group by usua_codi_dest, depe_codi_dest";
    ...
    $arrayRevisoAprobo = array();
     while (!$rsSqlRevisoAprobo->EOF) {
        ...
        if($nombreCreador != $RevisoAproboNom) {
...
if (file_exists($grafo)) {
    $grafo_html = '<img src="' . $grafo . '" alt="firma" width="220" height="60" style="width:220px;height:60px;object-fit:contain;">';
...
$codusua = $rsDepR->fields['RADI_USUA_ACTU'];*/
  ...
  if($idPlantilla == 100000){
    ...
    include ('./generadorpdf/ACFT01/ACFT01radanexo.php');
    ...
    include ('./generadorpdf/ATFT01/ATFT01radanexo.php');
  } elseif($idPlantilla == 100019) {
    include ('./generadorpdf/SEFT06/SEFT06radanexo.php');
  } else {
...
$pdf = new MYPDF('P', PDF_UNIT, 'LETTER', true, 'UTF-8', false, 3);
...
$hash = hash_file('md5', $ruta_raiz.$ruta2);
...
if ($_SESSION['apiFirmaDigital']=='false' && $_SESSION["usua_perm_firma"] >= 1){
    ...
    $commandFirmado='java -jar '.$ABSOL_PATH.'/include/jsignpdf/JSignPdf.jar '.$nombreArchivo.' '.($JSIGNPDF_OPTS ?? '').' -kst PKCS12 -ksf '.$P12_FILE.' -ksp '.$clave.' --font-size 7 -r \'Firmado al Radicar en CADET\' -V -llx 0 -lly 0 -urx 550 -ury 27';
