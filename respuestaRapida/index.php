...
if ($esNotificacion){  
    ...
    if($radicado_sgd_trad_codigo == RESOLUCION) {
        ...
        $sql21       ="SELECT ID, PLAN_PLANTILLA, PLAN_NOMBRE, PLAN_FECHA, DEPE_CODI, USUA_CODI, PLAN_TIPO
                        FROM SGD_PLAN_PLANTILLAS WHERE PLAN_TIPO = 4 AND ((ID >= 100001 AND ID <= 100009) OR ID = 100016 OR ID = 100017 OR ID = 100018 OR ID = 100019)";      
    }   
...
if(!$esNotificacion && $radicado_sgd_trad_codigo != MEMORANDO) {
   $idDre = [];
    ...
    $sql_medio="select mrec_codi from radicado where radi_nume_radi=".$radicado;
    ...
    while($rsDestinatarios && !$rsDestinatarios->EOF){
      ...
      switch($rs_medio->fields['MREC_CODI']) {
        case 4:
              ...
              break;
        case 1:
              $tablaHtmlDestinatarios .= "<td><select name='envio_$sgdDirId' class=''><option value='Físico'>E-Físico</option></select></td>";
              break;
        case 10:
              $tablaHtmlDestinatarios .= "<td><select name='envio_$sgdDirId' class=''><option value='Físico'>E-Físico</option></select></td>";
              break;              
        case 2:
              ...
              break;
        default:
              $tablaHtmlDestinatarios .= "<td><select name='envio_$sgdDirId' class=''><option value='E-mail'>E-mail</option><option value='Físico'>E-Físico</option><option value='Ambos'>Ambos</option></select></td>";
              break;
      }
...
$cierre=0;
$sql_ca="SELECT anex_env_email FROM anexos WHERE anex_codigo='".$_GET['anexo']."'";
$rs_ca = $db->conn->Execute($sql_ca);
$cierre= $rs_ca->fields['ANEX_ENV_EMAIL'];
