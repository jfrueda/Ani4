...
#echo ">".$codigo_mun."<";
  ...
  { if (strpos($codigo_dep,'-'))
    { $codigo_dep = explode('-', $codigo_dep);
      ...
      if(!$codigo_pai) $codigo_pai = $this->idPais;
    ...
    if(!$depto) $depto=$rs1->fields["dpto_nomb"];
    $codigo_pai=$rs1->fields["ID_PAIS"];
    if(!$codigo_pai) $rs1->fields["id_pais"];
    if(!$codigo_pai) $codigo_pai = $this->idPais;
    $this->departamento = ( $depto === 'D.C.' ? $depto : trim( $depto ) );
   }
  else
  { $this->departamento=""; }
  if($codigo_mun)
  { if (strpos($codigo_mun,'-'))
    ...
    {
      $codigo_pai = $this->idPais;
      $isql ="SELECT MUNI_CODI,MUNI_NOMB FROM MUNICIPIO where DPTO_CODI=$codigo_dep AND MUNI_CODI=$codigo_mun ";}
    $flag_d = 0;
    $rs1 = $db->query($isql);
    $this->municipio=trim($rs1->fields["MUNI_NOMB"]);
    if(!$this->municipio) $this->municipio=trim($rs1->fields["muni_nomb"]);
  }
  else
  { $this->municipio="";  }
...
#echo ">".$codigo_mun."<";
	...
	{	if (strpos($codigo_mun,'-'))
		...
		if(!$this->municipio) $this->municipio=ucfirst(strtolower(trim($rs1->fields["muni_nomb"])));
	}
	else
 	{	$this->municipio="";	}
