...
<body>
    <div id="content" class="container-fluid py-4">
            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    <div class="card-header bg-orfeo text-white ">
                        <h4 class="mb-0">
                            Radicación previa <?= $tRadicacionDesc ?>
                        </h4>
                        <p class="mb-0">
                            Este paso previo de la radiación es para buscar datos ya existentes en
                    ...
                    <div class="card-body">
                        <form
                            name="formulario"
                            method="post"
                            action="chequear.php?<?= session_name() . '=' . session_id() ?>&krd=<?= $krd ?>&dependencia=<?= $dependencia ?>&krd=<?= $krd ?>&faxPath=<?= $faxPath ?>">
        ...
        <form action='NEW.php?<?= session_name() . "=" . trim(session_id()) ?>&dependencia=<?= $dependencia ?>&faxPath=<?= $faxPath ?>' method="post" name="form1">
            ...
            if ($buscar_por_asunto) {
                $where_general .= " $and_asunto a.ra_asun like '%$buscar_por_asunto%'";
            }
            if ($buscar_por_dep_rad) {
                $where_general .= " $and a.radi_depe_radi in($buscar_por_dep_rad) ";
            }
            if ($buscar_por_correo) {
                $where_general .= " $and_correo d.SGD_DIR_MAIL like '%$buscar_por_correo%'";
            }
            ...
            if (($buscar_por_doc) || trim($nombres)) {
            ...
            if (
                $Submit == "Buscar" &&
                ($buscar_por_cuentai || $buscar_por_radicado ||
                    $buscar_por_asunto || $buscar_por_correo ||
                    $buscar_por_nombres || $buscar_por_doc ||
                    $buscar_por_dep_rad || $buscar_por_exp)
            ) {
                $estoybuscando = 1;
