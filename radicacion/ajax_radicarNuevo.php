...
if ($nurad) {
    ...
    if ($esNotificacion || $ent == MEMORANDO || $ent == SALIDA) {
...
if ($nurad == "-1") {
    ...
    if ($modificar) {
        ...
        $obsHist = "";
        $labelsR = [
            'RADI_CUENTAI' => 'Referencia',
            'MREC_CODI' => 'Medio Recepción/Envío',
            'RADI_FECH_OFIC' => 'Fecha oficio',
            'RA_ASUN' => 'Asunto',
            'RADI_NUME_FOLIO' => 'Folios',
            'RADI_DESC_ANEX' => 'Descripción Anexos',
            'RADI_NUME_ANEXO' => 'No. Anexos',
            'EMP_TRANSPORTADORA' => ' Transportadora',
            'RADI_NUME_GUIA' => 'Guía'
        ];
...
if (($esNotificacion || $ent == MEMORANDO || $ent == SALIDA) && !isset($_POST['modificar'])) {
    $hist->insertarHistorico(
        $radicadosSel,
        $dependencia,
        $codusuario,
