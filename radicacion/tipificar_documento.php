...
if ($insertar_registro && $tdoc !=0 && $idSubSerie !=0 && $idSerie !=0 ){
    ...
    else
    {
        ...
        $observa = "*TRD*$deta_serie/$deta_subserie/$deta_tipodocu (Asigancion tipo documental.)";
...
$querySerie = "select distinct($sqlConcat) as detalle, s.id,s.sgd_srd_codigo, UPPER(s.sgd_srd_descrip) as name
    ...
    where (cast(m.depe_codi as varchar(100)) = '$coddepe' or cast(m.depe_codi_aplica as varchar(100)) like '%$coddepe%' or cast(m.depe_codi as varchar(100))='$depDireccion')
