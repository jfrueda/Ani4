...
if(!$envio_salida and !$busq_salida){
    ...
    $label_us   = empty($nombreTp1)? "Remitente" : $nombreTp1;
    $label_pred = empty($nombreTp2)? "Predio"    : $nombreTp2;
    $label_emp  = empty($nombreTp3)? "Empresa"   : $nombreTp3;
}
...
{
?>
<form method="post" name="formu1" id="formu1" action="buscar_usuario.php?busq_salida=<?=$busq_salida?>&verrad=<?=$verrad?>&nombreTp1=<?=$nombreTp1?>&nombreTp2=<?=$nombreTp2?>&nombreTp3=<?=$nombreTp3?>&tipoval=<?=$_GET['tipoval'] ?>" >
<?
}
?>
                ...
                <div class="widget-body no-padding">
                  ...
                  <tr>
                    <td class="listado5" valign="middle">
                      ...
                      <input type=text name=no_documento value='<?=$_POST["no_documento"] ?>' class="tex_area">
                      </font>
                    </td>
                    <td class="listado5" valign="middle">
                      <span class="titulos5">Nombre</span>
                      <input type=text name=nombre_essp value='<?=$_POST["nombre_essp"] ?>' class="tex_area">
                  ...
                  <table class="table table-bordered table-striped">
                    ...
                    <tr class="grisCCCCCC" align="center">
                      ...
                      if($modificar=="MODIFICAR" and $tagregar==0)
                      {
                          ...
                          $isqlCodigo = "select SGD_CIU_CODIGO  from SGD_CIU_CIUDADANO WHERE  SGD_CIU_CEDULA='$no_documento'";
                          $rs=$db->query($isqlCodigo);
                      $codigoCiu = 0;

                            if  (!$rs->EOF)	$codigoCiu = $rs->fields["SGD_CIU_CODIGO"] ;
                                $flag ==0;
                              }
                      if($codigoCiu != $codigo and $codigoCiu){
                        echo "<center><b><font color=red><center><< No se ha podido MODIFICAR el usuario, Ya existe otro usuario con esa identificaci&oacute;n >> </center></font>";
                        }else{
                      ...
                      if (!$rs){
                        die ("<span class='etextomenu'>No se pudo actualizar SGD_CIU_CIUDADANO ($isql) ");
                                ...
                                echo "<center><b><font color=red><center><< usuario MODIFICADO con &eacute;xito >> </center></font>";
                            }
                      ...
                      $nombre_us1 = strtoupper($nombre_us1);
                      $seg_apell_nus1 = strtoupper($seg_apell_nus1);
                      $prim_apell_nus1 = strtoupper($prim_apell_nus1);
                    ...
                    if($no_documento or $nombre_essp)
                    {
                      ...
                      if($db->driver == "mssql")
                      {
                      $where_split = str_replace("UPPER(sgd_ciu_nombre)","sgd_ciu_nombre",$where_split);
                            $where_split = str_replace("UPPER(sgd_ciu_apell1)","sgd_ciu_apell1",$where_split);
                            $where_split = str_replace("UPPER(sgd_ciu_apell2)","sgd_ciu_apell2",$where_split);
                            $isWhere = "Si";
                      }

                        if($no_documento)
                        {	if($nombre_essp and 	$isWhere == "Si") $isql .= " or "; else 	   $isql .= " where ";
                      ...
                      if($db->driver == "mssql")
                            {
                            $isql = str_replace("UPPER(SGD_OEM_SIGLA)","SGD_OEM_SIGLA",$isql);
                      $isql = str_replace("UPPER(SGD_OEM_OEMPRESA)","SGD_OEM_OEMPRESA",$isql);
                      ...
                      where USUA_ESTA='1' AND USUARIO.depe_codi = dependencia.depe_codi /*AND m.muni_codi = dependencia.muni_codi*/ ";
                      if($nombre_essp)
                      ...
                      if($array_nombre[3]) {$where_split .= " AND UPPER(USUA_NOMB) LIKE '%". $array_nombre[3] ."%' ";}
                            ...
                            if($db->driver == "mssql")
                            {
                            $where_split = str_replace("UPPER(USUA_NOMB)","USUA_NOMB",$where_split);
                            }

                      $isql .= " and $where_split ";
                      }
                      if($no_documento)
                      {	if($nombre_essp) $isql .= " and "; else 	   $isql .= " and ";
                      ...
                      <TD width="6%" align="center" valign="top" class="listado5" colspan="2">
...
break;
	...
	?>
                  ...
                  <tr align="center" >
                    ...
                    <TD><input type="text" name="no_documento1" id="no_documento1" value="<?=$no_documento ?>" size="10" class="ecajasfecha">
                    </TD>
                    <TD>
