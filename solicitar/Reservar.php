...
// incluyo el modulo de historicos
...
function PRESTAMO_action($sAction) {
   ...
   // Registro de una nueva solicitud
      ...
      if($db->conn->query($sSQL)) { 
        echo "Actualizo Regitro de Prestamo.";
        $codTx="66";
        $arrRadicados[] = $fldradicado;
        $hist->insertarHistorico($arrRadicados,  $dependencia , $_SESSION["codusuario"], 0 , 0, "Solicitud Prestamo", $codTx);
    }
	  else{ echo "<script> alert(\" El registro no pudo ser realizado\"); </script>";	}
   }
   
   // Cancelacion, prestamo o devolucion de un documento
   elseif ($sAction=="prestamo" || $sAction=="prestamoIndefinido" || $sAction=="delete" || $sAction=="devolucion" || $sAction=="rechazado") {    
	...
	// Se mantiene el nombre de la variable $fldPRES_ID dentro de tosql() de la version original del modulo.
     ...
     elseif($sAction=="rechazado") { 
       $estadoNew=7;
       //$nombTx="Rechazar solicitud de prestamo"; 
		 $nombTx=$_GET["observa"];
       $setFecha="PRES_FECH_CANC=".$fldPRES_FECH.", USUA_LOGIN_CANC='".$krd."'";
       $COD_TX_RECHAZAR_SOLICITUD_DE_PRESTAMO = "99";
       $codTx = $COD_TX_RECHAZAR_SOLICITUD_DE_PRESTAMO;
   }	
	  // Devolucion	  
     ...
     $codTx="68";
      ...
      // Execute SQL statement 
if($db->conn->query($sSQL)){ 
    $arrRadicados = explode(',', $fldradicado);
    //$db->conn->debug = true;
    $hist->insertarHistorico($arrRadicados,  $dependencia , $_SESSION["codusuario"], 0 , 0, $nombTx, $codTx);
