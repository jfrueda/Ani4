...
// si el usuario autenticado no tiene dependencia, se cierra la sesion
if (!$_SESSION['dependencia']) {
    header("Location: " . $ruta_raiz . "/cerrar_session.php");
}

// Si el usuario no es administrador del sistema, se cierra la sesion
...
//Traemos los grupos
if ($roles->retornarGrupos($grupo_adm, $dependencia)) {
    $grupos = $roles->grupos;
}
...
$grupos_requeridos = $db->conn->getAll('SELECT id, nombre FROM autg_grupos WHERE nombre IN (?, ?)', ['Profesional', 'Contratista']);

$smarty->assign('grupos_requeridos', $grupos_requeridos);
...
if ($_SESSION["usua_admin_sistema"] != 1) {
    $Perm_solo_usuario = $_SESSION['USUA_PERM_ONLY_USER'];
} else {
    $Perm_solo_usuario = 0;
}
...
$smarty->assign("tiene_filtro", UsuarioFiltro::tieneFiltro() ? 1 : 0);

// Desplegamos la plantilla
