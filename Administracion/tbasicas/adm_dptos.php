...
if ($db->conn) {
	if (isset($_POST['btn_accion'])) {
		...
		$dpto_codi =  $_POST['txtIdDpto'];
		$record['DPTO_NOMB'] = strtoupper($_POST['txtModelo']);
		switch ($_POST['btn_accion']) {
			...
			case 'Eliminar': {
					...
					if ($rs->RecordCount() > 0) {
						...
						$ok = $db->conn->Execute('DELETE FROM MUNICIPIO WHERE DPTO_CODI= $dpto_codi', $record['DPTO_CODI']);
						if ($ok) {
...
<script language="JavaScript">
	...
	function ver_listado() {
		continente = document.form1.idcont.options[document.form1.idcont.selectedIndex].value;
		pais = document.form1.idpais.options[document.form1.idpais.selectedIndex].value;
		window.open('listados.php?<?= session_name() . "=" . session_id() ?>&var=dpt&cont=' + continente + '&pas=' + pais, '', 'scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
	}
</script>
									...
									<table class="table table-bordered table-striped">
										...
										<tr bordercolor="#FFFFFF">
											...
											<td class="listado2" height="1">
												<?php
												if (isset($_POST['idpais']) && $_POST['idpais'] > 0) {	// Listamos los Departamentos según pais seleccionado
