...
if ($error) {
	...
	switch ($error) {
		...
		case 5:
			$msg .= "No se puede eliminar Contacto, se encuentra ligado a historial.";
...
<head>
	...
	<script language="JavaScript">
		...
		function ver_listado() {
			window.open('listados.php?<?= session_name() . "=" . session_id() ?>&var=ctt', '', 'scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
		}
		...
		if (is_array($tmp_vector)) {
			echo arrayToJsArray($tmp_vector, 'vc');
		}
		?>
		//
		-->
		...
		<table width="75%" border="1" align="center" class=t_bordeGris>
			<tr bordercolor="#FFFFFF">
				<td width="100%" class="titulos4">
					<B><span class=etexto>
							<center>Administrador de Contactos</center>
						</span></B>
				</td>
			</tr>
		</table>
