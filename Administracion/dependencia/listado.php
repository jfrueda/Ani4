  ...
  <table width="75%" border="1" cellspacing="0" cellpadding="4" bordercolor="#CCCCCC" align="left">
    ...
    <tr>
      <td width="5%" class="vbmenu_control" align="center">
        <strong>
          <font face="Verdana, Arial, Helvetica, sans-serif" size="2"><a href='listado.php?codigo=1&<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>'>Codigo</a></font>
        </strong>
      </td>
      <td width="60%" class="vbmenu_control" align="center">
        <span class="Estilo1">
          <font face="Verdana, Arial, Helvetica, sans-serif" size="2"><a href='listado.php?nombre=1&<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>'>Nombre Dependencia</a></font>
        </span>
      </td>
      <td width="10%" colspan="2" class="tfoot" align="center">
        <font face="Verdana" size="2" color="FFFFFF"><b><a href='perfiles.php?<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>'>Registrar Dependencia</a></b></font>
      </td>
    </tr>
    <?
    while (!$rs->EOF) {
    ?>
      <tr>
        ...
        <td width="5%">
          <font face="Verdana" size="2" color="#000000"><a href='perfiles.php?codigo=<?= $rs->fields["DEPE_CODI"] ?>&editar=1&<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>'><B>Editar</B></a></font>
        <td width="5%">
          <font face="Verdana" size="2" color="#000000"><a href='historico.php?codigo=<?= $rs->fields["DEPE_CODI"] ?>&<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>'><B>Hist&oacute;rico</B></a></font>
      </tr>
    <?
      $rs->MoveNext();
    }
