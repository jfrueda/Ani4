...
if (trim($row["USUA_LOGIN"]) == trim($krd)) {
	...
	if ($row["USUA_NUEVO"] == "1") {
		if (trim($contraxx) == SUBSTR($drde, 1, 26)) {
			...
			echo "    <tr> ";
			echo "      <td width=180  align='left' ><a href='contraxx.php?drde=$drde&krd=$krd&depsel=$dependencia' alt='Cambio de Contrase�as'><img src='iconos/icono_llave.gif' border=0></a></td><td> <a href='estadisticas.php?krd=$krd&drde=$drd'> E </a> </td>";
			echo "      <td width='400'> ";
			...
			if ($orno == 3) {
				$order = " ra_asun $ascdesc";
				$img3 = "<img src='iconos/$imagen' border=0 alt='$data'>";
			}
			if ($orno == 4) {
				$order = " tdoc_desc $ascdesc";
			...
			echo "<tr ><td width=95%>";
			echo "<span class='tpar'><font color=black><b>Documentos:</b>  </font></span> <a href='cuerpo.php?$datosaenviar&orno=7' alt='Ordenar Por Leidos'><span class='tpar'>Le�dos  </span></a>$img7<a href='cuerpo.php?$datosaenviar&orno=8' alt='Ordenar Por Leidos'><span class='tparr'>No Le�dos</span></a>     ";
			echo "";
			...
			if ($carpeta != 11) {
				echo "Acci�n a realizar</b></span><br>\n";
			}
			...
			if ($carpeta != 11) {	// No muestra la opcion por enviar si se encuetra en la carpeta 11
