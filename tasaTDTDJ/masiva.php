...
function generarDocumento($t, $dia, $mes, $anho, $num, $pathAux, $complementDireccion, $asunto) {
    ...
    $causal = '';
    $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('ResolucionTDTDJ.docx');

    if($t[51] == 1) {
        $causal = 'Trasladada la obligación al Grupo de Cobro Persuasivo y Jurisdicción Coactiva, a través de la Resolución ' . $t[52] . ' del ' . $t[53] . ', se libró mandamiento de pago en contra de la ejecutada y a favor de la Superintendencia Nacional de Salud.';
    } else {
        $causal = '';
    }
    ...
    $templateProcessor->setValue('NUMERO TITULO_1', $t[33] != '' ? $t[33] : '------------');
    $templateProcessor->setValue('NUMERO TITULO_2', $t[36] != '' ? $t[36] : '------------');
    $templateProcessor->setValue('NUMERO TITULO_3', $t[39] != '' ? $t[39] : '------------');
    $templateProcessor->setValue('NUMERO TITULO_4', $t[42] != '' ? $t[42] : '------------');
    $templateProcessor->setValue('NUMERO TITULO_5', $t[45] != '' ? $t[45] : '------------');
    $templateProcessor->setValue('NUMERO TITULO_6', $t[48] != '' ? $t[48] : '------------');

    $templateProcessor->setValue('FECHA ELABORACION_1', $t[34] != '' ? $t[34] : '------------');
    $templateProcessor->setValue('FECHA ELABORACION_2', $t[37] != '' ? $t[37] : '------------');
    $templateProcessor->setValue('FECHA ELABORACION_3', $t[40] != '' ? $t[40] : '------------');
    $templateProcessor->setValue('FECHA ELABORACION_4', $t[43] != '' ? $t[43] : '------------');
    $templateProcessor->setValue('FECHA ELABORACION_5', $t[46] != '' ? $t[46] : '------------');  
    $templateProcessor->setValue('FECHA ELABORACION_6', $t[49] != '' ? $t[49] : '------------');    

    $templateProcessor->setValue('VALOR_1',   $t[35] != '' ? '$ ' . number_format($t[35], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_2', $t[38] != '' ? '$ ' . number_format($t[38], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_3', $t[41] != '' ? '$ ' . number_format($t[41], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_4', $t[44] != '' ? '$ ' . number_format($t[44], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_5', $t[47] != '' ? '$ ' . number_format($t[47], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_6', $t[50] != '' ? '$ ' . number_format($t[50], 2, ',', '.') : '------------');

    $direccion = $t[5] != '' ? $t[5] : '--';
    $templateProcessor->setValue('DIRECCION_NO', $direccion . ' en ' . $complementDireccion);
    $templateProcessor->setValue('CORREO_NO', $t[6] != '' ? $t[6] : '--');
    $templateProcessor->setValue('NO_DOC', $t[56]);
    $templateProcessor->setValue('DIA_S', $dia);
    $templateProcessor->setValue('MES_S', $mes);
    $templateProcessor->setValue('ANHO_S', $anho);
    $templateProcessor->setValue('EXP_SP', $t[55]);
...
$asunto = 'Por la cual se termina un proceso de Cobro Coactivo y se ordena la devolución de unos Títulos de Deposito Judicial';
...
foreach ($sheetData as $t) {
    ...
    if($t[0] != '') {
       ...
       if ($nurad=="-1"){
            ...
            */            
            $errorExp = '';
            $sqlValidarExpediente = "SELECT count(*) FROM public.sgd_sexp_secexpedientes WHERE sgd_exp_numero =  '$t[55]'";
            ...
            if($existeExp > 0) {
                ...
                $sqlExpedienteEstado = "SELECT sgd_sexp_estado FROM public.sgd_sexp_secexpedientes  where sgd_exp_numero = '$t[55]'";
                ...
                if($estadoExp == 0) {
                    ...
                    $historialExpediente="INSERT INTO sgd_hfld_histflujodoc(
                        ...
                        usua_doc, usua_codi, depe_codi, sgd_ttr_codigo, sgd_fexp_observa, sgd_hfld_observa, 
                        sgd_fars_codigo, sgd_hfld_automatico) values
                        (0,null, CURRENT_TIMESTAMP,'$t[55]' ,$nurad, '" . $usua_doc_radica . "',$usuRadica,$depeRadica, 53, 
                            null,'Incluir radicado en Expediente', null,null)";
                     $db->conn->Execute($historialExpediente);                  
