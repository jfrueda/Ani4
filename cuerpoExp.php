...
if (isset($_GET["busqExp"])) {
	$busqExp = $_GET["busqExp"];
}
if (isset($_GET["busq_exp"])) {
	$busq_exp = $_GET["busq_exp"];
}
if (isset($_GET["depeBuscada"])) {
	$depeBuscada = $_GET["depeBuscada"];
}
if (isset($_GET["filtroSelec"])) {
...
if ($busq_exp) {
	...
	if (substr($busq_exp, -2) == "or") {
		$busq_exp = substr($busq_exp, 0, strlen($busq_exp) - 2);
	}
	if (trim($busq_exp)) {
		$where_filtro .= "and ( $busq_exp) ";
	}
}
?>
<!DOCTYPE html>
