...
if (
	substr($verrad, -1) == 4 || substr($verrad, -1) == 5 || substr($verrad, -1) == 6 ||
	substr($verrad, -1) == 7
) {
	$sqlParaRadicar = "SELECT  count(id) FROM  anexos where anex_radi_nume = " . $verrad . " and radi_nume_salida is not null";
...
<table WIDTH="100%" align="center" id="tableDocument" class="table">
	...
	?>
			...
			?>
				...
				if ($anex_estado == 3) {
					$img_estado = "<img src='imagenes/docImpreso.gif' title='Se Archivo Radicado y listo para enviar . . .'>";
				}
				if ($anex_estado == 4) {
				...
				<td width="1" valign="middle" align=right>
					...
					if ($pathRadSalida && substr($pathRadSalida, -10) != substr(str_replace("d.", ".", $linkarchivo), -10)) {
						//Se trata de un Radicado
						...
						if (($valImgRs == "SI" || $verradPermisos == "Full")) {
					...
					echo "</td><td>";
					if ($valImg == "SI" || $verradPermisos == "Full") {
	...
	*/
				...
				<td width="1%" valign="middle">
					<font face="Arial, Helvetica, sans-serif">
						...
						$es_extension = ($rs->fields["EXT"] == "rtf" ||
							$rs->fields["EXT"] == "doc" ||
							$rs->fields["EXT"] == "docx" ||
							$rs->fields["EXT"] == "odt" ||
							$rs->fields["EXT"] == "xml") && $no_es_impreso;
						...
						if ($rs->fields["DESTINO_RADICADO"]) {
							$destino = $rs->fields["DESTINO_RADICADO"];
						} else {
							$destino = $rs->fields["DESTINO"];
						}

						if (strlen($cod_radi) <= 18) {
							$sqlAenxDesti = "select anex_destinatario from public.anexos where radi_nume_salida = '$cod_radi'";
							$rsSqlAenxDesti = $db->conn->query($sqlAenxDesti);
							if ($rsSqlAenxDesti->fields["ANEX_DESTINATARIO"] != '') {
								$destino =  $rsSqlAenxDesti->fields["ANEX_DESTINATARIO"];
							}
						}
				...
				<td>
					<font size=1><?= substr($destino, 0, 18) ?></font>
				</td>
				<td>
					<font size=1><?= $rs->fields["TAMA"] ?></font>
				</td>
				<td>
					<font size=1><?= $rs->fields["CREA"] ?></font>
				</td>
				<td>
					<font size=1><?= $rs->fields["DESCR"] ?><br><?= $tprDescrip ?></font>
				</td>
				<td>
				...
				<td>
					<font size=1>
						...
						if ($origen != 1 && $linkarchivo  && $verradPermisos == "Full") {
				...
				<td>
					...
					<?php
							...
							if ($radiNumeAnexo > 0 && trim($linkarchivo)) {
								if (!$codserie) {
									$codserie = "0";
								}
								if (!$tsub) {
								...
								$inact = "javascript:sendMail('$anexCodigo')";
								$glyphi = '';
				...
				<td>
					<?php
							...
							$usuario_creador = $rs->fields["RADI_NUME_SALIDA"] == 0 &&
								$ruta_raiz != ".." &&
								(trim($rs->fields["ANEX_CREADOR"]) == trim($krd) or $es_administrador);


							if ($usuario_creador || $tieneAsignacion) {
								if ($origen != 1  && ($linkarchivo || $tieneAsignacion) && ($npBorrLosDep || $dependencia == 900)) {
									$v = "0";
				...
				<td>
					<?php
							...
							if (intval($tpradic) != 0) {
								if ($permitir_radicar && $es_extension) {
									if ($_SESSION['apiFirmaDigital'] == '' || ($_SESSION['apiFirmaDigital'] != '' && $_SESSION["usua_perm_firma"])) // si se utiliza firma digital permitir radicar los que tienen el permiso
										if (!$rs->fields["RADI_NUME_SALIDA"]) {
											if (((($num_archivos >= 2 || $tieneAnexoSalida == 1) && (substr($verrad, -1) != 2)) || (substr($verrad, -1) == 2 || (substr($verrad, -1) == 9) || $num_archivos >= 2)) &&
												$rs->fields["ANEX_SALIDA"] == 1 && $puedeRadicar == 1 && $_SESSION["usua_perm_firma"] != 0
											) {
											...
											if ((substr($verrad, -1) != 2) && (substr($verrad, -1) != 9) && $num_archivos == 1 && $tieneAnexoSalida == 0 && $rs->fields["ANEX_SALIDA"] == 1 && $_SESSION["usua_perm_firma"] != 0) {
											...
											if ($rs->fields["ANEX_SALIDA"] == 1 && $puedeRadicar == 1 && $_SESSION["usua_perm_firma"] != 0) {
											...
											if ($anex_estado < 4 && $rs->fields["ANEX_SALIDA"] == 1 && $puedeRadicar == 1 && $_SESSION["usua_perm_firma"] != 0) {
			...
			?>
				<td>
					<?php
							if ($origen != 1  && $linkarchivo && $_SESSION['perm_borrar_anexo'] == 1 && $anexTipo == 4 && ($dependencia == 93004 || $dependencia == 900)) {
		...
		<tr align="center">
			<td><small>
					<? if ($permRespuesta == 1) { ?>
						...
						&nbsp;&nbsp;&nbsp;&nbsp;
					<? } ?>
					<a class="vinculos" href='javascript:nuevoArchivo(<? if ($num_archivos == 0 && !$swRadDesdeAnex)  echo "1";
																		else echo "0";  ?>)' class="timpar">
						Anexar Archivo</a>
				</small></td>
		</tr>
	</table>
...
<br><br>
<?php if ($entidad == 'CRA') {
	$opt_ver_anexos_borrados = false;
}
if ($opt_ver_anexos_borrados) { ?>
...
<script language="javascript" defer>
		...
		if (confirm('Se asignar\xe1 un n\xfamero de radicado a \xe9ste documento. Est\xe1 seguro  ?')) {
			...
			$.post("<?= $ruta_raiz ?>/lista_anexos_seleccionar_transaccion.php" + url, function(data) {
				if ((data.success !== undefined) && (data.success.length > 0)) {
					...
					//Logica para lanzar ventana emergente para clasificacion TRD
					if ((data.trd !== undefined) && (data.trd.length > 0)) {
						window.open(data.trd[0], 'Tipificar documento', 'top=0,height=780,width=870,scrollbars=yes,resizable=yes');
					}
				}
			...
			$.post("<?= $ruta_raiz ?>/lista_anexos_seleccionar_transaccion.php" + url, function(data) {
				...
				if ((data.success !== undefined) && (data.success.length > 0)) {
					...
					if ((data.borrador !== undefined) && (data.borrador.length >= 3)) {
						popUpBorrador(data.borrador[1], data.borrador[0], data.borrador[2], 0);
					} else {
						globalCargarPagina('./lista_anexos.php', 'tabs-c');
					}
				}
				...
				if ((data.error !== undefined) && (data.error.length > 0)) {
					...
					$('#tableDocument').removeClass('widget-body-ajax-loading');
					if ((data.borrador !== undefined) && (data.borrador.length >= 3)) {
						popUpBorrador(data.borrador[1], data.borrador[0], data.borrador[2], 1);
					} else {
						globalCargarPagina('./lista_anexos.php', 'tabs-c');
					}
				}
	...
	function popUpBorrador(radAnterior, radNuevo, tipoRadicadp, error) {
		window.open('respuestaRapida/borradorPopUp.php?borrador=' + radAnterior + '&radicado=' + radNuevo + '&error=' + error, 'Mensaje Borrador', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=750,height=460,left = 390,top = 50');
