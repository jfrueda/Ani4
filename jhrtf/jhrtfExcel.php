...
class jhrtf {
            ...
            for($ii=0; $ii < count ($this->datos) ; $ii++){ 
               ...
               // $this->datos[$ii] = $excel->sheets[0]['cells'][$ii];
                ...
                //for ($i = 1; $i <= $excel->sheets[0]['numCols']; $i++) {
                    ...
                    if($campos_d=="*DEPENDENCIA_NOMBRE*") $dependencia_nombre    = $dato_r;
                    if($campos_d=="*USUA_PROYECTO*") $usuario_nombre    = $dato_r;
                    ...
                    if($campos_d=="*EXP_DE_RADICADO*" && $dato_r && strlen($numeroExpediente)<=20){
                     ...
                     if(count($this->objExp->expedientes)>=2){
                         ...
                         $datoNumeroExpediente =  "<font color=red><small>El radicado $dato_r contiene mas de un expediente <br>($exps). No se incluyo!</small></font>";  
                         
                        }elseif(count($this->objExp->expedientes) == 0){
                         $datoNumeroExpediente =  "<font color=red><small>El radicado $dato_r no contiene un Expediente. No se incluyo!</small></font>";  
                        }else{
                ...
                $dir_tipo      = 1;
                $nombre_us = substr(preg_replace('/[^A-Za-z0-9 áéíóúÁÉÍÓÚñÑ\.\-]/u','', trim($nombre_us)),0,100);
                ...
                if($dir2){
                    ...
                    $carp_codi     = substr($dep_radicado,0,2);
                    $dir_tipo      = 1;
                    $nombre_us = substr(preg_replace('/[^A-Za-z0-9 áéíóúÁÉÍÓÚñÑ\.\-]/u','', trim($nombre_us2)),0,100);
                ...
                $observa = "*TRD*$deta_subserie/$deta_subserie (Asigancion tipo documental.)";
                $_Tx_id = 32;

                $objHist->insertarHistorico(array($nurad), $dependencia, $codusuario, $dependencia, $codusuario, $observa,$_Tx_id);
                ...
                if($tipoRad == "1" && !empty($tipoEnvio)){
                    ...
                    if($tipoEnvio == "EMAIL"){
                        ...
                        $isqlE = "UPDATE RADICADO SET mrec_codi = 4 WHERE radi_nume_radi = $nurad";
                        $rsE=$this->conexion->query($isqlE);
                    ...
                    if($tipoEnvio == "FISICO"){
                        ...
                        $isqlE = "UPDATE RADICADO SET mrec_codi = 2 WHERE radi_nume_radi = $nurad";
                        $rsE=$this->conexion->query($isqlE);
                    }
                    if($tipoEnvio == "AMBOS"){
                        ...
                        $isqlE = "UPDATE RADICADO SET mrec_codi = 7 WHERE radi_nume_radi = $nurad";
                        $rsE=$this->conexion->query($isqlE);
                    }
    ...
    function anexarRespuesta($radPadre, $dependencia, $respuesta, $codusuario, $usua_log, $usua_nomb, $usua_doc, $tipoRad,$autofirma)
    {
        ...
        // variables requeridas de processConfig
        global $ABSOL_PATH, $P12_FILE, $P12_PASS, $tsUrlTimeStamp, $tsuUserTimeStamp, $tspPasswordTimeStamp;
        $cwd = getcwd();

        require(dirname(__DIR__, 1).'/respuestaRapida/accion_radicar_anexar.php');

        // volver al directorio requerido de masiva
        chdir($cwd);
    ...
    function validarArchs() {   
        ...
        $this->validarNombre();
        $this->validarSiAnexo();
        $this->validarExpedienteCerrado();
        $this->validarEmails();
        if($this->validarExpediente) $this->validarSiExpedienteExiste();
    }

    /**
     * Valida que los correos en el campo *EMAIL* sean válidos y estén bien formateados.
     * Si encuentra error, lo agrega a $this->errorComplCsv.
     */
    function validarEmails() {
        $colEmail = $this->getNumColEnc("EMAIL");
        if ($colEmail != -1) {
            for ($i = 0; $i < count($this->datos); $i++) {
                $valor = trim($this->datos[$i][$colEmail]);
                if ($valor !== '') {
                    $emails = explode(';', $valor);
                    foreach ($emails as $email) {
                        $email = trim($email);
                        // No espacios dentro del correo y formato válido
                        if (strpos($email, ' ') !== false || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                            $this->errorComplCsv[] = "REG " . ($i+1) . ": El campo *EMAIL* contiene un correo inválido o con espacios: \"$email\"";
                        }
                    }
                }
            }
    ...
    function hayError(){
        if (count($this->errorEncab)>0||count($this->errorPlant)>0||count($this->errorLugar)>0||count($this->errorESP)>0||
            ...
            count($this->errorRadAnexo)>0||count($this->errorExpediente)>0||count($this->errorExpedienteCerrado)>0 )
            return true;
        else
            return false;
    }
    ...
    function validarExpedienteCerrado(){  
        ...
        if ($colExp!=-1){
            ...
            for($i=0; $i < count ($this->datos) ; $i++){  
                ...
                if (strlen(trim($dato)) > 0 && !$objExpediente->expedienteValido($dato))
                {
                    $this->errorExpedienteCerrado[]="REG ".($i+1).": $dato";
                }
            } 
        } 
    }
    ...
    function mostrarError(){    
        ...
        $auxErrorExpedienteCerrado = $this->errorExpedienteCerrado;
        $ruta_raiz = "../..";
