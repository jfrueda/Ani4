...
class jhrtf {
            ...
            for($ii=0; $ii < count ($this->datos) ; $ii++){
                ...
                // Aqui se accede a la clase class_control para actualizar expedientes.
	                ...
	                foreach($this->encabezado as $campos_d){
	                    ...
	                    if($campos_d=="COMENTARIO") 	$comentario    =trim($dato_r);
	                    if($campos_d=="EMPRESA") 		$empresa       =trim($dato_r);
	                    if($campos_d=="NO_GUIA") 		$num_envio     =trim($dato_r);
	                    if($campos_d=="FECHA") 			$fecha_reporte =$dato_r;
	                    if($campos_d=="RADICADO") 	    $radicado_envio=trim($dato_r);
	                    $i++;
	                }

	                $numrad = intval($radicado_envio);
	                if(!empty($numrad) && strlen($numrad)>1 ){
	                	$conexion = & $this->conexion;
		                $queryUpdate = "update SGD_RENV_REGENVIO set
		                    SGD_RENV_OBSERVA = 'Entregado el ".$fecha_reporte." - No. de guia ".$num_envio." - Empresa mensajeria. $empresa - Comentarios. ".$comentario."' where RADI_NUME_SAL = ".$numrad;

		                $rs=$this->conexion->query( $queryUpdate );
		                if (!$rs){
		                    $this->conexion->conn->RollbackTrans();
		                    die ("<span class='etextomenu'>No se ha podido insertar la informaci&oacute;n de la secuencia '$nurad'");
		                }
	                }
	...
	function actualizar_envio_fisico(){
        ...
        clearstatcache();
            ...
            for($ii=0; $ii < count ($this->datos) ; $ii++){
                ...
                // Aqui se accede a la clase class_control para actualizar expedientes.
					...
					if (!empty($numrad) && strlen($numrad) > 1) {
						$conexion = &$this->conexion;
						$queryUpdate = "UPDATE SGD_RENV_REGENVIO 
										SET SGD_RENV_OBSERVA = 'Entregado el " . $fecha_reporte . 
										" - No. de guia " . $num_envio . " - Empresa mensajeria. $empresa - Comentarios. " . 
										$comentario . "' 
										WHERE id = (SELECT id FROM SGD_RENV_REGENVIO 
													WHERE RADI_NUME_SAL = " . $numrad . " 
													AND SGD_FENV_CODIGO NOT IN (105, 106, 109, 985) 
            ...
            foreach ($this->encabezado  as $encabezado) {
                ...
                if($encabezado == '*COMENTARIO*'){
                    ...
                    //var_dump($this->datos[$i-2][$j]);
                     ...
                     $this->datos[$i-2][$j] =str_replace(['ń','Ń'],["ñ","Ñ"],$excel->sheets[0]['cells'][$i][$j]); 
                }
