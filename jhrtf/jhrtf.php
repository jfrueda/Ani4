...
class jhrtf{
     ...
     function combinar_csv($dependencia,$codusuario,$usua_doc,$usua_nomb,$depe_codi_territorial,$codiTRD,$TipoRad){
        ...
        if ($fp){
			...
			{   $i=0;
				...
				// Por cada etiqueta de los campos del encabezado del CSV efectiva un reemplazo
                ...
                $queryUbi = "    
                            select 
                                  pai.id_pais   as PAIS
                                , dep.dpto_codi as DEPTO
                                , mun.muni_codi as MUNI
                                , mun.id_cont   as CONT
                            from 
                                sgd_def_paises pai 
                                , departamento dep 
                                , municipio mun
                            where 
                                  mun.muni_nomb like '$muni_nombre'
                              and mun.id_pais = dep.id_pais
                              and mun.dpto_codi = dep.dpto_codi
                              and mun.id_cont = dep.id_cont
                              and mun.id_cont = pai.id_cont
                              and mun.id_pais = pai.id_pais";                 


                $rsq=$this->conexion->conn->Execute($queryUbi);
                
                if  (!$rsq->EOF){
                    $pais_codi = $rsq->fields['PAIS']; 
                    $dpto_codi = $rsq->fields['DEPTO']; 
                    $muni_codi = $rsq->fields['MUNI']; 
                    $cont_codi = $rsq->fields['CONT']; 
                }


                $this->arrCodPais[$pais_codi];
                $this->arrCodDepto[$dpto_codi];
                $this->arrCodMuni[$muni_codi];
				...
				{     // Segun el tipo de remitente se graba en la tabla respectiva.
                    ...
                    $nurad = $this->btt->radicar_salida_masiva(      $tipoanexo
                                                                    ,$cuentai
                                                                    ,$documento_us3 
                                                                    ,$med 
                                                                    ,$fec
                    ...
                    if($tip_doc==2){
                        $codigo_us = $this->btt->grabar_usuario($doc_us1,$nombre_us1,$direccion_us1,$prim_apell_us1,$seg_apell_us1,$telefono_us1,$mail_us1,$muni_codi,$dpto_codi,$pais_codi,$cont_codi);
                        $tipo_emp_us1=0;
					...
					if($tip_doc==1)
					{	
						$codigo_oem = $this->btt->grabar_oem($doc_us1,$nombre_us1,$direccion_us1,$prim_apell_us1,$seg_apell_us1,$telefono_us1,$mail_us1,$muni_codi);
						$tipo_emp_us1=2;
	...
	/**
     * Gestiona la validaci�n de las archivos que intervienen 
     * en el proceso antes de invocar esta funci�n 
     * debe haberse invocado 	cargar_csv() y abrir();
	*/
	...
	/**
     * Carga los campos obligatorios del tipo de archivo enviado como 
     *parametro y lo hace en el arreglo referenciado en el arreglo definido como par�metro
	...
	*/
	...
	/**
     * Busca si se ha de generar radicados como anexos, 
     * validando que el radicado relacionado exista. 
     * Para esto debe existir una columna en el archivo 
     * csv llamada *RAD_ANEXO* Si existe algun
     * error lo registra en el arreglo errorRadAnexo
	*/
            ...
            if (!$rs->EOF){	
                ...
                if  (!$rs->EOF){	
                    ...
                    $q2= "  select 
                            ...
                            from 
                                municipio 
                            where muni_nomb     ='$dato_muni' 
                                and dpto_codi   = $codigoDepto 
                                and ID_PAIS     = $codigoPais";
