...
<?
	...
	/** Modificado Supersolidaria 01-Nov-2006
		...
		* entidad y la dependencia a la que pertenece.
		*/
  //echo "alert('indice '+ indice + '".$db->entidad."'+ supervisor[indice]);";
	switch( $db->entidad )
	{
		case 'SES':
		...
		print "
					...
					document.getElementById('supervisor_us1').value = supervisor[indice];
			";
		break;
	}
	echo "	}";
...
?>
	...
	$cc_documento = trim($rs->fields["SGD_CIU_CEDULA"]) ;
   	//echo "<hr> $supervisor1 ---->". $db->conn->entidad ."*** <hr>";
	switch( $db->entidad )
	{
	case 'SES':
		/** Modificado Supersolidaria 01-Nov-2006
