    ...
    if ($sec && $vp == "n") {
        ...
        if ($generar_numero != "no" && $radicar_a == "si") {
            ...
            $isql = "update RADICADO
                   set RADI_PATH='$linkarchivo_grabar' {$condUpdPriv}
...
if ($ext == "ODT" || $ext == "odt") {
    ...
    if ((isset($_REQUEST['clave']) || $P12_SERVER) && isset($_SESSION["usua_perm_firma"])) {
        ...
        if ($_SESSION['apiFirmaDigital'] == 'false') {
            ...
            $commandFirmado = 'java -jar ' . $ABSOL_PATH . '/include/jsignpdf/JSignPdf.jar ' . str_replace('docx', 'pdf', $nombreArchivo) . ' ' . ($JSIGNPDF_OPTS ?? '') . ' -kst PKCS12 -ksf ' . $P12_FILE . ' -ksp ' . $clave . ' --font-size 7 -r \'Firmado al Radicar en CADET\' -V -llx 0 -lly 0 -urx 550 -ury 27';
