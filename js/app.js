...
function limitPaste(input) {
    setTimeout(function() {
        var max = parseInt(input.getAttribute('maxlength'), 255);
        if (input.value.length > max) {
            input.value = input.value.substring(0, max);
        }
    }, 0);
}
// Delegado global para cualquier input type="text" con maxlength
document.addEventListener('paste', e => {
    const t = e.target;

    if (t.tagName === 'INPUT' && t.type === 'text' && t.hasAttribute('maxlength')) {
        const max = t.maxLength;

        // Espera a que el navegador ponga el texto, luego recorta
        setTimeout(() => {
