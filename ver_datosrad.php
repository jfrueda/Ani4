...
$radi_fech_vcmto      = $rs->fields["FECH_VCMTO"];
$radi_fech_ofic       = $rs->fields["RADI_FECH_OFIC"];
...
if (!function_exists('sanitize_qs_field')) {
  function sanitize_qs_field($v)
  {
    ...
    $v = str_replace('#', 'No.', $v);
    // Normalizar saltos de línea y tabs a espacio
    $v = str_replace(["\r\n", "\n", "\r", "\t"], ' ', $v);
...
$params_envio = [
  ...
  'dpto_nombre_us11'  => sanitize_qs_field($dpto_nombre_us1),
  'muni_nombre_us11'  => sanitize_qs_field($muni_nombre_us1),
  'direccion_us11'    => sanitize_qs_field($direccion_us1),
  'nombret_us11'      => sanitize_qs_field($nombret_us1),
...
if ($no_tipo != "true") {
  ...
  $isql = "SELECT $radi_nume_radi2 AS RADI_NUME_RADI,
    ...
    r.sgd_mrd_codigo = m.sgd_mrd_codigo AND
    s.id = m.sgd_srd_id AND
    su.sgd_srd_id = m.sgd_srd_id AND
  ...
  //Informacion del modulo de Notificaciones
  if (!empty($esNotificacion)) {
    include_once "$ruta_raiz/include/tx/notificacion.php";

    $notificacion = new Notificacion($db);

    $infoNotificacion = $notificacion->cargarNotificacionAntigua($verradicado);
    ...
    $prioridad_prestablecido = $infoNotificacion["prioridad"] === "t" ? SI : NO;

    $medio_pub_desc      = $notificacion->obtenerMedioPublicacion($medio_pub);
    $caracter_adtvo_desc = $notificacion->obtenerCaracterAdministrativo($caracter_adtvo);
    $ordenesNotificacion = $notificacion->cargarOrdenesDeNotificacion($verradicado);

    if ($esNotificacionCircular) {
      $destinatarios_circ = $notificacion->destinatariosPorRadicado($radicado);
  ...
  $expSeguridad    = $trdExp->getSeguridad($numExpediente);
  ...
  if (!$tdoc) {
    switch ($db->driver) {
      case "oci8":
        ...
        fech_vcmto,
        sgd_tpr_termino,
        fech_vcmto)-to_date(SYSDATE) diasparavencimiento,
        fech_vcmto) - to_date(SUBSTR(TO_CHAR(radi_fech_radi), 1, 8),'dd/mm/yy')  diasplazo,
        to_date(SYSDATE)-to_date(SUBSTR(TO_CHAR(radi_fech_radi), 1, 8),'dd/mm/yy')  diashoy
        from
      ...
      default:
        ...
        fech_vcmto,
        sgd_tpr_termino,
        fech_vcmto::date - current_date as diasparavencimiento,
        fech_vcmto::date - radi_fech_radi::date  diasplazo,
        current_date - radi_fech_radi::date  diashoy
        from
        sgd_tpr_tpdcumento tpr,radicado r
