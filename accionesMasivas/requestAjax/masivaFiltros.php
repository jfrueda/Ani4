...
$com_todos			= 	(!empty($todos)) ? '' : "EXP.SGD_EXP_ESTADO = 	0 AND";	//Filtra los expedientes activos
...
switch ($evento) {
	...
	case 1:
		...
		//rovisional query para oracle.			
		/*$sql1 = "
			select distinct (su.sgd_sbrd_descrip||'-'||su.sgd_sbrd_codigo) AS DETALLE, su.sgd_sbrd_codigo AS CODIGO_SUBSERIE 
			from sgd_mrd_matrird m,
			sgd_sbrd_subserierd su
			where cast(m.sgd_mrd_esta as numeric(1)) = 1 and m.sgd_srd_codigo = $selectSerie 
			and su.sgd_srd_codigo = $selectSerie 
			and su.sgd_sbrd_codigo = m.sgd_sbrd_codigo 
			and sysdate between su.sgd_sbrd_fechini and su.sgd_sbrd_fechfin order by 1
			";
		*/
		$fecha_hoy = Date("Y-m-d");
		$sql1  = "
			select distinct (su.sgd_sbrd_descrip||'-'||su.sgd_sbrd_codigo) AS DETALLE, su.id AS CODIGO_SUBSERIE 
				from sgd_mrd_matrird m, sgd_sbrd_subserierd su
				where
				cast(m.sgd_mrd_esta as numeric(1))       = 1
				and m.sgd_srd_id = $selectSerie
				and su.sgd_srd_id = $selectSerie
...
*/
	...
	case 2:
		...
		//Provicional para ORACLE
		/*$sql2 = "
select distinct (t.sgd_tpr_descrip||'-'||t.sgd_tpr_codigo) as detalle, t.sgd_tpr_codigo AS CODIGO_SUBSERIE 
from sgd_mrd_matrird m, sgd_tpr_tpdcumento t, sgd_sbrd_subserierd sb 
where 
cast(m.sgd_mrd_esta as numeric(1)) = 1 and
m.sgd_srd_codigo = $selectSerie and 
m.sgd_sbrd_codigo =  $selectSubSerie and 
t.sgd_tpr_codigo = m.sgd_tpr_codigo and 
sb.sgd_sbrd_codigo = m.sgd_sbrd_codigo and
sb.sgd_srd_codigo = m.sgd_srd_codigo and 
t.sgd_tpr_estado = 1 and 
sgd_tpr_tp1 >= 1 and 
sysdate between sb.sgd_sbrd_fechini and 
sb.sgd_sbrd_fechfin 
order by detalle
";*/
		if ($ent) {
			$queryTrad = " and sgd_tpr_tp$ent >= 1";
		}
		$sql2 = "
				SELECT DISTINCT (cast(T.SGD_TPR_CODIGO as varchar(5))||' - '||T.SGD_TPR_DESCRIP) AS DETALLE
								,T.SGD_TPR_CODIGO AS CODIGO_SUBSERIE  
					from sgd_mrd_matrird m, sgd_tpr_tpdcumento t, sgd_sbrd_subserierd sb
					where cast(m.sgd_mrd_esta as numeric(1))       = 1
					and m.sgd_srd_id     = $selectSerie
					and m.sgd_sbrd_id    = $selectSubSerie
					and t.sgd_tpr_codigo = m.sgd_tpr_codigo
					and sb.id = m.sgd_sbrd_id
					and sb.sgd_srd_id = m.sgd_srd_id
					and t.sgd_tpr_estado = 1
					$queryTrad
					and '" . $fecha_hoy . "' between sb.sgd_sbrd_fechini
					and sb.sgd_sbrd_fechfin and (cast(m.depe_codi as varchar(100)) = '$depenUsua' or m.depe_codi_aplica  like '%$depenUsua%')";
