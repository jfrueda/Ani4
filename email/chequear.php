...
if ($buscar_por_radicado) {
	$buscar_por_radicado = trim($buscar_por_radicado);
}
...
?>
				<hr>
				<font color=red>POR FAVOR VERIFIQUE CON <?= $krdTmp ?> QUE EL FAX QUE TOMO NO SEA RADICADO POR SEGUNDA VEZ, YA OTRA PERSONA (<?= $krdTmp ?>) lo habia radicado</font>
				<hr>
			<?
			}
			if ($fechRadicacion) {
			?>
				<hr>
				</font color=red>CUIDADO !!!!! ESTE FAX YA APARECE CON FECHA DE RADICACION !!!!!!! <?= $krdTmp ?> CONSULTE
				CON (<?= $krdTmp ?>) FECHA DE RADICACION <?= $fechRadicacion ?></font>
				<hr>
<?
			}
		}
...
if ($tipoMedio == 'eMail' || $_SESSION['tipoMedio'] == 'eMail') {
	if ($_GET['eMailPid']) {
...
<body topmargin="0" bgcolor="#FFFFFF">
	...
	if ($Submit) {
		...
		if ($busq == 2) {
			$noradicado = $buscar_por;
		}
		if ($busq == 3) {
			$documento = $buscar_por;
		}
		if ($busq == 4) {
			$nombres = $buscar_por;
		}
   ...
   <input name="buscareespbot" type=submit class="ebuttons2" value="Limitar EESP">-->
	...
	<form action='radicar.php?<?= session_name() . "=" . trim(session_id()) ?>&krd=<?= $krd ?>&dependencia=<?= $dependencia ?>&faxPath=<?= $faxPath ?>' method="post" name="form1" class="style1">
		<input type=hidden name=ent value='<?= $ent ?>'>
		<input type=hidden name=eMailMid value='<?= $eMailMid ?>'>
		<tr>
			<td colspan="3">
				<div align="center">
						...
						<table border=0 width=100% class="borde_tab">
							<tr align="center">
								...
								<td width="33%" class="listado2" height="25">
									<center><input name="rad0" type=submit class="botones_funcion" value="Como Anexo"></center>
								</td>
								<td width="33%" class="listado2" height="25">
					...
					echo "</table>";
					error_reporting(7);
					if ($primera != 1 && $Submit == "BUSCAR" && ($buscar_por_cuentai || $buscar_por_radicado || $buscar_por_nombres || $buscar_por_doc || $buscar_por_dep_rad || $buscar_por_exp)) {
						...
						if ($whereTrd) {
							$whereTrd = " AND d.SGD_TRD_CODIGO IN ($whereTrd)";
						} else {
							$whereTrd = "";
						}
						...
						?>
							<table width='100%' class="borde_tab">
								...
								if ($cent == 0 && $tpBuscarSel == "ok") {
									?>
									<tr>
										<td align='center' colspan='7' CLASS=titulosError>&nbsp;
											LO SENTIMOS NO HAY REGISTROS</font>
										</td>
									</tr>
								<?
								}
