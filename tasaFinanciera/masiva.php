...
function generarDocumento($t, $dia, $mes, $anho, $num, $pathAux, $complementDireccion, $nombreRadica, $nombreEnvio) {

    if($t[31] == 'APERTURA')
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('ResolucionApertura.docx');
    else 
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('Resolucion.docx');
        
    $templateProcessor->setValue('RA_NOTI_S', $num);
    $templateProcessor->setValue('RA_ASUN', $t[2]);
    $templateProcessor->setValue('CONTRI', $t[3]);
    $templateProcessor->setValue('NIT_NO', $t[4]);
    $templateProcessor->setValue('EMAIL_CONTRI', $t[5]);
    $templateProcessor->setValue('VIGILADO', $t[6]);
    $templateProcessor->setValue('NIT_VIG', $t[7]);
    $templateProcessor->setValue('EMAIL_VIG', $t[8]);
    $templateProcessor->setValue('RESOLU', $t[9]);
    $templateProcessor->setValue('RESOLU_DIA', $t[10]);
    $templateProcessor->setValue('RESOLU_MES', $t[11]);
    $templateProcessor->setValue('RESOLU_ANO', $t[12]);
    $templateProcessor->setValue('VALOR', number_format($t[13], 2, ',', '.'));
    $templateProcessor->setValue('PORCEN', $t[14]);
    $templateProcessor->setValue('VAL_LETR', $t[15]);
    $templateProcessor->setValue('VAL_CONT', number_format($t[16], 2, ',', '.'));
    $templateProcessor->setValue('EXP_SP', $t[17]);
    $templateProcessor->setValue('DIR_CONTRI', $t[18]);
    $templateProcessor->setValue('IDTR_MUNIC', $t[20]);

    $templateProcessor->setValue('RANGO_1', $t[21]);
    $templateProcessor->setValue('TARIF_1', $t[22]);
    $templateProcessor->setValue('RANGO_2', $t[23]);
    $templateProcessor->setValue('TARIF_2', $t[24]);    
    $templateProcessor->setValue('RANGO_3', $t[25]);
    $templateProcessor->setValue('TARIF_3', $t[26]);    
    $templateProcessor->setValue('RANGO_4', $t[27]);
    $templateProcessor->setValue('TARIF_4', $t[28]);    
    $templateProcessor->setValue('RANGO_5', $t[29]);
    $templateProcessor->setValue('TARIF_5', $t[30]);    
    
    $templateProcessor->setValue('DIA_S', $dia);
    $templateProcessor->setValue('MES_S', $mes);
    $templateProcessor->setValue('ANHO_S', $anho);
    $templateProcessor->setValue('USUA_PROYECTO', $nombreRadica . '  – Profesional');
    $templateProcessor->setValue('USUA_APROBO', $nombreEnvio . '  – Director(a) Dirección Financiera');
    $templateProcessor->setValue('FIRMA', 'Firmado electrónicamente por: ' . $nombreEnvio);
    $templateProcessor->setValue('FIRMANTE_NOMBRE', $nombreEnvio);
...
$isqlDepRadica = "select usua_nomb, usua_login,usua_doc from usuario where depe_codi = $depeRadica and usua_codi = $usuRadica";
$rsisqlDepRadica = $db->conn->Execute($isqlDepRadica);

$krdCreador = $rsisqlDepRadica->fields['USUA_LOGIN'];
$usua_doc_radica = $rsisqlDepRadica->fields['USUA_DOC'];
$nombreRadica = $rsisqlDepRadica->fields['USUA_NOMB'];

$isqlDepFirma = "select usua_nomb, usua_login,usua_doc from usuario where depe_codi = $depeEnvio and usua_codi = $usuEnvio";
$rsisqlDepFirma = $db->conn->Execute($isqlDepFirma);
$nombreEnvio = $rsisqlDepFirma->fields['USUA_NOMB'];
       ...
       if ($nurad=="-1"){
            ...
            */  
            ...
            $hist->insertarHistorico($radicadosSel,
                ...
                generarDocumento($t, $dia, $mes, $anho, $nurad,  $pathAux, $complementDireccion, $nombreRadica, $nombreEnvio);
            ...
            */            
            $errorExp = '';
            $sqlValidarExpediente = "SELECT count(*) FROM public.sgd_sexp_secexpedientes WHERE sgd_exp_numero =  '$t[17]'";
            ...
            if($existeExp > 0) {
                ...
                $sqlExpedienteEstado = "SELECT sgd_sexp_estado FROM public.sgd_sexp_secexpedientes  where sgd_exp_numero = '$t[17]'";
                ...
                if($estadoExp == 0) {
                    ...
                    $historialExpediente="INSERT INTO sgd_hfld_histflujodoc(
                        ...
                        usua_doc, usua_codi, depe_codi, sgd_ttr_codigo, sgd_fexp_observa, sgd_hfld_observa, 
                        sgd_fars_codigo, sgd_hfld_automatico) values
                        (0,null, CURRENT_TIMESTAMP,'$t[17]' ,$nurad, '" . $usua_doc_radica . "',$usuRadica,$depeRadica, 53, 
                            null,'Incluir radicado en Expediente', null,null)";
                     $db->conn->Execute($historialExpediente);                  
