...
function Header()
{
    ...
    $this->SetXY($l,20+$margen);
 	$this->Cell(472,14,'REMITENTE: FUNDACION CORRELIBRE.ORG',1,1,'L');
    $this->SetXY($l,34+$margen);
 	$this->Cell(472,13,'CRA 7',1,1,'L');
    $this->SetXY($l,47+$margen);
 	$this->Cell(472,13,'CONTRATO No.  ',1,1,'L');
	$l = 500;
	$this->SetXY($l,20+$margen);
 	$this->Cell(525,14,'ADMINISTRACION  POSTAL NACIONAL',1,1,'C');
    $this->SetXY($l,34+$margen);
 	$this->Cell(525,13,'PLANILLA PARA  CONSIGNACION',1,1,'C');
    $this->SetXY($l,47+$margen);
 	$this->Cell(525,13,'ENVIOS CON CONTRATO',1,1,'C');
	$l = 1025;
	$this->SetXY($l,20+$margen);
...
function Footer() {
    ...
    if(!$this->footerset[$this->page]) {
        ...
        $this->SetY(-30+$margen);
        $this->Cell(615,20,strtoupper($this->dependencia),1,1,'C');
 	// AL LADO 
        $this->SetY(-90+$margen);
		$this->SetX(642);		
...
function morepagestable($db,$lineheight=10,$query,$row_ini,$row_fin) {
    ...
    $row = 1;
	error_reporting(7);
//	$this->connect('localhost','root','',$db);
	$this->query($query,$db);
			...
			if($i_datos>=$row_ini and $i_datos<=($row_fin))
	   		{
			    ...
			    $gp    = $this->rs->fields[13];    //	   $gp = trim(ora_getcolumn($this->cursor,14));
    	  		$gp1   = $this->rs->fields[2]; //  $gp1 =trim(ora_getcolumn($this->cursor,2));
      			$gptot = $this->rs->fields[12];	 //  $gptot = trim(ora_getcolumn($this->cursor,13));
	    
	   			$registro++;
	   			if($gptot!=0)
				{		

		    		for($col=0;$col<$this->rs->FieldCount()-1;$col++) //  for($col=0;$col<ora_numcols($this->cursor)-1;$col++)
    	    		{	
        	    		$txt = $this->rs->fields[$col];	//$txt = ora_getcolumn ($this->cursor,$col);
	  					if($col==2)  $txt .= $grupo;
//						if($col==7)  $subtotal7 = $txt + $subtotal7;
						if($col==8)  $subtotal8 = $txt + $subtotal8;
						if($col==9)  $subtotal9 = $txt + $subtotal9;
						if($col==10)  $subtotal10 = $txt + $subtotal10;		
						if($col==11)  $subtotal11 = $txt + $subtotal11;		
						if($col==12)  $subtotal12 = $txt + $subtotal12;			
						if($col==13) $total = $txt + $total;			 
			    		if($col==0  ) 
						{
							if ($this->rs->fields[0] == 1)  //	if( ora_getcolumn($this->cursor,0)==1)
			    			{
								//$this->rs->FetchInto(&$txt);			
									    $txt=$row; 
							}
						}
            			$this->page = $currpage;
		...
		{ 
        	...
        	{	
			    $txt = "";
//				if($col==7) $txt = $subtotal7;
				if($col==8) $txt = $subtotal8;
				if($col==9) $txt = $subtotal9;			
				if($col==10) $txt = $subtotal10;			
				if($col==11) $txt = $subtotal11;	
				if($col==12) $txt = $subtotal12;			
				if($col==13) $txt = $total;	
			    if($col==0) $this->rs->FetchInto($txt);		//$txt=$row; 
	            $this->page = $currpage;
	            $this->SetXY($l,$h); 
...
function oracle_report($db,$query,$dump=false,$attr=array(),$head_table=array(),$head_table_size=array(),$file_pdf,$row_ini,$row_fin)
{
...
//        for($i=0;$i<$this->numFields-1;$i++){
        for($i=0;$i<$this->numFields-1;$i++){
           ...
           //$stringWidth = $this->getstringwidth($this->rs->FetchField($i)) + 6 ;   // $stringWidth = $this->getstringwidth(Ora_ColumnName($this->cursor,$i)) + 6 ;
           $stringWidth = $this->getstringwidth($this->rs->FetchField($i)->name) + 6;
            ...
            $this->colTitles[$i] =  $this->rs->FetchField($i);  //Ora_ColumnName($this->cursor,$i)-1;
            $this->colTitles[$i] =$head_table[$i];
  		    $nombCampo = $this->rs->FetchField($i);
	        $tipoCampo = $this->rs->MetaType($nombCampo->type);
        ...
        if($dump){
            ...
            for($i=0;$i<$this->numFields-1;$i++){
            if(strlen($this->rs->FetchField($i))>$flength){  //if(strlen(Ora_ColumnName($this->cursor,$i))>$flength){
                $flength = strlen($this->rs->Fields[$i]);  //$flength = strlen(Ora_ColumnName($this->results,$i));
                }
            }
            switch($this->k){
                case 72/25.4:
...
*/					
   ...
   for($i=0;$i<$this->numFields-1;$i++){
          ...
          $this->colTitles[$i] = $this->rs->FetchField($i) ; // $this->colTitles[$i] = Ora_ColumnName($this->cursor,$i) ;
    	   $nombCampo = $this->rs->FetchField($i);
	        $tipoCampo = $this->rs->MetaType($nombCampo->type);
	...
	$this->AddPage();
    $this->morepagestable($db,$this->FontSizePt+11,$query,$row_ini,$row_fin);
