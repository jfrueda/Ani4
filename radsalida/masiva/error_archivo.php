...
<span class=tituloListado>Se termin&oacute; de realizar la verificaci&oacute;n de los archivos 
y se encontraron los siguientes errores: </span> <BR>
<?
//variable que viene de radasalida/masiva/adjuntar_masivaExcel.php
$phpsession  = session_name()."=".session_id()."&krd=".$_SESSION["krd"];
...
if (count($auxErrLugar)>0)  {
  ...
  if (count($auxErrorExpediente)>0)  {
  ?>
  Los siguientes datos de Expedientes en radicados asociados no existen !<BR>
  <?  
   $num = count($auxErrorExpediente);
   $i = 0; 
  
  while ($i < $num) {
      
      $record_id = key($auxErrorExpediente); 
      echo ("*".$auxErrorExpediente[$record_id]."*"."<BR>");
      next($auxErrorExpediente); 
      ++$i; 
   }
   
  }  

if (count($auxErrorExpedienteCerrado)>0)  {
?>
Los siguientes expedientes estan cerrados o anulados:<BR>
<?  
	$num = count($auxErrorExpedienteCerrado);
	$i = 0; 
	
		while ($i < $num) {
			
			$record_id = key($auxErrorExpedienteCerrado); 
			echo ("*".$auxErrorExpedienteCerrado[$record_id]."*"."<BR>");
			next($auxErrorExpedienteCerrado); 
			++$i; 
		}
...
?>	
  ...
  <BR>
   <a type="button" href="../masiva3/upload2PorExcel.php?<?=$phpsession?>" class="botones">Menu Masiva</a>
  </span> </p>
