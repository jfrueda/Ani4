...
//generados por la dependencia y da la opción de recuperar el listado de cualquiera de
...
/**
 * Programa que muestra en formato PDF un conjunto de radicados 
 * que hacen parte de un grupo de masiva, 
 * seleccionado previamente desde 
 * cuerpo_masiva_recuperar_listado.php
 */
...
if (trim(strlen($grupo))){
	...
	while ($i < $num) {	
		...
		$data=  array_merge ($data,array (array('NUMERO RADICADO'=>$radsGrupo[$i],'FECHA RADICADO'=>$rad->getRadi_fech_radi(),'DESTINATARIO'=>utf8_decode($datosRad["nombre"]),'DIRECCION'=>utf8_decode($datosRad["direccion"]),'DEPARTAMENTO'=>$datosRad["deptoNombre"],'MUNICIPIO'=>utf8_decode($datosRad["muniNombre"]),'ELIMINADO'=>$eliminado)));				
		$i++; 
	}
...
<form action="menu_masiva.php?<?=$params?>" name="formMensaje">
  ...
  <table width="53%" align="center" border=0 border=0 cellpadding=0 cellspacing=5 class='borde_tab' >
    <tr align="center"> 
      ...
      <tr> <td align="center">
      <input name="cerrar" type="button"  class="botones" id="envia22"  value="Volver"   onClick="history.back()"  >
      </td>
  </tr>
</table>
</form>
