...
class Anulacion{
     ...
     */
    var $db;
    var $radicadosErrorSolicitud;  // Arreglo con radicadon que no fue posible solicitar la anulacion.
...
*/
      ...
      $anuId = $this->db->conn->GenID( "sgd_anu_id" );
       ...
       $comentario = substr($comentario,0,246);
           ...
           if ($this->db->conn->Execute($sql) == false) {
                ...
                $isql = "UPDATE radicado SET radi_depe_actu = 999, radi_usua_actu = 1, carp_codi=12, carp_per=1,SGD_EANU_CODIGO=1 WHERE radi_nume_radi = $noRadicado";
                if ($this->db->conn->Execute($isql) == false) {
                  print 'error Archivar: '.$this->db->conn->ErrorMsg().'<BR>';
                  //$radicadosErrorSolicitud[] = $noRadicado;
                  $varcountError=$varcountError+1;
                  $radicadosErrorSolicitud= $noRadicado.",";
                }else{
                  $radicadosOkSolicitud[] = $noRadicado; 
                  $varCount= $varCount+1;
                }
            }
    ...
    $this->radicadosErrorSolicitud = $radicadosErrorSolicitud;  
    return ($radicadosOkSolicitud);
