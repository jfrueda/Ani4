...
class Usuario {
    ...
    private function removeInvalidCharacters($array) {
        $validArray = array();
    
        foreach ($array as $key => $value) {
            $validArray[$key] = mb_convert_encoding($value, 'UTF-8', 'UTF-8');
        }
     ...
     *
     */
    public function guardarUsuarioRadicado($user, $nurad, $borrable=true){
        ...
        $labels=['MUNI_CODI'=>'Municipio','DPTO_CODI'=>'Departamento','ID_PAIS'=>'Pais','ID_CONT'=>'Continente',
            'SGD_TRD_CODIGO'=>'Trd','SGD_DIR_DIRECCION'=>'Dirección','SGD_DIR_TELEFONO'=>'Teléfono',
            'SGD_DIR_TDOC'=>'TDoc','SGD_DIR_MAIL'=>'email','SGD_DIR_CARGO'=>'Cargo','SGD_DIR_TIPO'=>'DirTipo',
            'SGD_DIR_APELLIDO'=>'Apellido','SGD_DIR_NOMBRE'=>'Nombre','SGD_DIR_NOMREMDES'=>'Nombre Completo',
            'SGD_DIR_DOC'=>'Documento','MEDIO_ENVIO'=>'Medio de envío'
        ];
	...
	if($prev_dir['MUNI_CODI'] != $user['muni_tmp']){
		$record['MUNI_CODI']         = $user['muni_tmp'];
		$reMod[$labels['MUNI_CODI']]         = $prev_dir['MUNI_CODI'];
	}
	if($prev_dir['DPTO_CODI'] != $user['dpto_tmp']){
		$record['DPTO_CODI']         = $user['dpto_tmp'];
		$reMod[$labels['DPTO_CODI']]         = $prev_dir['DPTO_CODI'];
	}
	if($prev_dir['ID_PAIS'] != $user['pais_tmp']){
		$record['ID_PAIS']           = $user['pais_tmp'];
		$reMod[$labels['ID_PAIS']]         = $prev_dir['ID_PAIS'];
	}
	if($prev_dir['ID_CONT'] != $user['cont_tmp']){
		$record['ID_CONT']           = $user['cont_tmp'];
		$reMod[$labels['ID_CONT']]         = $prev_dir['ID_CONT'];
	}	
	if($prev_dir['SGD_TRD_CODIGO'] != $user['sgdTrd']){
		$record['SGD_TRD_CODIGO']    = $user['sgdTrd']; // Tipo de documento
		$reMod[$labels['SGD_TRD_CODIGO']]         = $prev_dir['SGD_TRD_CODIGO'];
	}
	if($prev_dir['SGD_DIR_DIRECCION'] != $user['direccion']){
		$record['SGD_DIR_DIRECCION'] = $user['direccion'];
		$reMod[$labels['SGD_DIR_DIRECCION']]         = $prev_dir['SGD_DIR_DIRECCION'];
	}
	if($prev_dir['SGD_DIR_TELEFONO'] != $user['telef']){
		$record['SGD_DIR_TELEFONO']  = $user['telef'];
		$reMod[$labels['SGD_DIR_TELEFONO']]         = $prev_dir['SGD_DIR_TELEFONO'];
	}
	if($prev_dir['SGD_DIR_TDOC'] != $user['tdid_codi']){
		$record['SGD_DIR_TDOC']      = $user['tdid_codi'];
		$reMod[$labels['SGD_DIR_TDOC']]         = $prev_dir['SGD_DIR_TDOC'];
	}
	if($prev_dir['SGD_DIR_MAIL'] != $user['email']){
		$record['SGD_DIR_MAIL']      = $user['email'];
		$reMod[$labels['SGD_DIR_MAIL']]         = $prev_dir['SGD_DIR_MAIL'];
	}
	if($prev_dir['SGD_DIR_CARGO'] != $user['cargo']){
		$record['SGD_DIR_CARGO']     = $user['cargo'];
		$reMod[$labels['SGD_DIR_CARGO']]         = $prev_dir['SGD_DIR_CARGO'];
	}
	if($prev_dir['SGD_DIR_TIPO'] != $sgdDirTipo){
		$reMod[$labels['SGD_DIR_TIPO']]         = $prev_dir['SGD_DIR_TIPO'];
	}
    

    $record['SGD_DIR_TDOC']     = $user['dir_tdoc'];
    ...
    //$this->db->log_error ("002-- $nurad ", $borrable ? "borrable" : "no borrable");
    if (!$borrable)
    {
        $record['BORRABLE'] = 0;
    }
	...
	if($prev_dir['SGD_DIR_APELLIDO'] != $sgd_dir_apellido_var){
		$record['SGD_DIR_APELLIDO'] =  $sgd_dir_apellido_var;
		$reMod[$labels['SGD_DIR_APELLIDO']]         = $prev_dir['SGD_DIR_APELLIDO'];
	}
	...
	if($prev_dir['SGD_DIR_NOMBRE'] != trim($sgd_dir_nombre_var)){
		$record['SGD_DIR_NOMBRE']    = trim($sgd_dir_nombre_var);
		$reMod[$labels['SGD_DIR_NOMBRE']]         = $prev_dir['SGD_DIR_NOMBRE'];
	}

        ############### COMPRUEBO QUE DEBO COLOCAR EN NOMREMDES
        if($user['dignatario'] && $user['sgdTrd']==2){

            //$sgd_dir_nombre_var = $user['dignatario'];
        /*if(strlen($sgd_dir_nombre_var)>=499){$sgd_dir_nombre_var= substr($sgd_dir_nombre_var,0,499);}
                if($prev_dir['SGD_DIR_NOMREMDES'] != trim($user['dignatario'])){
                //$record['SGD_DIR_NOMREMDES'] = $sgd_dir_nombre_var;
                $record['SGD_DIR_NOMREMDES']         = trim($user['dignatario']);
                $reMod['Remitente']         = $prev_dir['SGD_DIR_NOMREMDES'] ;
                }*/
                $record['SGD_DIR_NOMREMDES']         = trim($user['dignatario']);
        }else{
            $record['SGD_DIR_NOMREMDES']         = trim($user['dignatario']);
/*
            ...
            $sgd_dir_nombre_var = $user['nombre'];
            $sgd_dir_apellido_var  = $user['apellido'];
            if($prev_dir['SGD_DIR_NOMREMDES'] != $sgd_dir_nombre_var.' '.$sgd_dir_apellido_var){
                $record['SGD_DIR_NOMREMDES'] = $sgd_dir_nombre_var.' '.$sgd_dir_apellido_var;
                $reMod[$labels['SGD_DIR_NOMREMDES']]         = $prev_dir['SGD_DIR_NOMREMDES'];
            }*/
        }
        ################
	if($prev_dir['SGD_DIR_DOC'] != $user['cedula']){
		$record['SGD_DIR_DOC']       = empty($user['cedula'])?  '0' : $user['cedula'];
		$reMod[$labels['SGD_DIR_DOC']]         = $prev_dir['SGD_DIR_DOC'];
	}

        $record['RADI_NUME_RADI']    = $nurad; // No de radicado
	if($prev_dir['SGD_SEC_CODIGO'] != 0)
        ...
        if(!$record['SGD_OEM_CODIGO'] && $prev_dir['SGD_OEM_CODIGO'] != 0) $record['SGD_OEM_CODIGO']=0;
        if(!$record['SGD_DOC_FUN'] && $prev_dir['SGD_DOC_FUN'] != 0)    $record['SGD_DOC_FUN']=0;
        if(!$record['SGD_CIU_CODIGO'] && $prev_dir['SGD_CIU_CODIGO'] != 0) $record['SGD_CIU_CODIGO']=0;

        if (!empty($user['medio_envio']) && $prev_dir['MEDIO_ENVIO'] != $user['medio_envio']) {
            $record['MEDIO_ENVIO'] = $user['medio_envio']; // Medio de envio del modulo de Notificaciones
		$reMod[$labels['MEDIO_ENVIO']]= $prev_dir['MEDIO_ENVIO'];
        }
	if(is_array($prev_dir))
		$prev_dir=true;
	if(sizeof($record)<=3){
            $this->result =  array("state" => false, "value" => $nextval, 'record'=>$reMod, 'prev'=>$prev_dir);
            return true;
	 }

        $record = $this->removeInvalidCharacters($record);

        //$this->db->log_error ("003-- $nurad ", is_array($esUpdate) ? "editar" : "crear");
        ...
        if(!empty($insertSQL)){
            $this->result =  array("state" => true, "value" => $nextval, 'record'=>$reMod, 'prev'=>$prev_dir);
            return true;
        }else{
            ...
            if(!empty($insertSQL)){
                $this->result =  array("state" => true, "value" => $record['SGD_DIR_CODIGO'], 'record'=>$reMod, 'prev'=>$prev_dir);
                return true;
            }else{
    ...
    function agregarUsuario($data){
        ...
        $nuevo = true;
        return $this->resRadicadoHtml($nuevo);
    }
    ...
    function parsearNombreCompleto($nombreCompleto) {
        $nombresComunes = ['juan', 'john', 'jairo', 'maria', 'ana', 'carlos', 'david', 'ingrid', 'patricia', 'luis', 'jose', 'andres', 'sandra', 'laura'];
        
        $nombreCompleto = trim(preg_replace('/\s+/', ' ', $nombreCompleto));
        $nombreCompleto = preg_replace('/\s*\d+\s*$/', '', $nombreCompleto);
        if (empty($nombreCompleto)) return null;

        $palabras = explode(' ', $nombreCompleto);
        $total = count($palabras);

        $capitalizar = fn($txt) => ucwords(mb_strtolower(trim($txt), 'UTF-8'));

        if ($total === 1) {
            return [
                'primer_nombre' => $capitalizar($palabras[0]),
                'segundo_nombre' => '',
                'primer_apellido' => '',
                'segundo_apellido' => ''
            ];
        ...
        if ($total === 2) {
            return [
                'primer_nombre' => $capitalizar($palabras[0]),
                'segundo_nombre' => '',
                'primer_apellido' => $capitalizar($palabras[1]),
                'segundo_apellido' => ''
            ];
        }

        if ($total === 3) {
            $primero = mb_strtolower($palabras[0]);
            $segundo = mb_strtolower($palabras[1]);

            if (in_array($primero, $nombresComunes) && in_array($segundo, $nombresComunes)) {
                // Caso: nombre compuesto + un apellido
                return [
                    'primer_nombre' => $capitalizar($palabras[0]),
                    'segundo_nombre' => $capitalizar($palabras[1]),
                    'primer_apellido' => $capitalizar($palabras[2]),
                    'segundo_apellido' => ''
                ];
            } else {
                // Caso: un nombre + dos apellidos
                return [
                    'primer_nombre' => $capitalizar($palabras[0]),
                    'segundo_nombre' => '',
                    'primer_apellido' => $capitalizar($palabras[1]),
                    'segundo_apellido' => $capitalizar($palabras[2])
                ];
            }
        }
        ...
        if ($total === 4) {
            return [
                ...
                'segundo_nombre' => $capitalizar($palabras[1]),
                'primer_apellido' => $capitalizar($palabras[2]),
                'segundo_apellido' => $capitalizar($palabras[3])
            ];
        }

        // Más de 4 palabras
        $segundoApellido = array_pop($palabras);
        $primerApellido = array_pop($palabras);
        $primerNombre = array_shift($palabras);
        $segundoNombre = implode(' ', $palabras);

        return [
            'primer_nombre' => $capitalizar($primerNombre),
            'segundo_nombre' => $capitalizar($segundoNombre),
            'primer_apellido' => $capitalizar($primerApellido),
            'segundo_apellido' => $capitalizar($segundoApellido)
        ];
    }

    function procesarNombreApellidoEnt3($result) {
        // Verificamos si el tipo de radicado es 3 y el apellido está vacío
        if (in_array($result["TIPO_RADICADO"], [3, 6]) && empty(trim($result["APELLIDO"])) && !empty(trim($result["NOMBRE"]))) {
            $partes = $this->parsearNombreCompleto($result["NOMBRE"]);
            
            // Combinar primer y segundo nombre
            $nombresCompletos = trim($partes['primer_nombre'] . ' ' . $partes['segundo_nombre']);
            
            // Combinar primer y segundo apellido
            $apellidosCompletos = trim($partes['primer_apellido'] . ' ' . $partes['segundo_apellido']);
            
            // Si tenemos apellidos, actualizar los resultados
            if (!empty($apellidosCompletos)) {
                $result["NOMBRE"] = $nombresCompletos;
                $result["APELLIDO"] = $apellidosCompletos;
            }
        }
        return $result;
    }
    ...
    public function resRadicadoHtml($nuevo = false){
        ...
        foreach ($this->result as $k => $result){
            ...
            if($tipo == 2){
                $options = "<option value='4'>Nit</option>";
            }
            ...
            $html = '<table style="width:100%;">
                        <tr>';
                $html .= (
                                $borrable == 1 || is_null($borrable) ? 
                                '<td class="search-table-icon">
                                    <div class="row-fluid">
                ...
                $html .= '<td>
                            <label name="inp_'.$idtx.'_ced" class="input">
                                Numero documento
                                <input id="id_documen_'.$incremental.'" type="text" name="'.$idtx.'_cedula" maxlength="13" onpaste="limitPaste(this)" value="'.$result["CEDULA"].'">
                            </label>
                        </td>';
            ...
            if($ent >= 4 && ($tipo == 0 || $tipo == 6)) {
                ...
                if (in_array($ent, [6]) && empty(trim($result["APELLIDO"]))) {
                    $result = $this->procesarNombreApellidoEnt3($result);
                }
            } elseif($ent >= 4 && $tipo == 2) {
                $auxLabelNombre = "*Nombre";
                ...
                if (in_array($ent, [3]) && empty(trim($result["APELLIDO"]))) {
                    $result = $this->procesarNombreApellidoEnt3($result);
                }
            } else {
            ...
            $inputApell = ($auxLabelApellido === "Representante Legal de la Entidad")
                    ? '<input type="text" data-role="representante-legal" data-rel="solo-text" id="id_apellido_' . $incremental . '" name="' . $idtx . '_apellido" maxlength="50" onpaste="limitPaste(this)" value="' . $result["APELLIDO"] . '"'
                    : '<input type="text" data-rel="solo-text" id="id_apellido_' . $incremental . '" name="' . $idtx . '_apellido" maxlength="50" onpaste="limitPaste(this)" value="' . $result["APELLIDO"] . '"';

            $html .= ' <td colspan="2">
                            <label name="inp_'.$idtx.'_nomb" class="input">
                                ' . $auxLabelNombre .'
                                <input type="text" data-rel="solo-text" id="id_nombre_'.$incremental.'" name="'.$idtx.'_nombre" maxlength="149" onpaste="limitPaste(this)" value="'.$result["NOMBRE"].'">
                            </label>
                        </td>';
            ...
            $html .= '<tr><td></td>';
                $html .= '<td>
                    <label name="inp_'.$idtx.'_dire" class="input">
                        Dirección
                        <input id="id_dir_'.$incremental.'" type="text" name="'.$idtx.'_direccion" maxlength="149" onpaste="limitPaste(this)"
                ...
                $html .= '<td>
                    <label name="inp_'.$idtx.'_email" class="input">
                        ...
                        onChange="validate(this)" value="'.$result["EMAIL"].'">
                    </label>
                </td>';
                $html .= '<td>
                    <label name="inp_'.$idtx.'_tel" class="input">
                        Teléfono
                        <input type="text" id="id_telefono_'.$incremental.'" name="'.$idtx.'_telefono" maxlength="49" onpaste="limitPaste(this)" value="'.$result["TELEF"].'">
                    </label>
                </td>';
                $html .= '<td>
            ...
            $html .= '<tr><td></td>';
                $html .= '<td><div ' . $auxFlagDignatario . '>
                    <label name="inp_'.$idtx.'_digna" class="input">
                        ' . $auxLabelDignatario . '
                        <input id="id_dignatario_'.$incremental.'" data-rel="solo-text" type="text" maxlength="250" onpaste="limitPaste(this)" name="'.$idtx.'_dignatario"
            ...
            if($ent >=1 && $ent <= 3) {
                ...
                ' . $auxLabelCargo .'
                    <input  type="text" data-rel="solo-text" id="id_cargo_'.$incremental.'" name="'.$idtx.'_cargo" maxlength="100" onpaste="limitPaste(this)" value="'.$result["CARGO"].'">
                </label>
                    </td>';
            }   
...
/****************************************************************************************************************/
/** FUNCIÓN PARA REEMPLAZAR LAS TILDES EN CASO DE NO ENCONTRAR RESULTADOS **/
/****************************************************************************************************************/
    public function remplaceWordsAcents($cadena) {
        // Dividir la cadena en palabras
        $palabras = preg_split('/\s+/', $cadena);
        $resultado = [];
        $nameBus = strtoupper($cadena);
        // Definir caracteres con tildes, diéresis, y especiales
        $conTilde = ['Á', 'É', 'Í', 'Ó', 'Ú', 'á', 'é', 'í', 'ó', 'ú', 'Ñ', 'ñ', 'Ü', 'ü'];
        $sinTilde = ['A', 'E', 'I', 'O', 'U', 'a', 'e', 'i', 'o', 'u', 'N', 'n', 'U', 'u'];

        $sqlDest = "SELECT DISTINCT ON (s.SGD_CIU_CODIGO) 
                        s.SGD_CIU_CODIGO, 
                        s.SGD_CIU_NOMBRE AS nombre
                    FROM SGD_CIU_CIUDADANO s
                    WHERE UPPER(CONCAT(SGD_CIU_NOMBRE, ' ', SGD_CIU_APELL1, ' ', SGD_CIU_APELL2)) 
                        --ILIKE '%$cadena%'
                        ILIKE '$nameBus'
                    LIMIT 1; ";
        
        $rsDestin = $this->db->conn->query($sqlDest);
        //var_dump($sqlDest);
        if($rsDestin->EOF == false){
            return false;
        }else{
            // Verificar y procesar cada palabra
            foreach ($palabras as $palabra) {
                if (preg_match('/[ÁÉÍÓÚáéíóúÑñÜü]/u', $palabra)) {
                    // Reemplazar las tildes y caracteres especiales en la palabra
                    $resultado[] = str_replace($conTilde, $sinTilde, $palabra);
                }
            }
            //var_dump($resultado);
    ...
    public function buscarPorParametros($search) {
        ...
        case 0:
            if(!empty($name)){
                ...
                /********************************************************************************************************************************* */
                /* LLAMA LA FUNCIÓN Y CONDICIONA EL NÓMBRE A BUSCAR */
                /********************************************************************************************************************************* */
                $palabrasSinTilde = $this->remplaceWordsAcents($name);
                $name = ($palabrasSinTilde == false) ? str_replace(' ', '%', trim($name)) : str_replace(' ', '%', trim($palabrasSinTilde));
                /********************************************************************************************************************************* */
            ...
            SELECT 
                distinct on (s.SGD_CIU_CODIGO) s.SGD_CIU_CODIGO
                ,s.SGD_CIU_NOMBRE    as nombre
            ...
            FROM
                ...
                left join municipio m on (m.muni_codi = s.muni_codi AND m.dpto_codi = s.dpto_codi)
                left join sgd_def_paises p on  p.id_pais = s.id_pais
            WHERE
               
                s.SGD_CIU_CODIGO in(select max(SGD_CIU_CODIGO) from SGD_CIU_CIUDADANO where  $where group by SGD_CIU_NOMBRE limit 15)
                limit 15 ";
        ...
        case 1:
            ...
            if(!empty($name)){
                // Implementar búsqueda insensible a tildes usando unaccent
                $where = "unaccent(UPPER(NOMBRE_DE_LA_EMPRESA)) LIKE unaccent(UPPER('%" . strtoupper($name) . "%')) 
                          OR unaccent(UPPER(SIGLA_DE_LA_EMPRESA)) LIKE unaccent(UPPER('%" . strtoupper($name) . "%'))";
            }
...
/////////////////////////-----------------------------
            ...
            case 2:
                ...
                if(!empty($name)){
                    // Implementar búsqueda insensible a tildes usando unaccent
                    $where = "unaccent(UPPER(SGD_OEM_OEMPRESA)) LIKE unaccent(UPPER('%" . strtoupper($name) . "%')) 
                              OR unaccent(UPPER(SGD_OEM_SIGLA)) LIKE unaccent(UPPER('%" . strtoupper($name) . "%'))";
                }
     ...
     $concatSigla = $this->db->conn->Concat('s.sgd_oem_sigla',"' '",'s.SGD_OEM_REP_LEGAL');
      ...
      FROM
        ...
        ,SGD_DEF_PAISES p
      WHERE
        s.sgd_oem_codigo in(select max(sgd_oem_codigo) from SGD_OEM_OEMPRESAS where $where group by sgd_oem_oempresa  limit 15)
        and m.muni_codi = s.muni_codi
        ...
        break;
            ...
            case 6:
                ...
                if(!empty($name)){
                    ...
                    $where = "unaccent(UPPER(s.USUA_NOMB)) LIKE unaccent(UPPER('%" . $name . "%'))";
                }
