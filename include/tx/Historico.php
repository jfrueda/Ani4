...
class Historico
{
  ...
  * Constructor de la clase Historico
  ...
  /**
	...
	*
	*/
  function insertarHistorico($radicados,  $depeOrigen , $usCodOrigen, $depeDestino,$usCodDestino, $observacion, $tipoTx, $bndBoss = null)
  {
  	$fechSys = date('Y-m-d H:i:s');
		$fechBoss = $fechSys.'.'.'9999';
		...
		{	# Asignar el valor de los campos en el registro
			...
			$record["HIST_FECH"] = (isset($bndBoss)) ?  "'$fechBoss'" : $this->db->sysdate(); //$this->db->conn->OffsetDate(0,$this->db->conn->sysTimeStamp);
