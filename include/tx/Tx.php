...
// JUNIO  DE 2009
	...
	function informar($radicados, $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $observa, $idenviador = null, $ruta_raiz = "", $infConjunto = 0, $SendMail = false) {
		...
		while ((list(, $noRadicado) = each($radicados)) and $informaSql) {
			...
			// Validar si ya fue informado
			$radNum = $record["RADI_NUME_RADI"];
			$sqlCheck = "SELECT 1 FROM INFORMADOS WHERE RADI_NUME_RADI = $radNum AND USUA_DOC = '$docUsuarioDest'";
			$rsCheck = $this->db->conn->Execute($sqlCheck);
	...
	function tramiteConjunto($radicados, $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $observa, $idenviador = null, $ruta_raiz = "", $infConjunto = 0, $SendMail = false) {
		...
		while ((list(, $noRadicado) = each($radicados)) and $informaSql) {
			...
			// Validar si ya fue informado
			$radNum = $record["RADI_NUME_RADI"];
			$sqlCheck = "SELECT 1 FROM TRAMITECONJUNTO WHERE RADI_NUME_RADI = $radNum  AND USUA_DOC = '$docUsuarioDest' ";
			$rsCheck = $this->db->conn->Execute($sqlCheck);
			if ($rsCheck && !$rsCheck->EOF) {
				// Ya fue informado, retornar valor especial
				return "YA_INFORMADO";
	...
	function borrarInformado($radicados, $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $observa, $infConjunto = 0) {
		...
		if ($infConjunto == 1)
			...
			$codTxNew = 7;
		if ($deleteSQL) {
			$this->insertarHistorico($tmp_rad, $depOrigen, $codUsOrigen, $depDestino, $codUsDestino, $observa, 7, $observa);
			return $tmp_rad;
		} else {
			return $deleteSQL;
		}
	...
	function validarBorrador($radicados) {
		...
		$rs = $this->db->conn->Execute($depeActual);

		$flag = true;
		while ($rs && !$rs->EOF) {
			
			if($rs->fields['IS_BORRADOR'] == 't') {
				$flag = false;
				break;
			}				
			$rs->MoveNext();
	...
	function validarReasignar($radicados, $operacion) {
		...
		$flag = true;
		while ($rs && !$rs->EOF) {
			if($operacion == 1 && $rs->fields['RADI_DEPE_ACTU'] != 999) {
				$flag = false;
				break;
			}	
			if($operacion == 2 && $rs->fields['RADI_DEPE_ACTU'] == 999) {
				$flag = false;
				break;
			}				
			$rs->MoveNext();
	...
	function desarchivar($radicados, $loginOrigen,  $depOrigen,  $codUsOrigen, $tomarNivel, $codTx, $carp_codi, $valUsuarioActual = false) {
		foreach($radicados as $rad) {
			...
			$rs = $this->db->conn->Execute($sqlUsuAnte);
			if ($rs->fields['USUA_ESTA'] != NULL && $rs->fields['USUA_ESTA'] == 1) {	
					$depDestino = $rs->fields['DEPE_CODI'];
					$codUsDestino = $rs->fields['USUA_CODI'];
					$observa = 'Se desarchiva automaticamente el radicado debido a la devolución';
					$this->reasignar(array($rad), $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $tomarNivel, $observa,  $codTx, $carp_codi, $valUsuarioActual);
			} else {
				$sqlDepeRadi = "select radi_depe_radi from radicado  where radi_nume_radi = $rad";	
				$rsDepeRadi = $this->db->conn->Execute($sqlDepeRadi);
				$depDestino = $rsDepeRadi->fields['RADI_DEPE_RADI'];
					$jefeUsuario = "SELECT u.usua_codi, u.depe_codi FROM usuario u
								JOIN autm_membresias me on me.autu_id = u.id
								JOIN autg_grupos gr on gr.id = me.autg_id
								WHERE u.depe_codi = $depDestino AND
									gr.id = 2 limit 1";

					$rs = $this->db->conn->Execute($jefeUsuario);		
					$depDestino = $rs->fields['DEPE_CODI'];		
					$codUsDestino = $rs->fields['USUA_CODI'];
					$observa = 'Se desarchiva automáticamente el radicado debido a la devolución y se asigna al jefe del área proyectora por inactividad del último usuario responsable del radicado';
					$this->reasignar(array($rad), $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $tomarNivel, $observa,  $codTx, $carp_codi, $valUsuarioActual);
			}
		}
	}
	...
	function reasignarNoArchivado($radicados, $loginOrigen, $depDestino, $depOrigen, $codUsDestino, $codUsOrigen, $tomarNivel, $observa, $codTx, $carp_codi, $valUsuarioActual = false) {
		...
		$isql = "update radicado
				set
				  ...
				  ,radi_agend=null
				  $whereNivel
			 where RADI_DEPE_ACTU != 999 and  RADI_NUME_RADI in($radicadosIn)
           $whereValUsuarioActual
				    ";
...
/***************************************************************************************************/
// Se realiza consulta para traer el nivel de seguridad del radicado
/***************************************************************************************************/
$sqlNivelSeg = "SELECT SGD_SPUB_CODIGO FROM radicado WHERE radi_nume_radi  in ($radicadosIn)";
$res = $this->db->conn->Execute($sqlNivelSeg);
$nivelSeguridad=$res->fields['SGD_SPUB_CODIGO'];
//var_dump($nivelSeguridad);die();
/***************************************************************************************************/
...
if($rs_rol->fields['AUTG_ID']==37)
	{
      ...
      set
        ...
        ,radi_agend=null
        ,CODI_NIVEL=0
        ,SGD_SPUB_CODIGO={$nivelSeguridad}
      where RADI_NUME_RADI in($radicadosIn)";
	}
else
	{
        ...
        AND h.RADI_NUME_RADI = $noRadicado";
			...
			$depDestino = $rs->fields['DEPE_CODI'];
				...
				$sqlJefe="SELECT  
					...
					,u.USUA_DOC
					FROM autg_grupos b
				    left join autm_membresias a on b.id = a.autg_id
				    left join USUARIO u on  a.autu_id=u.id
				    where 
					u.depe_codi=$depDestino
					AND u.usua_esta='1'
					AND b.id=2";

				$rs = $this->db->conn->Execute($sqlJefe); # Ejecuta la busqueda
			...
			if ($codUsDestino) {
				...
				/*permite devolver si es un tramitador */
