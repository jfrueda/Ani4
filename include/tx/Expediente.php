 ...
 * @copyleft
 *
 * Models are the data definition of OrfeoGpl Information System
...
class Expediente
{
    ...
    var $codiSRD;
    var $codiSBRD;
    var $idSerie = 0;   // Identificador Unico de Serie
    var $idSubSerie = 0; // Identificador Unico de Subserie
    var $db;
    var $codigoTipoDoc;
    var $err = array();
    ...
    function consulta_exp($radicado)
    {
        ...
        if (trim($radicado) >= "20") {
            $query = "select e.SGD_EXP_NUMERO,e.SGD_EXP_ESTADO,$radi_nume_radi AS RADI_NUME_RADI
				...
				where e.RADI_NUME_RADI =" . trim($radicado) . " AND SGD_EXP_ESTADO <> 2";
    ...
    function consultaExp($radicado, $numExpediente = null, $traerRadicadosYAnexos = "Si")
    {
        //$this->db->conn->debug = true;
        switch ($this->db->driver) {
            case 'mssql':
        ...
        $query = "SELECT
                  ...
                  WHERE
                    ...
                    SGD_EXP_ESTADO <> 2";
        if ($numExpediente) $query .= " AND e.sgd_exp_numero = '$numExpediente'";
            ...
            while (!$rs->EOF) {
                ...
                if ($traerRadicadosYAnexos == "Si") {
                    ...
                    if (!$rsRas->EOF) {
					 ...
					 ,a.ANEX_NOMB_ARCHIVO, a.ANEX_BORRADO, a.ANEX_NUMERO, a.ANEX_CODIGO
					 ,(select r.RADI_PATH FROM radicado r WHERE r.radi_nume_radi=a.radi_nume_salida limit 1) RADI_PATH_SALIDA
           ,(select r.RADI_PATH FROM radicado r WHERE r.radi_nume_radi='" . $numRadicado . "' limit 1) RADI_PATH_PRINCIPAL
				    from anexos a LEFT JOIN  SGD_TPR_TPDCUMENTO t
                                       on ( a.sgd_tpr_codigo=t.sgd_tpr_codigo   )
				where a.anex_radi_nume='" . $numRadicado . "' and a.anex_codigo ilike '$numRadicado%'
				ORDER BY a.anex_fech_anex";
                            ...
                            while (!$rsAnex->EOF) {
                                ...
                                $arrAnexos[$iA]["RADI_PATH_SALIDA2"] = $rsAnex->fields['RADI_PATH_SALIDA'];
                                $arrAnexos[$iA]["ANEX_NUMERO"] = $rsAnex->fields['ANEX_NUMERO'];
                                $arrAnexos[$iA]["ANEX_CODIGO"] = $rsAnex->fields['ANEX_CODIGO'];
                                $arrAnexos[$iA]["ANEX_PATH"] = $rsAnex->fields['ANEX_NOMB_ARCHIVO'];
                                $arrAnexos[$iA]["ANEX_BORRADO"] = $rsAnex->fields['ANEX_BORRADO'];
                                $arrAnexos[$iA]["RADI_PATH_PRINCIPAL"] = $rsAnex->fields['RADI_PATH_PRINCIPAL'];
					...
					where
								exp.radi_nUme_radi='$numRadicado' -- and exp.sgd_exp_numero = '$numExpediente'
											and exp.sgd_exp_numero=sexp.sgd_exp_numero AND exp.sgd_exp_estado <>2 limit 20";
                            //$this->db->conn->debug = true;
                ...
                } // if de la variable $traerRadicadosYAnexos en caso true
    ...
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc)
    {
        ...
        if (!$rs->fields["SGD_EXP_NUMERO"]) {
            ...
            if (!$rs2->fields["SGD_EXP_NUMERO"]) {
                $this->err[] = "<hr>No hay Documentos en el expediente $num_expediente<hr>";
                return 0;
            } else {
                $codiSRD = 0;
                $codiSBRD = 0;
        ...
        if (!$rs->EOF or $creadoOld == 1 or $rs->fields['SGD_CERRADO'] == 0 or $rs->fields['SGD_CERRADO'] == '') {
            ...
            if (!$this->db->conn->query($query)) {
                ...
                /**
                 * Logica de herencia de radicado al ingresar al expediente
                 */
                $sqlSeguridadRadicado = 'select sgd_spub_codigo from radicado where radi_nume_radi = ' . $radicado;
                $rsSeguridadRadicado = $this->db->conn->Execute($sqlSeguridadRadicado);
                $seguRadicado = $rsSeguridadRadicado->fields["SGD_SPUB_CODIGO"];

                $sqlSeguridadExpediente = "select seguridad FROM sgd_sexp_secexpedientes where sgd_exp_numero = '$num_expediente'";
                $rsSeguridadExpediente = $this->db->conn->getOne($sqlSeguridadExpediente);
                $seguExpediente = 0;
                $observaTx = '';
                switch($rsSeguridadExpediente) {
                    case '0':
                        $seguExpediente = 0;
                        $observaTx = "Radicado Público";
                        break;
                    case '1':
                        $seguExpediente = 1;
                        $observaTx = "Reservado: Solo la dependencia.";
                        break;
                    case '2':
                        $seguExpediente = 2;
                        $observaTx = "Clasificado: Usuario que proyectó, Jéfe y usuario actual del radicado.";
                        break;
                    default:
                        $seguExpediente = 0;
                        $observaTx = "Radicado Público";
                ...
                if($seguExpediente > $seguRadicado) {
                    $queryCambioSeguridad = "UPDATE RADICADO SET SGD_SPUB_CODIGO=$seguExpediente where radi_nume_radi=$radicado";
                    $this->db->conn->Execute($queryCambioSeguridad);
                    
                    $comenEx= 'Cambio automático de nivel de seguridad ya que se anexó al expediente ' . $num_expediente . '. el cúal tiene nivel de seguridad -- ' . $observaTx;
                    $fechaActual = $this->db->sysdate();
                    $insertSql = "INSERT INTO public.hist_eventos(
                        depe_codi, hist_fech, usua_codi, radi_nume_radi, hist_obse, usua_codi_dest, usua_doc, sgd_ttr_codigo, depe_codi_dest,hist_doc_dest)
                        VALUES ($depe_codi, $fechaActual, $usua_codi, $radicado, '$comenEx', $usua_codi, $usua_doc, 54, $depe_codi, $usua_doc)";
                    $this->db->conn->Execute($insertSql);                    
                }
            ...
            if ($rs->fields['SGD_CERRADO'] == 1) return 2;
            else return 0;
        }
    }
     ...
     *
     */
    function crearExpediente($numExpediente, $radicado, $depe_codi, $usua_codi, $usua_doc, $usuaDocExp, $codiSRD, $codiSBRD, $expOld = null, $fechaExp = null, $codiPROC = null, $arrParametro = null, $nivelExp = 0)
    {
        $this->idSerie = $codiSRD;
        $this->idSubSerie = $codiSBRD;
        if (!$this->idSerie) {
            $this->idSerie = 0;
            $this->err[] = "No puede crear expediente, No viene el codigo serie";
            return 0;
        }
        if (!$this->idSubSerie) {
            $this->idSubSerie = 0;
            $this->err[] = "No puede crear expediente, No viene el codigo SubSerie";
            return 0;
        }
        $p = 1;
        ...
        if ($expOld == "false") {
            ...
            if ($expManual == 1) {
                ...
                //$secExp = substr("0000000" . $secExp, -7);
            }
        ...
        $rsNumExp = $rs->fields["SGD_EXP_NUMERO"];
        if ($rsNumExp == "") {
            $rsNumExp = $rs->fields["sgd_exp_numero"];
        }
        if ($rsNumExp == $numExpediente) {
            # echo "1"; exit;
            return 0;
        } else {
            # echo "2"; exit;
            ...
            $query = "insert into SGD_SEXP_SECEXPEDIENTES(SGD_EXP_NUMERO   ,SGD_SEXP_FECH      ,DEPE_CODI   ,USUA_DOC   ,SGD_FEXP_CODIGO,SGD_SRD_CODIGO,SGD_SBRD_CODIGO,SGD_SEXP_SECUENCIA, SGD_SEXP_ANO, USUA_DOC_RESPONSABLE, SGD_PEXP_CODIGO, SGD_SRD_ID, SGD_SBRD_ID, SGD_EXP_PRIVADO";
            if ($campoParametro != "") {
                $query .= ", $campoParametro";
            }
            $query .= " )";
            $query .= " VALUES ('$numExpediente'," . $sqlFechaHoy . " ,'$depe_codi','$usua_doc',0              ,$codiSRD     ,$codiSBRD        ,'$secExp' ,$anoExp, $usuaDocExp, $codiPROC, $this->idSerie, $this->idSubSerie, $nivelExp";
            if ($valorParametro != "") {
    ...
    function secExpediente($dependencia, $codiSRD, $codiSBRD, $anoExp)
    {
        $query = "select se.SGD_EXP_NUMERO
			...
			ORDER BY
                substr(sgd_exp_NUMERO, 14, LENGTH(sgd_exp_NUMERO) - 14) desc, sgd_sexp_secuencia DESC
			";
        ...
        $secExp = substr($numExpediente, 13, -1);
    ...
    function datos_expediente($radicado, $num_expediente)
    {
        ...
        $query = "select
				...
				";
        $rs = $this->db->conn->query($query);
        $sqlbe = "select sgd_cerrado from sgd_sexp_secexpedientes where  SGD_EXP_NUMERO = '$num_expediente'";
        $rsb = $this->db->conn->query($sqlbe);
        if ($rs) {
            ...
            $this->exp_cerrado = $rsb->fields['SGD_CERRADO'];
    ...
    function consultaTipoExpediente($numExpediente)
    {
        ...
        if ($numExpediente) {
            ...
            $this->cerrado = $rs->fields["SGD_CERRADO"];
    ...
    function getDatosParamExp($numExp, $depeCodi)
    {
        ...
        $q_datosParametro = "SELECT  $limitMsql CASE WHEN PAREXP.SGD_PAREXP_ORDEN = 1 THEN SEXP.SGD_SEXP_PAREXP1
         ...
         AND parexp.DEPE_CODI = '$depeCodi' $limitOci8
         ORDER BY PAREXP.SGD_PAREXP_ORDEN asc $limitPsql";
    ...
    function expedienteValido($numExpediente)
    {
        $query = "SELECT sgd_exp_numero, sgd_sexp_estado FROM SGD_SEXP_SECEXPEDIENTES WHERE sgd_exp_numero = ?";
        $result = $this->db->conn->getRow($query, [$numExpediente]);
        
        if(count($query) == 0)
            return false;

        return !in_array($result['SGD_SEXP_ESTADO'], ['1', '2']);
    }
    ...
    function expedienteAnexoAsociado($radicado)
    {
        ...
        $query .= ' AND EXP.RADI_NUME_RADI=RAD.RADI_NUME_RADI)';
        $query .= ' AND RAD.RADI_NUME_DERI NOT IN (';
        $query .= '   SELECT EXP.RADI_NUME_RADI FROM SGD_EXP_EXPEDIENTE EXP WHERE EXP.RADI_NUME_RADI <> ' . $radicado;
        $query .= ' AND EXP.RADI_NUME_RADI=RAD.RADI_NUME_DERI)';
        $query .= ' AND RAD.RADI_NUME_RADI NOT IN (';
    ...
    function crearExpedienteExterno($numExpediente, $radicado, $depe_codi, $usua_codi, $usua_doc, $usuaDocExp, $codiSRD, $codiSBRD, $expOld = null, $fechaExp = null, $codiPROC = null, $arrParametro = null)
    {
      ...
      ";
        ...
        if ($rsNumExp == $numExpediente) {
            ...
            if (!$this->idSerie) $this->idSerie = 0;
            if (!$this->idSubSerie) $this->idSubSerie = 0;
    ...
    function getResponsable($numExpediente)
    {
        $query = "select USUA_DOC_RESPONSABLE from SGD_SEXP_SECEXPEDIENTES WHERE SGD_EXP_NUMERO='";
        $query .= $numExpediente . "';";
        //$this->db->conn->debug = true;
        $rs = $this->db->conn->Execute($query);
        if (!$rs->EOF) {
            return $rs->fields['USUA_DOC_RESPONSABLE'];
        } else {
            return "ERROR";
        }
    }

    function getSeguridad($numExpediente)
    {
        $query = "select SGD_EXP_PRIVADO from SGD_SEXP_SECEXPEDIENTES WHERE SGD_EXP_NUMERO='" . $numExpediente . "';";
        $rs = $this->db->conn->Execute($query);
        if (!$rs->EOF) {
            $nivel = $rs->fields['SGD_EXP_PRIVADO'];
            if ($nivel == 0) $datoss = "Público";
            else if ($nivel == 1) $datoss = "Privado dependencia (Solo usuarios área y usuarios jefes )";
            else if ($nivel == 2) $datoss = "Privado (Solo Usuario Responsable Y Jefe)";
            else $datoss = "Valor no indeterminado";
            return array('value' => $nivel, 'label' => $datoss);
        } else {
            return array('value' => -1, 'label' => "Indefinido");
        }

    }

    function getDependencia($numExpediente)
    {
        $query = "select DEPE_CODI from SGD_SEXP_SECEXPEDIENTES WHERE SGD_EXP_NUMERO='";
        $query .= $numExpediente . "';";
        //$this->db->conn->debug = true;
        $rs = $this->db->conn->Execute($query);
        if (!$rs->EOF) {
