...
class Radicacion
{
    ...
    var $codiNivel=1;
    var $noDigitosRad;
    var $digitosSecuenciaPorDefecto = 7;
    var $noDigitosDep=3;
    var $usuaFirma;
    var $depeFirma;
    var $radUsuaAnte = '';
    ...
    function Radicacion($db){
        ...
        $this->noDigitosRad = isset($_SESSION['digitosSecRad']) ? $_SESSION['digitosSecRad'] : $this->digitosSecuenciaPorDefecto;
        if(isset($_SESSION)){
    ...
    function newRadicado($tpRad, $tpDepeRad=null, $privRad=null){

         $this->sgdSpubCodigo = empty($privRad) ? $this->sgdSpubCodigo : $privRad;
        ...
        if(!empty($this->radUsuaAnte)) $recordR["RADI_USU_ANTE"]  = "'{$this->radUsuaAnte}'";
    ...
    function borradorArradicado($numRadicadoPadreAnt, $numRadicadoPadre, $rutaRaiz){
        ...
        $radTemporal =  rand(-999999999, -10000);
        error_log(date(DATE_ATOM)." i,{$_SESSION['dependencia']},{$_SESSION['codusuario']},$radTemporal,$numRadicadoPadreAnt,$numRadicadoPadre\n", 3, "{$_SESSION['ABSOL_PATH']}/bodega/borrador.log");
    ...
    where radi_nume_radi =  $numRadicadoPadreAnt";    
        ...
        error_log(date(DATE_ATOM)." f,{$_SESSION['dependencia']},{$_SESSION['codusuario']},$radTemporal,$numRadicadoPadreAnt,$numRadicadoPadre\n", 3, "{$_SESSION['ABSOL_PATH']}/bodega/borrador.log");
    }
    ...
    function updateRadicado($radicado, $radgplthUpdate = null){
        ...
        //EESP_CODI,RADI_PAIS,RADI_DATO_001,RADI_DATO_002
        $sqlCon="select RADI_CUENTAI,  MREC_CODI,to_char(RADI_FECH_OFIC,'YYYY-MM-DD') AS RADI_FECH_OFIC,RA_ASUN,RADI_NUME_FOLIO,RADI_DESC_ANEX,RADI_NUME_ANEXO,EMP_TRANSPORTADORA,RADI_NUME_GUIA,SGD_SPUB_CODIGO ";
            $sqlCon.=" FROM radicado WHERE radi_nume_radi=$radicado";
        $reMod=array();
    //$this->db->conn->debug=true;
        $rsc=$this->db->conn->query($sqlCon);
        if(!$rsc->fields)
            return false;

        if(trim($this->radiCuentai) != trim($rsc->fields["RADI_CUENTAI"])){
            $recordR["RADI_CUENTAI"]    = trim("'".$this->radiCuentai."'");
            $reMod["RADI_CUENTAI"]=trim($rsc->fields["RADI_CUENTAI"]);
        }
    //if(trim($this->radiCuentai) != trim($rsc->fields["EESP_CODI"]))
    //		$recordR["EESP_CODI"] = $this->eespCodi ;
        if(trim($this->mrecCodi)=='')
            $this->mrecCodi=0;
        if(trim($this->mrecCodi) != trim($rsc->fields["MREC_CODI"])){
            $recordR["MREC_CODI"]       = $this->mrecCodi;
            $reMod["MREC_CODI"]=trim($rsc->fields["MREC_CODI"]);
        }
    //var_dump($rsc->fields);
        if(trim($this->db->conn->DBDate($this->radiFechOfic)) !=trim($this->db->conn->DBDate($rsc->fields["RADI_FECH_OFIC"]))){
            $recordR["RADI_FECH_OFIC"]  = $this->db->conn->DBDate($this->radiFechOfic);
            $reMod["RADI_FECH_OFIC"]=trim($this->db->conn->DBDate($rsc->fields["RADI_FECH_OFIC"]));
        }
            //if(trim($this->radigplis)) $recordR["RADI_PAIS"]       = "'".$this->radigplis."'";
        if(trim($this->raAsun) != trim($rsc->fields["RA_ASUN"])){
            $recordR["RA_ASUN"]         = trim("'".$this->raAsun."'");
            $reMod["RA_ASUN"]=trim($rsc->fields["RA_ASUN"]);
        }
        if(trim($this->descAnex) != trim($rsc->fields["RADI_DESC_ANEX"])){
            $recordR["RADI_DESC_ANEX"]  = trim("'".$this->descAnex."'");
            $reMod["RADI_DESC_ANEX"]=trim($rsc->fields["RADI_DESC_ANEX"]);
        }
            // $recordR["SGD_APLI_CODI"]   = $this->sgd_apli_codi;

            if(!empty($this->nofolios) && trim($this->nofolios) != trim($rsc->fields["RADI_NUME_FOLIO"])){
                $recordR["RADI_NUME_FOLIO"] = trim($this->nofolios);
            $reMod["RADI_NUME_FOLIO"]=trim($rsc->fields["RADI_NUME_FOLIO"]);
        }
    /*
            if($this->tdocCodi=="null"){
                $this->tdocCodi = 0;
    ...
    */
            if(!empty($this->noanexos) && trim($this->noanexos) != trim($rsc->fields["RADI_NUME_ANEXO"])){
                $recordR["RADI_NUME_ANEXO"] = trim($this->noanexos);
            $reMod["RADI_NUME_ANEXO"]=trim($rsc->fields["RADI_NUME_ANEXO"]);
        }
            

            if(!empty($this->empTrans) && trim($this->empTrans) != trim($rsc->fields["EMP_TRANSPORTADORA"])){
                $recordR["EMP_TRANSPORTADORA"]  = trim($this->empTrans);
            $reMod["EMP_TRANSPORTADORA"]=trim($rsc->fields["EMP_TRANSPORTADORA"]);
        }

            if(!empty($this->guia) && trim($this->guia) != trim($rsc->fields["RADI_NUME_GUIA"])){
            $recordR["RADI_NUME_GUIA"]  = "'$this->guia'";
            $reMod["RADI_NUME_GUIA"]=trim($rsc->fields["RADI_NUME_GUIA"]);
        }

        $nuevoNivelSeguridad = trim((string) $this->sgdSpubCodigo);
        $nivelSeguridadActual = trim((string) $rsc->fields["SGD_SPUB_CODIGO"]);
        $cambioNivelSeguridad = false;
        $observaNivelSeguridad = '';
        $nivelSeguridadInt = null;

        if ($nuevoNivelSeguridad !== '' && $nuevoNivelSeguridad !== $nivelSeguridadActual) {
            $nivelSeguridadInt = (int) $nuevoNivelSeguridad;
            $recordR["SGD_SPUB_CODIGO"] = $nivelSeguridadInt;
            $reMod["SGD_SPUB_CODIGO"] = $nivelSeguridadActual;
            $cambioNivelSeguridad = true;

            switch ($nivelSeguridadInt) {
                case 0:
                    $observaNivelSeguridad = "Nivel de seguridad actualizado: Radicado Publico.";
                    break;
                case 1:
                    $observaNivelSeguridad = "Nivel de seguridad actualizado: Reservado, solo la dependencia.";
                    break;
                case 2:
                    $observaNivelSeguridad = "Nivel de seguridad actualizado: Clasificado, usuario que proyecto, jefe y usuario actual del radicado.";
                    break;
                default:
                    $observaNivelSeguridad = "Nivel de seguridad actualizado: Codigo {$nivelSeguridadInt}.";
                    break;
            }
        }
        //$recordR["RADI_DATO_001"]  = "'$this->radi_dato_001'";
	...
	if(count($recordR)>1)
        ...
        if ($cambioNivelSeguridad && $insertSQL) {
            $dependenciaActual = $this->dependencia;
            $usuarioActual = $this->usuaCodi;

            if ($dependenciaActual === null && isset($_SESSION['dependencia'])) {
                $dependenciaActual = $_SESSION['dependencia'];
            }

            if ($usuarioActual === null && isset($_SESSION['codusuario'])) {
                $usuarioActual = $_SESSION['codusuario'];
            }

            if ($dependenciaActual !== null && $usuarioActual !== null) {
                include_once "{$this->rutaRaiz}/include/tx/Historico.php";
                $Historico = new Historico($this->db);
                $radicadosHistorico = array($radicado);
                // Keep historico in sync with security changes.
                $Historico->insertarHistorico($radicadosHistorico, $dependenciaActual, $usuarioActual, $dependenciaActual, $usuarioActual, $observaNivelSeguridad, 54);
            }
        }

        return array($insertSQL,$reMod);
