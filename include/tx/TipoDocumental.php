...
class TipoDocumental
{
    ...
    /**  Funcion consultaTRDradicado que trae la clasificacion documental de un radicado.
     *
    ...
    function consultaTRDradicado( $noRadicado )
    {
        ...
        $sql = "SELECT rdf.radi_nume_radi, 
								...
								FROM 
									...
									sgd_mrd_matrird mrd
								WHERE 
									mrd.sgd_mrd_codigo = rdf.sgd_mrd_codigo
									and rdf.RADI_NUME_RADI = '$noRadicado'
								ORDER BY
        ...
        if(!$rs->EOF){
            ...
            $this->subSerieCodigo = $rs->fields["SGD_SBRD_CODIGO"];
					  $this->idSerie = $rs->fields["SGD_SRD_ID"];
            $this->idSubSerie = $rs->fields["SGD_SBRD_ID"];                        
    ...
    /**
     *
     * @var $db Cursor de la base de datos que estamos trabajando.
     * @varno $Radicado numero al cual se le cambiara la fecha
     * @var $tRad tipo documental que se asignara al radicado
     * @var $update Si esta esta variable no busca la fecha de radicado ni actulaliza, realiza el calculo con la fecha de hoy y devuelve la fecha de vencimiento.
     * 
     */
    function setFechAlerta($noRadicado,$tRad, $update=true){
        //Agregamos la fecha de vencimiento
        //Sacando los dias de fiesta y los sabados
        $isqla = 'SELECT NOH_FECHA FROM SGD_NOH_NOHABILES';       
        $resa  = $this->db->conn->Execute($isqla);
        $resul = date("Y-m-d", strtotime("+2 week"));
        ...
        //tiempo del tipo de Radicado
        //numero
        $genAlerta="";
        if(!empty($tRad)){
            $sql3 = "SELECT sgd_trad_codigo     , sgd_trad_descr , sgd_trad_icono         , sgd_trad_genradsal, 
                sgd_trad_diasbloqueo, sgd_trad_alerta, sgd_trad_tiempo_alerta
                FROM sgd_trad_tiporad
                WHERE sgd_trad_alerta>=1 and sgd_trad_codigo=$tRad ";

            $rs2 = $this->db->conn->Execute($sql3); 
            if($rs2 && !$rs2->EOF){
                $genAlerta =
                    $tiemTdoc = $rs2->fields["SGD_TRAD_TIEMPO_ALERTA"];
            }else{
                $genAlerta="";
            }
        }
        ...
        return $resul;
     /**echo "  fecha radicado $frad
         dias por tipo documental $sal
         fecha procesando $resul"; **/
    ...
    function insertarTRD($codiTRDS,$codiTRD,$noRadicado, $coddepe , $codusuario, $tdoc=null){
        ...
        if(!$rs->EOF){
            ...
            if($tdoc){
              $uSql = "UPDATE RADICADO SET tdoc_codi=$tdoc
                      WHERE RADI_NUME_RADI = '$noRadicado'";
              $rs = $this->db->conn->Execute($uSql);                
            }
