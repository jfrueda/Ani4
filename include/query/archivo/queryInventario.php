<?
	...
	case 'oracle':
	case 'oci8':
		$quer="update SGD_EXP_EXPEDIENTE set SGD_EXP_ISLA='$exp_piso',SGD_EXP_UFISICA = '$exp_item1'
	,SGD_EXP_ESTANTE ='$estante',SGD_EXP_ENTREPA='$entrepano',SGD_EXP_EDIFICIO='$exp_edificio',SGD_EXP_CARRO ='$exp_carro' where SGD_EXP_NUMERO LIKE '$expedientes[$t]' and SGD_EXP_ESTADO like '1'";
		$query="update SGD_EXP_EXPEDIENTE set SGD_EXP_ISLA='$exp_piso',SGD_EXP_UFISICA = '$exp_item1'
	,SGD_EXP_ESTANTE ='$estante',SGD_EXP_ENTREPA='$entrepano',SGD_EXP_EDIFICIO='$exp_edificio',SGD_EXP_CARRO ='$exp_carro' where SGD_EXP_NUMERO
	...
	'$sbrd','$sbrd_desc','$rete','$disfinal','$ubicacion','$observa')";
		...
		$sql="select SGD_EIT_PISO from SGD_EIT_ITEMS order by SGD_EIT_PISO";
		$sql7="select SGD_EIT_SIGLA,SGD_EIT_CODIGO from SGD_EIT_ITEMS where CODI_MUNI like '$codMuni'
				and CODI_DPTO like '$codDpto' order by SGD_EIT_SIGLA";
		...
		$queryDpto = "select d.DPTO_NOMB,d.DPTO_CODI FROM DEPARTAMENTO d, SGD_EIT_ITEMS i WHERE d.DPTO_CODI=i.CODI_DPTO ORDER BY DPTO_NOMB";
		$queryMuni = "select m.MUNI_NOMB,m.MUNI_CODI FROM MUNICIPIO m , SGD_EIT_ITEMS i WHERE m.MUNI_CODI=i.CODI_MUNI and DPTO_CODI='$codDpto' ORDER BY MUNI_NOMB";
		$queryMuni2 = "select MUNI_NOMB,MUNI_CODI FROM MUNICIPIO WHERE DPTO_CODI='$codDpto2' ORDER BY MUNI_NOMB";
		$sqli="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$exp_edificio' order by sgd_eit_codigo";
		$sqlie="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$pisocod'";
		$sqliD="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$item1_cod' order by sgd_eit_codigo";
		$sqliR="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$item2_cod' order by sgd_eit_codigo";
		$sqliQ="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$item3_cod' order by sgd_eit_codigo";
		$sqliT="select sgd_eit_nombre,sgd_eit_codigo from sgd_eit_items where sgd_eit_cod_padre like '$item4_cod' order by sgd_eit_codigo";
		$sqliw="select count(sgd_eit_nombre) CO from sgd_eit_items where sgd_eit_cod_padre like '$item1_cod'";
		$sqli1="select count(sgd_eit_nombre) CO from sgd_eit_items where sgd_eit_cod_padre like '$item2_cod'";
		$sqli2="select count(sgd_eit_nombre) CO from sgd_eit_items where sgd_eit_cod_padre like '$item3_cod'";
		$sqli3="select count(sgd_eit_nombre) CO from sgd_eit_items where sgd_eit_cod_padre like '$item4_cod'";
		$sqli4="select count(sgd_eit_nombre) CO from sgd_eit_items where sgd_eit_cod_padre like '$pisocod'";
 	...
 	and s.sgd_srd_fechfin order by detalle ";
		$sqle="select sgd_exp_numero from sgd_exp_expediente where sgd_exp_numero like '$expe%'
		 and sgd_exp_caja>='$cajas' and sgd_exp_caja<='$cajas2'";
		$sqlc="select max(cast(sgd_exp_caja as int)) CAJA from sgd_exp_expediente where sgd_exp_numero like '$expe%'";
		$bexp="select max(sgd_exp_numero) EXPE from sgd_exp_expediente where sgd_exp_numero like '$expe%' and sgd_exp_rete=1 and SGD_EXP_FECHFIN>='$fechaInii' and SGD_EXP_FECHFIN<='$fechaInif'";
		$sqlex="select max(cast(sgd_exp_carpeta as int)) CARP from sgd_exp_expediente where sgd_exp_numero like '$expde'";
		$sqlex2="select sgd_exp_entrepa,sgd_exp_estante,sgd_exp_caja,sgd_exp_ufisica,sgd_exp_edificio,
		sgd_exp_isla from sgd_exp_expediente where sgd_exp_numero like '$expde'";
