...
class radicacionAjax extends Radicacion{
  ...
  var $errorNewRadicado;
     ...
     * @param var $ruta_raiz Esta variable indica en que 
     * sitio se encuenta la Raiz de Orfeo, es Una 
     * ruta relativa ... que muestra cuantos directorios 
     * debe devolverse para encontrar la raiz
	 * @access public
	...
	function highlight($id) { $response = new HTML_AJAX_Action();
  ...
  /**
	...
	function newRadicadoAjax($idObjetoHtml
		,$asunto
		,$tipoRadicado
		,$radiDepeRadi
		,$radiDepeActu
		,$dependenciaSecuencia
		,$radiUsuaRadi
		,$radiUsuaActu
		,$usuaDoc
		,$cuentai
		...
		$this->radiguia     = "".trim($guia)."";
		$this->radianexo    = $noanexos;
		$this->radifolio    = $nofolios;
		...
		if(!$radiUsuaActu) $radiUsuaActu    = 1;
	...
	$cadena1 = " seleccion = document.getElementById('$idObjetoHtml'); ";
	//if($noRad>=1)
	//{

	 //$cadena5 .= "document.getElementById('numeroRadicado').value=".$noRad."; grabarDirecciones(document.getElementById('numeroRadicado').value);";
	 $cadena5 = "";
   $cadena1 .= "document.getElementById('numeroRadicado').value=".$noRad.";";
	 $cadena5 .= "document.getElementById('Submit33').style.visibility='hidden'; ";
	 $cadena5 .= "document.getElementById('grabarDir').style.visibility='visible'; ";
	 //$cadena1 .= "verDatosRad(".$noRad."); ";
	 $cadena = 'valor="<table wdith=50%><tr class=titulos1><td><img src=../imagenes/gnu.png width=50 alt=GNU/GPL title=GNU/GPL></td><td><center><font size=4>Radicado No '.$noRad.' - - '.htmlspecialchars($errorNewRadicado).'</font></center></td></tr></table>";';
