...
function listaInformados($radicado,$idObjetoHtml,$usuaDocOrigen)
{
	...
	$iSql = "select 
		...
		and dep.depe_codi=us.depe_codi
		and inf.usua_codi=us.usua_codi
  ...
  //$this->db->conn->debug = true;		
	...
	$cadena .= "<small>SE HA INFORMADO A:</small><TABLE class='table table-striped table-bordered' width='100%' >  ";
	while (!$rsUs->EOF)
	{
	 ...
	 $cadena .= "<td ><small> $usuaNomb (<a onClick=\"borrarInformado('$radicado',$depeCodiBorrar,$usuaCodiBorrar,'Borrado de Informado')\"><span style='color: RED;'>Borrar</ span></a>)</small></td>";
	 $cadena .= "</tr>";
	 $rsUs->moveNext();
	}
	$cadena .= "</TABLE>  ";
...
/**
 * Metodo Devuelve Lista de Usuarios Tramite Conjunto+
 ...
 *
 **/
function listaTramiteConjunto($radicado,$idObjetoHtml,$usuaDocOrigen)
{

	$cadena = "";
	$iSql = "select 
		inf.INFO_DESC, dep.DEPE_NOMB, us.USUA_NOMB, inf.DEPE_CODI, inf.USUA_CODI,inf.INFO_FECH
		from tramiteconjunto inf, dependencia dep, usuario us
		where
		dep.depe_codi=inf.DEPE_CODI
		and dep.depe_codi=us.depe_codi
		and inf.usua_codi=us.usua_codi
  ...
  //$this->db->conn->debug = true;		
	...
	//$cadena .= "seleccion = document.getElementById('$idObjetoHtml'); ";
	$cadena .= "<small>Se ha Enviado a:</small><TABLE class='table table-striped table-bordered' width='100%' >  ";
	while (!$rsUs->EOF)
	{
	 $infoDesc = htmlentities(strtoupper(trim($rsUs->fields["INFO_DESC"])));
	 $depeCodiBorrar = trim($rsUs->fields["DEPE_CODI"]);
	 ...
	 $cadena .= "<tr><td ><small>$infoDesc</small></td><td ><small>$fechaInf</small></td>";
	 $cadena .= "<td ><small> ($depeCodiBorrar) $depeNomb </small></td>";
	 $cadena .= "<td ><small> $usuaNomb (<a onClick=\"borrarTramiteConjunto('$radicado',$depeCodiBorrar,$usuaCodiBorrar,'Borrado de Envio')\"><span style='color: RED;'>Borrar</ span></a>)</small></td>";
	 $cadena .= "</tr>";
	 $rsUs->moveNext();
	}
	$cadena .= "</TABLE>  ";
...
function borrarTramiteConjunto($idObjetoHtml,$radicado,$depeCodi,$usuaCodi,$depeCodiBorrar,$usuaCodiBorrar,$observacion) {
	$iSql = "select USUA_DOC from usuario where depe_codi=$depeCodi and usua_codi=$usuaCodi ";
	$rsUs = $this->db->conn->Execute($iSql);
	$usuaDoc = $rsUs->fields["USUA_DOC"];
	$radicados[]=$usuaDoc."-".$radicado;
	...
	$tx = new Tx($this->db);
	$loginOrigen = "";
	$rta = $tx->borrarTramiteConjunto($radicados,$loginOrigen,$depeCodi,$depeCodiBorrar,$usuaCodi,$usuaCodiBorrar,$observacion);
	$cadena = $this->listaTramiteConjunto($radicado,$idObjetoHtml,$usuaDoc );
	$response->insertScript($cadena);
	return $response;
}
/**
