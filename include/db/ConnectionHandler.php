...
class ConnectionHandler
{
    ...
    function __construct($ruta_raiz)
    {
        ...
        // tiempo sesión
        if (isset($tiempo_sesion) && isset($_SESSION['usua_doc'])) {
            $ex = ['cerrar_session.php', 'radicacion/ajax_buscarUsuario.php',
                'lista_anexos_consulta.php', 'login.php', 'formularioWeb/'];
            $r = false;
            foreach ($ex as $e) {
              if (strpos($_SERVER['SCRIPT_FILENAME'], $e) !== false) {
                $r = true;
              }
            }
            if (isset($_SESSION['timeout']) && (time() - $_SESSION['timeout'] > $tiempo_sesion)) {
                session_destroy();
