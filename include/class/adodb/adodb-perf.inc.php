...
"select count(1),tracer 
	...
	*/
	...
	function InvalidSQL($numsql = 10)
	{
		...
		$this->conn->fnExecute = false;
        $perf_table = adodb_perf::table();
		$rs =& $this->conn->SelectLimit("select distinct count(1),sql1,tracer as error_msg from $perf_table where tracer like 'ERROR:%' group by sql1,tracer order by 1 desc",$numsql);//,$numsql);
		$this->conn->fnExecute = $saveE;
		if ($rs) {
	...
	*/
	function _ExpensiveSQL($numsql = 10)
	{
		global $ADODB_FETCH_MODE;
			...
			$rs =& $this->conn->SelectLimit(
			"select sum(timer) as total,$sql1,count(1),max(timer) as max_timer,min(timer) as min_timer
				from $perf_table
				where {$this->conn->upperCase}({$this->conn->substr}(sql0,1,5))  not in ('DROP ','INSER','COMMI','CREAT')
				and (tracer is null or tracer not like 'ERROR:%')
				group by sql1
