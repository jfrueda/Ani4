...
if (!$archivo){$archivo = "plantillas/txt/$dependencia/$plantilla.txt";}
...
if($fmes_nombre=="Dec"){$fmes_nombre="Diciembre";}
if($fdia_nombre=="Mon"){$fdia_nombre="Lunes";}
...
if($fdia_nombre=="St"){$fdia_nombre="Sabado";}
...
// *******************  FIN GRABACION O CREACION DE NUEVO DOC 
 ...
 if (!$nombre_pl)  {$archivo = "pl_txt/plantilla_nueva.txt"; $nombre_pl = "Descripcion Plantilla"; 	}
if(!$enviar and !$enviar_new)
{
...
?>
<FORM  action='plantilla.php?<?=session_name()."=".session_id()."&fechah=$fechah"?>&verrad=$verrad&gen_rad=<?=$gen_rad?>' method="post">
...
<input type='hidden' name='<?=session_name()?>' value='<?=session_id()?>'> 
  ...
  <input type=hidden name='arpdf' value='<?php  echo "$archivopdf"; ?>' > 
  <input type=hidden name='artxt' value='<?php  echo "$archivotmp"; ?>' >
