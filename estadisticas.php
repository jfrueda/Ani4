...
<head>
	<title>ESTADISTICAS -- SAD -- SSPD</title>
...
<body>
	...
	if (!$dependencia || !$krd || strlen(trim($nombusuario)) < 1) {
	...
	?>
		...
		<form name="formulario" method=post action='estadisticas.php?<?= session_name() . "=" . trim(session_id()) . "&krd=$krd&fechah=$fechah" ?>'>
			<TABLE class="FormTABLE">
				...
				<TR>
					...
					<TD><select name=codus class="DataFONT">
				...
				<Tr>
					<TD bgcolor="#CCCCCC">
						<font class="DataFONT">Tarea a Realizar</FONT>
					</TD>
					<TD>
						<select name=tarea class="DataFONT">
							<? if ($tarea == 0) {
								$datoss = " selected ";
							} else {
								$datoss = " "; ?>
				...
				<TR>
					<TD bgcolor="#CCCCCC">
						...
						if ($agrupar_tipo == 1) {
							$isql_group = "  d.usua_nomb ,b.SGD_TPR_DESCRIP   ";
							$isql_sel_group = "  d.usua_nomb USUARIO,b.SGD_TPR_DESCRIP TIPO_DOCUMENTO  ";
						} else {
							$isql_group = " d.USUA_NOMB ";
							$isql_sel_group = " d.usua_nomb NOMBRE ";
						}
						...
						?>
						<font class="DataFONT">Agrupar por Tipo de Documento</FONT>
					</TD>
					<TD>
						<? if ($agrupar_tipo == 1) {
							$datoss = " checked ";
		...
		if (!$estadisticas) {
			die("<hr>Fecha " . date("y-m-d")) . " Hora  " . time("h:m:s") . "<hr>";
		}
		$nrows = 0;
		?>
