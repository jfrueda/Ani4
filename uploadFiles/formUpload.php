...
elseif ($_REQUEST['anexRad']){
$anexar='../nuevo_archivo.php?krd='.$krd.'&usua='.$krd.'&numrad='.$valRadio.'&radi='.$valRadio;
echo '<frameset cols="100%"><frame src="'.$anexar.'"> </frameset>';
DIE ("<TABLE><tr><td></td></tr></TABLE><center><table class='borde_tab' width=100% CELSPACING=5><tr class=titulosError><td><center>lalalal</CENTER></td></tr></table><center>");
}
 else{
 }
/*
...
<table border=0 width=100% cellpadding="0" cellspacing="0">
	...
	<td width=100%>
		...
		<input type="hidden" name=asocImgRad value='<?=$asocImgRad?>'>
		<input type="hidden" name=asocImgAnexo value='<?=$asocImgAnexo?>'>

		<?php
		if($asocImgRad){
		 $classTr = "aler aler-success";
		}else{
		...
		<table width="98%" border="0" cellpadding="0" cellspacing="5" class="borde_tab">
			...
			<td ><?=$asocImgRad?> <?=$asocImgAnexo?><BR></td>
				...
				<table width="500"  border=0 align="center" bgcolor="White">
					<TR bgcolor="White">
						<TD colspan="2" align="left">
                            <label for="observa" style="color:red">* Observaciones</label>
