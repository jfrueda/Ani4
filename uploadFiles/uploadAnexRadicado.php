...
<?
if (isset($_POST["busqRadicados"])){
    $busqRadicados=$_POST["busqRadicados"];
    $_rs_=$db->conn->GetArray("select radi_nume_radi from radicado where radi_nume_radi=$busqRadicados");
    if(!empty($_rs_)){
        $busq_radicados_tmp="cast(r.RADI_NUME_RADI as varchar(20)) like '%$busqRadicados%' ";
        $Buscar="Buscar";
    }
}
