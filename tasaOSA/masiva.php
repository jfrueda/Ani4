...
function generarDocumento($t, $dia, $mes, $anho, $num, $pathAux, $complementDireccion) {
    ...
    $templateProcessor->setValue('RA_ASUN', 'Por la cual se Ordena Seguir Adelante con la Ejecución.');
    ...
    $templateProcessor->setValue('MEDIDA_PREVENTIVA', $t[16] != '' ? '$ ' . number_format($t[16], 0, ',', '.') : '------------');
    ...
    $templateProcessor->setValue('EXP_SP', $t[39]);
    $templateProcessor->setValue('RES_MP_NO', $t[40]);
    $templateProcessor->setValue('FECHA_MP_NO', $t[41]);
    $templateProcessor->setValue('MODALIDAD_NOT_MP', $t[42]);
    $templateProcessor->setValue('FECHA_ NOT_MP', $t[43]);
    $templateProcessor->setValue('RADICADO_NOT_MP', $t[44]);
    $templateProcessor->setValue('PROYECTO_NO', $t[45]);
    $templateProcessor->setValue('REVISO_NO', $t[46]);
...
foreach ($sheetData as $t) {
    ...
    if($t[0] != '') {
       ...
       $rad->raAsun        =    "Por la cual se Ordena Seguir Adelante con la Ejecución.";
       $rad->guia = "";
       $rad->radi_dato_001 = "";
       $rad->radi_dato_002 = "";
       $rad->esta_fisico = 1;
            ...
            if($existeExp > 0) {
                ...
                if($estadoExp == 0) {
                    ...
                    */  
                    ...
                    $errorExp = 'Creando con exito pero el expediente esta cerrado o anulado, No se envia a Notificaciones';
                    $retorno = "Creando con exito pero el expediente esta cerrado o anulado, No se envia a Notificaciones: " . $nurad;
                }

            } else {
                $errorExp = 'Creando con exito pero no existe el expediente, No se envia a Notificaciones';
                $retorno = "Creando con exito pero no existe el expediente, No se envia a Notificaciones: " . $nurad;
            }
