...
/*  Fabian Mauricio Losada Florez      22-Noviembre-2006 						     */
/*************************************************************************************/
session_start();
$ruta_raiz = "..";
	...
	<CENTER>
		<form action="cargarcsv.php?<?= $params ?>" name="formAdjuntarArchivos" method="POST" enctype="multipart/form-data">
				...
				if ($my_upload->upload($newFile)) {
					...
					for ($ii = 0; $ii < count($datos); $ii++) {
						...
						foreach ($encabezado[0] as $campos_d) {
							...
							if ($campos_d == "*No_UNIDAD_DOCUMENTAL_BODEGA*") {
								$nun_doc = $dato_r;
							}
							if ($campos_d == "*RADICADOS*") {
								$rad = $dato_r;
							}
							if ($campos_d == "*CAJA_BODEGA*") {
								$caja = $dato_r;
							}
							if ($campos_d == "*UBICACION*") {
								$ubica = $dato_r;
							}
							if ($campos_d == "*OBSERVACION*") {
								$observa = $dato_r;
							}
							if ($campos_d == "*No_EXPEDIENTE*") {
					...
					if ($p == 1) {
						echo "El archivo no pudo ser cargado";
					} else {
			...
			<table width="90%" border="0" cellspacing="5" class="borde_tab">
				<tr align="center">
					<td class="titulos5" colspan="2">Inserte el Nombre del Archivo CSV </td>
				</tr>
				<tr>
				...
				<tr>
					<td align="center"> <input type="button" class="botones_funcion" onClick="enviar();" id="envia22" name="Cargar" value="Cargar">
