	...
	<CENTER>
		<form name=inDepe method='post' action='inDepe.php?<?= session_name() ?>=<?= trim(session_id()) ?>&krd=<?= $krd ?>&edit=<?= $edit ?>&cod=<?= $cod ?>'>
			<table width="90%" border="0" cellspacing="0" cellpadding="0" align="center" class="borde_tab">
				...
				<TR class='titulos2'>
					<TD class=titulosError colspan=4 align=center>RELACION DEPENDENCIAS CON EDIFICIOS</TD>
				</TR>
				<TR class='titulos2'>
					<TD colspan=4>&nbsp;</TD>
				</TR>
				...
				if ($Ingresar || $Modificar) {
					if ($depe != "" && $exp_edificio != "") {
						...
						if ($Ingresar) {
							...
							if ($rs->EOF) {
								echo "El registro fue insertado";
							} else {
						...
						if ($Modificar) {
							...
							if ($rsm->EOF) {
								...
								echo "El registro no pudo ser modificado";
							}
						}
					} elseif ($depe == "") {
						echo "Debe seleccionar una Dependencia";
					} elseif ($exp_edificio == "") {
						echo "Debe seleccionar un Edificio";
					} else {
						echo "Debe seleccionar un Edificio y una Dependencia";
					}
				}
				...
				<tr>
					<td colspan="4" align="center">
						...
						<? } ?>
						<input name='CERRAR' type="button" class="botones" id="envia22" onClick="opener.regresar();window.close();" value="CERRAR" align="middle">
					</td>
				</tr>
			</table>
		</form>
