		...
		<table border=0 width=70% align="center" class="borde_tab" cellspacing="1" cellpadding="0">
			...
			?>
				...
				<tr class="titulos5">
					...
					<td>
						<?php
						$camposConcatenar = "(" . $db->conn->Concat(
							"sgd_sexp_parexp1",
							"sgd_sexp_parexp2",
							"sgd_sexp_parexp3",
							"sgd_sexp_parexp4",
							...
							<tr align="center">
								...
								if (!isset($_POST[$campo])) {
									...
									if (!$rs->EOF) {
										...
										if ($esp != "" && $rsParExp->fields['SGD_PAREXP_ORDEN'] == 1) {
											$sqe = "select nombre_de_la_empresa from bodega_empresas where identificador_empresa = $esp";
											$rse = $db->conn->Execute($sqe);
											if (!$rse->EOF) {
												$par = $rse->fields['NOMBRE_DE_LA_EMPRESA'];
											} else {
												$par = $_POST['parExp_' . $rsParExp->fields['SGD_PAREXP_ORDEN']];
											}
										} elseif ($oem > 0 && $rsParExp->fields['SGD_PAREXP_ORDEN'] == 2) {
											$sqe = "select sgd_oem_oempresa from sgd_oem_oempresas where sgd_oem_codigo = $oem";
											$rse = $db->conn->Execute($sqe);
											if (!$rse->EOF) {
												$par = $rse->fields['SGD_OEM_OEMPRESA'];
											} else {
												$par = $_POST['parExp_' . $rsParExp->fields['SGD_PAREXP_ORDEN']];
											}
										} elseif ($fun > 0 && $rsParExp->fields['SGD_PAREXP_ORDEN'] == 4) {
											$sqe = "select (usua_nomb) AS SGD_NOMBRE_COMPLETO  from usuario where usua_doc = $fun";
											$rse = $db->conn->Execute($sqe);
											if (!$rse->EOF) {
												$par = $rse->fields['SGD_NOMBRE_COMPLETO'];
											} else {
												$par = $_POST['parExp_' . $rsParExp->fields['SGD_PAREXP_ORDEN']];
											}
										} else {
								...
								?>
								<td width="13%" height="25" class="titulos5" align="left">
									<input type="text" name="<?= $campo ?>"" id=" <?= $campo; ?>" value="<?= $par ?>" size="75" class="listado2">
								</td>
							</tr>
						<?php
							$rsParExp->MoveNext();
		...
		<table border=0 width=70% align="center" class="borde_tab">
			<tr align="center">
				<td width="33%" height="25" class="listado2" align="center">
					<center>
						...
						if ($tsub && $codserie && !$Actualizar && $usuaDocExp && $par) {
							if (!$crearExpediente) {
								/*
             ...
             */
						...
						?>
									...
									if ($inc == 0) {
										echo "Existe el siguiente Expediente " . $expe . " para este item: " . $par . "";
									} elseif ($inc == 1 && $expe != "") {
										echo "Existe el siguiente Expediente " . $expe . " para este item: " . $par . "";
									} else {
									?>
				...
				<td width="33%" class="listado2" height="25">
					<center><span bgcolor="#CCCCCC" class="titulos2"><a href='crearexpediente.php?<?= $phpsession ?>&krd=<?= $krd ?>&<?= "fechaf=$fechah&carpeta=8&nomcarpeta=Expedientes&orno=1&adodb_next_page=1"; ?>' target='mainFrame' class="menu_princ"><b>REGRESAR </a></span></center>
				</TD>
			</tr>
		</table>
		<script>
