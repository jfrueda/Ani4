		...
		?>
									...
									<tr>
										...
										if ($Archivar) {
											...
											if ($exp_item12 != "") {
												...
												for ($i = 1; $i <= 22; $i++) {
													$tmp1 = explode("$i", $tmp);
													if ($tmp1[0] == "CAJA " || $tmp1 == "caja ") {
														$exp_caja = $exp_item12;
													}
													if ($tmp1[0] == "AREA " || $tmp1[0] == "ZONA " || $tmp1[0] == "area " || $tmp1 == "zona ") {
														$exp_item1 = $exp_item12;
													}
													if ($tmp1[0] == "CARRO " || $tmp1 == "carro ") {
														$exp_carro = $exp_item12;
													}
													if ($tmp1[0] == "ESTANTE " || $tmp1 == "estante ") {
														$exp_estante = $exp_item12;
													}
													if ($tmp1[0] == "ENTREPANO " || $tmp1 == "eentrepano ") {
											...
											if ($exp_item22 != "") {
												...
												for ($i = 1; $i <= 22; $i++) {
													...
													if ($tmp1[0] == "AREA " || $tmp1 == "ZONA " || $tmp1[0] == "area " || $tmp1 == "zona ") {
														$exp_item1 = $exp_item22;
													}
													if ($tmp1[0] == "CARRO " || $tmp1 == "carro ") {
														$exp_carro = $exp_item22;
													}
													if ($tmp1[0] == "ESTANTE " || $tmp1 == "estante ") {
														$exp_estante = $exp_item22;
													}
													if ($tmp1[0] == "ENTREPANO " || $tmp1 == "eentrepano ") {
											...
											if ($exp_item31 != "") {
												...
												for ($i = 1; $i <= 22; $i++) {
													...
													if ($tmp1[0] == "AREA " || $tmp1 == "ZONA " || $tmp1[0] == "area " || $tmp1 == "zona ") {
														$exp_item1 = $exp_item31;
													}
													if ($tmp1[0] == "CARRO " || $tmp1 == "carro ") {
														$exp_carro = $exp_item31;
													}
													if ($tmp1[0] == "ESTANTE " || $tmp1 == "estante ") {
											...
											if ($exp_entre != "") {
												...
												for ($i = 1; $i <= 22; $i++) {
													...
													if ($tmp1[0] == "ESTANTE " || $tmp1 == "estante ") {
														$exp_estante = $exp_entre;
													}
													if ($tmp1[0] == "ENTREPANO " || $tmp1 == "entrepano ") {
										...
										$queryUs = "select SGD_SEXP_PAREXP1,SGD_SEXP_PAREXP2,SGD_SEXP_PAREXP3,SGD_SEXP_PAREXP4,SGD_SEXP_PAREXP5, SGD_SEXP_PAREXP6 from
										...
										if (!$rsUs->EOF) {
											...
											$eti6 = $rsUs->fields['SGD_SEXP_PAREXP6'];
										}
										$etiquetas = $eti1;
										if ($eti2 != "") {
											$etiquetas .= "," . $eti2;
										...
										if ($exp_carpeta != "" && $car) {
											...
											if (!$rsrad4->EOF) {
												$exp_caja = $rsrad4->fields['SGD_EXP_CAJA'];
											}
										}
										?>
									</tr>
						...
						<tr>
							<td class=listado2>
								...
								<table width="80%" height="99%" cellspacing="5" align="center" class="borde_tab">
									...
									<tr>
										<td class='titulos2'>SUBEXPEDIENTE
				...
				print $rs->GetMenu2('exp_piso2',$exp_piso2,true,false,""," onChange='submit()' class=select");
   		    	*/ $exp_piso2 = 3;
												...
												<tr class='titulos2'>
													...
													?>
														...
														<TD>
															...
															if ($ent == 2) {
																if ($item3[0] == "CARRO ") {
																	$exp_item12 = $exp_carro;
																} elseif ($item3[0] == "ESTANTE ") {
																	$exp_item12 = $exp_estante;
																} elseif ($item3[0] == "ENTREPANO ") {
																	$exp_item12 = $exp_entrepa;
																} else {
																	$exp_item12 = $exp_item1;
																}
															}
													...
													?>
														...
														<TD>
															...
															if ($exp_item22 == "" || $ent == 2) {
																if ($item4[0] == "ESTANTE ") {
																	$exp_item22 = $exp_estante;
																}
																if ($item4[0] == "ENTREPANO ") {
																	$exp_item22 = $exp_entrepa;
																} else {
																	$exp_item22 = $exp_carro;
																}
															}
											...
											<tr>
												...
												?>
													...
													<TD>
														<?
														if ($exp_item31 == "" || $ent == 2) {
															if ($item5[0] == "ENTREPANO ") {
																$exp_item31 = $exp_entrepa;
															}
															if ($item5[0] == "CAJA ") {
																$exp_item31 = $exp_caja;
															} else {
																$exp_item31 = $exp_estante;
															}
														}
												...
												$rs = $db->query($sql);
												if (!$rs->EOF) {
													$item71 = $rs->fields["SGD_EIT_NOMBRE"];
												}
											...
											<tr>
												...
												?>
													...
													<TD>
														...
														$rs = $db->query($sql);
														print $rs->GetMenu2('exp_caja', $exp_caja, true, false, "", "onChange='submit()' class=select");

														?>
													</TD>
												<? }
												$qts = "select sgd_exp_numero from sgd_exp_expediente where sgd_exp_caja like '$exp_caja'";
												//$db->conn->debug=true;
												$rsq = $db->conn->Execute($qts);
												while (!$rsq->EOF) {
													$expedi = $rsq->fields['SGD_EXP_NUMERO'];
													$qts1 = "select count(distinct(sgd_exp_carpeta)) CAR from sgd_exp_expediente where sgd_exp_numero like '$expedi'";
													$rsq1 = $db->conn->Execute($qts1);
													if (!$rsq1->EOF) {
														$ct += $rsq1->fields['CAR'];
													}
													$rsq->MoveNext();
												...
												?>
													<script language="javascript">
														confirm('Debe hacer el cambio de caja');
													</script>
												<? }
												//			*Modificado por Fabian Mauricio Losada
												?>
											...
											<tr>
												...
												if ($exp_carpeta == "") {
													$sqlrad .= " and e.SGD_EXP_CARPETA IS NULL";
												}
											...
											<tr>
												<td class='titulos2' colspan="4" align="center">ESTADO :</td>
											</tr>
											<TR>
												<td class='titulos2' align="right">ABIERTO
												<td class='titulos2' align="left">
													<?
													if ($EST == 1 || $EST == "") { //$datoss = "checked"; else $datoss= "";
													?>
														<h1>&nbsp;&nbsp;&nbsp;&nbsp; X
														</h1>
													<? } ?>
											...
											<td class='titulos2' colspan="4" align="center">UNIDAD DE CONSERVACION :</td>
									</tr>
									<TR>
						...
						if ($exp_carpeta == "") {
							$sqlrad1 .= " and e.SGD_EXP_CARPETA IS NULL";
						}
						...
						?>
						<tr>
							<td class='titulos2' align="center" colspan="4">ESTOS SON LOS RADICADOS INCLUIDOS EN ESTE EXPEDIENTE:
						</tr>
						<tr>
			...
			<div align="center">
				...
				<span class='eerrores'><a href='../login.php' target=_parent><span class="textoOpcion">Por
							Favor intente validarse de nuevo. Presione aca !</span></a></span>
			</div>
		</form>
	<?
			}
