...
<script>
	...
	function NuevoV(codp) {
		window.open("relacionTiposAlmac2.php?<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>&codp=" + codp + "", "Relacion Tipos de Almacenamiento", "height=350,width=650,scrollbars=yes");

	}

	function NuevoT(codp) {
		window.open("relacionTiposAlmac.php?<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>&codp=" + codp + "", "Relacion Tipos de Almacenamiento", "height=450,width=650,scrollbars=yes");
	...
	<table border="0" width="90%" cellpadding="0" class="borde_tab">
		...
		<tr>
			<td class="titulos5" colspan="6" align="center"><input type="button" class="botones" align="middle" name="nuevo" value="NUEVO" onClick="NuevoV(<?= $codp ?>);">
				...
				<input type="button" name="cerrar" class="botones" value="SALIR" onClick="window.close();">
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="titulos2"><B>Borrar</B></td>
		</tr>
		...
		for ($i = 0; $i < $tm; $i++) {
			...
			$codigo = $codig;
			$sql = "select * from sgd_eit_items where sgd_eit_cod_padre like '$codig'";
			$rs = $db->conn->Execute($sql);
			while (!$rs->EOF) {
				$nom[$c] = $rs->fields['SGD_EIT_NOMBRE'];
				$codi[$c] = $rs->fields['SGD_EIT_CODIGO'];
		?>
				<tr>
					...
					<td class="titulos5"><input type="radio" name="borr" value="1" onClick="Borrar(<?= $codi[$c] ?>)" <?= $sel2 ?> align="absmiddle"></td>
				</tr>
			<?
				$c++;
				$rs->MoveNext();
