...
?>
	...
	?>
		<center>
			<input size="100" onClick="ubicacion('ubicacion','<? echo $_GET['codigo']; ?>',1,'<? echo $_GET['depeA']; ?>','cancelar');" value="cancelar" type="button" class="botones" />
		<?
	}
} else {
	if (!$depe_codi) {
	...
	if ($mod != 'not'  && $perm_mod == 2 || $_GET['cambiar'] == 1) {
		?> <center>
				<input size="100" onClick="ubicacion('ubicacion','<? echo $ubica ?>',2,'<? echo $depe_codi; ?>','modificar');" value="Cambiar Ubicacion" type="button" class="botones" />
