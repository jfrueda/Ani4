...
<body bgcolor="#FFFFFF">
	...
	<form name='inventario' action="<?= $encabezadol ?>" method='post' action='inventario.php?<?= session_name() ?>=<?= trim(session_id()) ?>&krd=<?= $krd ?>&tipo=<?= $tipo ?>&sel=<?= $sel ?>'>
		<br>
		<table border=1 width='50%' cellpadding="0" class="table table-bordered">
			<tr>
				<?
		...
		<table border="0" width="70%" cellpadding="0" class="table table-bordered ">
			<?
					$TEMP = 0;
					$TE = 0;
					$DE = 0;
					...
					if ($Generar && $min != 0) {
						if ($min == 1) {
							$tmp = "ESTANTE " || $tmp = "estante ";
							$IT = "ENTREPA&Ntilde;OS";
							$IT2 = "ENTREPANO " || $IT2 = "entrepano ";
							$IT3 = "ESTANTES";
						}
						if ($min == 2) {
							$tmp = "ENTREPANO " || $tmp = "entrepano ";
							$IT = "CAJAS";
							$IT2 = "CAJA " || $IT2 = "caja ";
							$IT3 = "ENTREPA&Ntilde;OS";
						}
						include("$ruta_raiz/include/query/archivo/queryInventario.php");
						$rs = $db->query($sqli);
						$i = 1;
			...
			?>
						<TR>
							...
							<td align="center" class="listado2"><?= $porce ?></td>
						</tr>
				<?
								$i++;
								$TEMP = 0;
				...
				<div class="card shadow-sm my-3">
					<div class="card-header bg-orfeo text-white">
						<h5 class="mb-0 text-center">
							MOVIMIENTO DE COLECCION
						</h5>
					</div>
				</div>
				<div class="d-flex justify-content-center gap-4">
					<div class="form-check form-check-inline">
						<? $TIPO1 == 1 ? $sel = "checked" : $sel = ""; ?>
						<input class="form-check-input" type="radio" name="TIPO1" value="1" onClick="submit();" <?= $sel ?>>
						<label class="form-check-label"> Por Cajas </label>
					</div>

					<div class="form-check form-check-inline">
						<? $TIPO1 == 2 ? $sel = "checked" : $sel = ""; ?>
						<input class="form-check-input" type="radio" name="TIPO1" value="2" onClick="submit();" <?= $sel ?>>
						<label class="form-check-label"> Por Expedientes </label>
					</div>
				</div>

				<table border=0 width='80%' cellpadding="1" class="table table-bordered">
					<BR>
					<center>DATOS ORIGEN</center>
					<BR>
					<?
					if ($TIPO1 == 1) {
					?>
						...
						<tr>
							<td class=titulos5>CAJAS </td>
							<td class=titulos5>
								DESDE <input type=text name=cajas value='<?= $cajas ?>' class="tex_area" size="3" maxlength="2">
								&nbsp;&nbsp;&nbsp;&nbsp;HASTA
								<input type=text name=cajas2 value='<?= $cajas2 ?>' class="tex_area" size="3" maxlength="2">
							</td>
						</tr>
						<tr>
							<TD class="titulos5" align="center" colspan="2"><br> NUEVA UBICACION <br><BR></td>
						</tr>
						<tr>
			...
			<tr>
				...
				?>
			<tr>
				<td class=titulos5> EXPEDIENTES </td>
				<td class=titulos5><input type=text name=expde value='<?= $expde ?>' class="tex_area">
					<input type="submit" value=">>" class="botones_2" name="exp">
				</td>
			</tr>
			<?
						if ($exp == '>>') {
			...
			<tr>
				...
				<td class=titulos5>
					DESDE <input type=text name=unidoc value='<?= $unidoc ?>' class="tex_area" size="3" maxlength="2">
		...
		<tr>
			...
			<TD class=titulos5>
				...
				<table border=0 width 100% cellpadding="1" class="borde_tab">
					...
					<tr>
						<td class="titulos5" align="left">FECHA ARCHIVO INICIAL</td>
						<td>
					...
					<tr>
						<td class="titulos5" align="right">FINAL </td>
						<td>
							<script language="javascript">
					...
					?>
					<tr>
						<td class=titulos5>EXPEDIENTES DESDE</td>

						<td class=titulos5> <input type=text name=cajas value='<?= $exp ?>' class="tex_area" size="8" maxlength="7"></td>
					<tr>
						<td class=titulos5 align="right">HASTA &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
						<TD class="titulos5"> <input type=text name=cajas2 value='<?= $exp2 ?>' class="tex_area" size="8" maxlength="7">
						</td>
					</tr>
					<tr>
						<td class="titulos5">OBSERVACIONES</td>
						<td class="titulos5"><input type=text name=observa value='<?= $observa ?>' class="tex_area"></td>
					</tr>
				</table>
			</TD>
			<?
					if ($Agregar) {
						...
						include("$ruta_raiz/include/query/archivo/queryInventario.php");
						if ($rs = $db->query($sql5)) $depe_nom = $rs->fields['DEPE_NOMB'];
						$c = 1;
						$rse = $db->query($sqld);
						...
						while (!$rse->EOF) {
							...
							if ($srd != "" or $sbrd != "") {
								include("$ruta_raiz/include/query/archivo/queryInventario.php");
								if ($rs = $db->query($sqlsr)) $srd_desc = $rs->fields['SGD_SRD_DESCRIP'];
								$c .= 4;
