  ...
  <form name='form1' class="smart-form" method="POST" action='datos_expediente.php?<?= session_name() . "=" . session_id() . "&krd=$krd&fechah=$fechah&nurad=$nurad&num_expediente=$num_expediente&exp_fechaFin=$exp_fechaFin&exp_fechaIni=$exp_fechaIni&exp_archivo=$exp_archivo&exp_unicon=$exp_unicon&item3=$item3&item4=$item4&item5=$item5&car=$car&exp_carpeta2=$exp_carpeta2 " ?>'>
        ...
        <TD class='titulos2'>
          <table class='table table-bordered' valign='top' align='center' cellpadding="1">
            <tr>
              ...
              if ($expItem1) {
                $exp_caja = $expItem1;
                if ($expItem1 != $expItem1Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem2 && $grabar != "No") {
                $exp_caja = $expItem2;
                if ($expItem2 != $expItem2Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem3 && $grabar != "No") {
                $exp_caja = $expItem3;
                if ($expItem3 != $expItem3Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem4 && $grabar != "No") {
                $exp_caja = $expItem4;
                if ($expItem4 != $expItem4Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem5 && $grabar != "No") {
                $exp_caja = $expItem5;
                if ($expItem5 != $expItem5Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem6 && $grabar != "No") {
                $exp_caja = $expItem6;
                if ($expItem6 != $expItem6Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem7 && $grabar != "No") {
                $exp_caja = $expItem7;
                if ($expItem7 != $expItem7Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem8 && $grabar != "No") {
                $exp_caja = $expItem8;
                if ($expItem8 != $expItem8Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem9 && $grabar != "No") {
                $exp_caja = $expItem9;
                if ($expItem9 != $expItem9Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem10 && $grabar != "No") {
                $exp_caja = $expItem10;
                if ($expItem10 != $expItem10Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem11 && $grabar != "No") {
                $exp_caja = $expItem11;
                if ($expItem11 != $expItem11Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem12 && $grabar != "No") {
                $exp_caja = $expItem12;
                if ($expItem12 != $expItem12Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem13 && $grabar != "No") {
                $exp_caja = $expItem13;
                if ($expItem13 != $expItem13Ant) {
                  $grabar = "No";
                }
              }
              if ($expItem14 && $grabar != "No") {
                $exp_caja = $expItem14;
                if ($expItem14 != $expItem14Ant) {
                  $grabar = "No";
                }
              }
              if ($grabar == "No") {
    ...
    SGD_SEXP_SECEXPEDIENTES where SGD_EXP_NUMERO='$num_expediente' order by sgd_exp_caja desc";
                  ...
                  <TR class='titulos2'>
                    ...
                    ?>
                      ...
                      <TD>
                        ...
                        if ($iItem >= 2) {
                          $itemCodigo = $btt->itemCodigoPadre[$iItem];
                        } else {
                          $itemCodigo = $btt->itemCodigo[$iItem];
                        }
        ...
        order by SGD_EIT_NOMBRE";
            ...
            <tr>
              ...
              while (!$rsrad->EOF) {
                ...
                if ($esta[$j] == 1) {
                  $noHojasXCarpeta += $fol[$j];
                }

                if ($noHojasXCarpeta >= 201 && $carpetaOld != $carpeta[$j]) {
              ?>
                  <script language="javascript">
      ...
      <TR>
        <td class='titulos2' colspan="4" align="center">CAR &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          ...
          ?>
          <input name="UN" type="radio" value="4" <?= $datoss ?>>
        </td>
      </tr>
      </tr>
      <?
    ...
    <div class="tabber">
      ...
      ?>
          <div class="tabbertab">
            <?
            if ($carpeta[$t] == 0 || !trim($carpeta[$t])) {
            ?>
              <H3>
                <font color=red>Sin Carpeta Asignada (<?= $foliosXCarpeta[$carpeta[$t]] ?>Folios)</font>
              </h3>
            <?
            } else {
            ?>
            ...
            <tr class='<?= $estiloC ?>'>
              ...
              <? if ($esta[$t] == '10000' || $arrayRad[$t] == $nurad9) {
                $st = "checked";
              } else {
                $st = "";
              }
