	...
	<form name=busqueda_archivo action="<?= $encabezadol ?>" method='post' action='busqueda_archivo.php?<?= session_name() ?>=<?= trim(session_id()) ?>krd=<?= $krd ?>'>
		...
		<div class="card shadow border-0">
			<div class="card-header bg-orfeo text-white py-3">
				<h5 class="mb-0">BUSQUEDA AVANZADA (SOLO PARA RADICADOS ARCHIVADOS)</h5>
			</div>

			<div class="card-body margin-botton-table">

				<table border=0 width="90%" cellpadding="0" class="table table-bordered">
					<tr>
						...
						<td width="20%" class='titulos2'>
							<input type=text name=buscar_exp value='<?= $buscar_exp ?>' class="tex_area form-control">
						</td>
					</tr>
					<tr>
						...
						<td width="20%" class='titulos2'>
							<input type=text name=buscar_rad value='<?= $buscar_rad ?>' class="tex_area form-control">
						</td>
					</tr>
					<tr>
						<td width="20%" class='titulos2' align="left">PROCESO </td>
						<td width="20%" class='titulos2'>
							<?
							...
							include "$ruta_raiz/include/tx/ComentarioTx.php";
							print $rsP->GetMenu2("codProc", $codProc, "0:-- Seleccione --", false, "", "class='form-select'");
							?>
						</td>
						<td width="20%" class='titulos2' align="left">PARAMETROS </td>
						<td width="20%" class='titulos2'>
							<input type=text name=buscar_parametros value='<?= $buscar_parametros ?>' class="tex_area form-control">
						</td>
					</tr>
					<?
					if ($docu1 == 1) {
					...
					?>
					<tr>
						<td width="20%" class='titulos2' align="left">
							NIT
							...
							?>
							<input name="docu1" type="checkbox" class="select" value="3" <?= $datoss ?>>
						</td>
						<td width="20%" class='titulos2'>
							<input type="text" name=buscar_docu value='<?= $buscar_docu ?>' size="11" maxlength="9" class="tex_area form-control">
					...
					<tr>
						<td width="20%" class='titulos2' align="left">PISO </td>
						<td width="20%" class='titulos2'>
							<?
							...
							print $rs1->GetMenu2('buscar_piso', $buscar_piso, true, false, "", "onChange='submit()' class=form-select");
							?>
						...
						<td width="20%" class='titulos2'>
							...
							$rs = $db->query($sql);
							print $rs->GetMenu2('buscar_ufisica', $buscar_ufisica, true, false, "", "onChange='submit()'  class=form-select");
							?>
						</td>
					</tr>
					<tr>
						...
						<td width="20%" class='titulos2'>
							...
							$rs1 = $db->conn->query($query);
							print $rs1->GetMenu2('buscar_estan', $buscar_estan, true, false, "", "onChange='submit()' class=form-select");
							?>
						...
						<td width="20%" class='titulos2'>
							...
							$rs = $db->query($sql);
							print $rs->GetMenu2('buscar_entre', $buscar_entre, true, false, "", "onChange='submit()' class=form-select");
							?>
						</td>
					</tr>
					<tr>
						...
						<td width="20%" class='titulos2'>
							...
							$rs1 = $db->conn->query($query);
							print $rs1->GetMenu2('buscar_caja', $buscar_caja, true, false, "", "onChange='submit()' class=form-select");
							?>
						...
						<td width="20%" class='titulos2'>
							...
							$rs = $db->query($sql);
							print $rs->GetMenu2('buscar_caja2', $buscar_caja2, true, false, "", " class=form-select");
							?>
						</td>
					</tr>
					<tr>
						...
						<td width="20%" class='titulos2'>
							...
							print $rsPE->GetMenu2("codDpto", $codDpto, "0:-- Seleccione --", false, "", " onChange='submit()' class='form-select'");
							?>
					...
					<tr>
						<td width="20%" class='titulos2' align="left">MUNICIPIO </td>
						<td width="20%" class='titulos2'>
							<?
							if ($codDpto != "") {
								$rsPX = $db->conn->query($queryMuni);
								print $rsPX->GetMenu2("codMuni", $codMuni, "0:-- Seleccione --", false, "", "class='form-select'");
							}
							?>
					...
					<tr>
						<td width="20%" class='titulos2' align="left">RETENCION </td>
						<?
						if ($buscar_rete == 1) {
							$datoss = "checked";
						} else {
						...
						?>
						<td width="20%" class='titulos2'>
							AG <input name="buscar_rete" type="checkbox" class="select" value="1" <?= $datoss ?>> &nbsp;&nbsp;
							<?
							if ($buscar_rete == 2) {
								$datoss = "checked";
							} else {
					...
					<td colspan="4" align="center">
						<input type=submit value=Buscar name=Buscar class="botones btn btn-success">&nbsp;
					</td>
				</table>
				<br>
				<?
				...
				?>
					<div class="table-responsive">
						<table border=0 width="100%" cellpadding="1" class="table table-hover align-middle mb-0">
							...
							<tbody>
								...
								if ($codProc != '0') {
									$pross = "s.SGD_PEXP_CODIGO = '$codProc'";
									$ef = "and";
								} else {
									$pross = "";
									$ef = "";
								...
								if ($buscar_piso != "") {
									$pis = "e.SGD_EXP_ISLA = '$buscar_piso'";
									$f = "and";
								} else {
									$pis = "";
									$f = "";
								...
								if ($buscar_estan != "") {
									if ($item3 == "ESTANTE ") {
										$estan = "e.SGD_EXP_ESTANTE = '$buscar_estan'";
									} elseif ($item3 == "CARRO ") {
										$estan = "e.SGD_EXP_CARRO = '$buscar_estan'";
									} elseif ($item3 == "ENTREPANO ") {
										$estan = "e.SGD_EXP_ENTREPA = '$buscar_estan'";
									}
									$h = "and";
								} else {
									$estan = "";
								...
								if ($buscar_entre != "") {
									if ($item4 == "ENTREPANO ") {
										$entre = "e.SGD_EXP_ENTREPA = '$buscar_entre'";
									} elseif ($item4 == "ESTANTE ") {
										$entre = "e.SGD_EXP_ESTANTE = '$buscar_entre'";
									}
									$v = "and";
								} else {
									$entre = "";
								...
								if ($buscar_caja != "") {
									if ($item5 == "ENTREPANO ") {
										$caja = "e.SGD_EXP_ENTREPA = '$buscar_caja'";
									} elseif ($item5 == "CAJA ") {
										$caja = "e.SGD_EXP_CAJA = '$buscar_caja'";
									}
									$t = "and";
								} else {
									$caja = "";
								...
								if ($buscar_caja2 != "") {
									$caja2 = "e.SGD_EXP_CAJA = '$buscar_caja2'";
									$u = "and";
								} else {
									$caja2 = "";
									$s = "";
								...
								if ($sep == '1') {
									if ($fechaInii == $fechaInif) {
										$fecha = "e.SGD_EXP_FECH_ARCH = to_date('$fechaInii','YYYY-mm-dd')";
									} else {
										$time = fnc_date_calcy($fechaInif, '1');
										$fecha = "e.SGD_EXP_FECH_ARCH <= '$time' and e.SGD_EXP_FECH_ARCH >= to_date('$fechaInii','YYYY-mm-dd')";
									}
									$i = "and";
								} else {
									$fecha = "";
								...
								if ($sel1 == '1') {
									if ($fechaFini == $fechaFinf) {
										$fecha = "e.SGD_EXP_FECHFIN = to_date('$fechaFini','YYYY-mm-dd')";
									} else {
										$time2 = fnc_date_calcy($fechaFinf, '1');
										$fechafin = "e.SGD_EXP_FECHFIN <= '$time2' and e.SGD_EXP_FECHFIN >= to_date('$fechaFini','YYYY-mm-dd')";
									}
									$j = "and";
								} else {
									$fechafin = "";
								...
								if ($buscar_parametros != "") {
									$param = "s.SGD_SEXP_PAREXP1 LIKE '%$buscar_parametros%' OR s.SGD_SEXP_PAREXP2 LIKE
								'%$buscar_parametros%' OR s.SGD_SEXP_PAREXP3 LIKE '%$buscar_parametros%' OR s.SGD_SEXP_PAREXP4 LIKE '%$buscar_parametros%'
								OR s.SGD_SEXP_PAREXP5 LIKE '%$buscar_parametros%'";
									$l = "and";
								} else {
									$param = "";
									$l = "";
								...
								if ($buscar_consecutivo != "") {
									$conse = "e.SGD_EXP_CARPETA = '$buscar_consecutivo'";
									$n = "and";
								} else {
									$conse = "";
									$n = "";
								...
								if ($buscar_ufisica != "") {
									if ($item2 == "AREA ") {
										$archi = "e.SGD_EXP_UFISICA = '$buscar_ufisica'";
									}
									if ($item2 == "ESTANTE ") {
										$archi = "e.SGD_EXP_ESTANTE = '$buscar_ufisica'";
									} elseif ($item2 == "CARRO") {
										$archi = "e.SGD_EXP_CARRO = '$buscar_ufisica'";
									}
									$o = "and";
								} else {
									$archi = "";
								...
								if ($codMuni != '0') {
									...
									LIKE '%$munici%' OR s.SGD_SEXP_PAREXP4 LIKE '%$munici%' OR s.SGD_SEXP_PAREXP5 LIKE '%$munici%')";
									$q = "and";
								} else {
									$muni = "";
									$p = "";
								...
								if ($codDpto != '0') {
									...
									LIKE '%$departa%' OR s.SGD_SEXP_PAREXP4 LIKE '%$departa%' OR s.SGD_SEXP_PAREXP5 LIKE '%$departa%')";
									$p = "and";
								} else {
									$depa = "";
									$q = "";
								...
								if ($fecha != "" || $fechafin != "") {
									$orde = " order by e. sgd_exp_fech_arch";
								} else {
									$orde = " order by e. sgd_exp_numero";
								}
								...
								if ($buscar_docu != "" && $at == '000') {
									include "$ruta_raiz/include/query/archivo/queryBusqueda_exp.php";
									$rs = $db->query($sqlca);

									while (!$rs->EOF) {
										$radi = $rs->fields["RADI_NUME_RADI"];
										$fechrad = $rs->fields['RADI_FECH_RADI'];
								...
								?>
											<tr>
												<?
												if ($valImg == "SI") {
													echo "<td class=leidos2 align=center><a class=\"vinculos\" href=\"#2\" onclick=\"funlinkArchivo('$radi','$ruta_raiz');\">$radi</a> </b></td>";
												?>
												...
												<?
													...
													echo "<td class=leidos2 align=center><a class=\"vinculos\" href=\"#\" onclick=\"javascript:noPermiso();\">$radi</a> </b></td>";
													echo "<td class=leidos2 align=center><a class=\"vinculos\" href=\"#\" onclick=\"javascript:noPermiso();\">$fechrad</a> </b></td>";
												}
												?>
									...
									while (!$rs->EOF) {
										...
										<tr>
											...
											<?
												...
												echo "<td class=leidos2 align=center><a class=\"vinculos\" href=\"#2\" onclick=\"javascript:noPermiso();\">$radi</a> </b></td>";
												echo "<td class=leidos2 align=center><a class=\"vinculos\" href=\"#2\" onclick=\"javascript:noPermiso();\">$fechrad</a> </b></td>";
											}
											?>
											<td class=leidos2 align="center"><a href='datos_expediente.php?<?= $encabezado . "&num_expediente=$expnum&ent=1&nurad=$radi" ?>' class='vinculos'><?= $expnum ?></td>
											<td class=leidos2 align="center"><b><span class=leidos2><?= $documento ?></b></td>
