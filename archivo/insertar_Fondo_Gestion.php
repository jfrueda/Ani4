...
<head>
	...
	?>
		<title>Modificar Archivo Fondo Gestion</title>
	<?
		$edit = 2;
	} else {
	?>
		<title>Insertar Archivo Fondo Gestion</title>
	<? } ?>
...
<body bgcolor="#FFFFFF">
	...
	<form name=insertar_Fondo_Gestion method='post' action='insertar_Fondo_Gestion.php?krd=<?= $krd ?>&edit=<?= $edit ?>&rad=<?= $rad ?>&depe2=<?= $depe2 ?>&ano2=<?= $ano2 ?>'>
		<br>
		...
		<table border=0 width="90%" cellpadding="0" class="borde_tab">
			...
			if ($edit >= 1 || $Modificar) {
			?>
				<td class=titulosError width="80%" colspan="4" align="center">MODIFICAR ARCHIVO FONDO GESTION </td>
			<?
			} else {
			?>
				<td class=titulosError width="80%" colspan="4" align="center">INSERTAR ARCHIVO FONDO GESTION </td>
			<? } ?>
			...
			$item6 = "CAJA";
			if ($edi >= 1) {
				include "$ruta_raiz/include/query/archivo/queryinsertar_gestion.php";
				//$db->conn->debug=true;
			...
			<tr>
				<td width="20%" class='titulos2' align="left"> FECHA INICIAL
				</td>
				<td width="20%" class='titulos2'>
	...
	dateAvailable2.dateFormat="yyyy-MM-dd";
			...
			<tr>
				<td width="20%" class='titulos2' align="left">ZONA </td>
...
$edi1=$edi->fields['SGD_EIT_CODIGO'];
  ...
  print $rs1->GetMenu2('buscar_zona',$buscar_zona,true,false,"","onChange='submit()' class=select");*/
			...
			<tr>
				...
				<td width="20%" class='titulos2'><input type="radio" name="cara" value="A" <?= $sec1 ?>>A &nbsp;
					...
					<input type="radio" name="cara" value="" <?= $sec3 ?>>Ninguno &nbsp;
				</td>
				<?
				/*
  ...
  */ ?>
			...
			<tr>
				...
				<td width="20%" class='titulos2'><input type="text" name="anexo" value=<?= $anexo ?>>
				</td>
		...
		?>
			...
			<td colspan="2" align="left"><a href='busqueda_Fondo_Gestion.php?<?= session_name() ?>=<?= trim(session_id()) ?>krd=<?= $krd ?>&Buscar'><input name='Regresar' align="middle" type="button" class="botones" id="envia22" value="Regresar"></td>
		<?
				} elseif ($edit == 3) {
		?>
		...
		?>
			<td colspan="2" align="right"><input type=submit value=Ingresar name=Ingresar class="botones">&nbsp;</td>
		...
		?>
						...
						<?
							...
							$rad2[1] = $rad;
							$observa = "Ingreso de registro en archivo de Gestion en la ubicacion: Zona " . $zona . "-Carro " . $carro . "-Cara " . $cara . "-Estante " . $estante . "-Entrepano " . $entre . "-Caja " . $caja;
				...
				<?
						$observa = "Modificacion de registro en archivo de Gestion en la ubicacion: Zona " . $zona . "-Carro " . $carro . "-Cara " . $cara . "-Estante " . $estante . "-Entrepano " . $entre . "-Caja " . $caja;
						$rad2[1] = $rad1;
				...
				?>
				<script language="javascript">
					confirm('Debe seleccionar un a&ntilde;
							o ');
				</script>
			<?
			} elseif ($depe == "") {
			...
			?>
				<script language="javascript">
					confirm('Debe ingresar la fecha de inicial con el a&ntilde;o que selecciono');
				</script>
		<?
			}
		}
