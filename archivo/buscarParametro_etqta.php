...
<head>
  ...
  <script>
    function pasar_datos(idParametro, elemento) {
      //	alert(opener.document.getElementById('nexped').value);	
      opener.document.getElementById('nexped').value = "";
      dato = eval('document.forma.' + elemento + '.value');
      eval('opener.document.TipoDocu.nexped.value = dato');
    }
...
<body bgcolor="#FFFFFF">
  ...
  if ($no_documento1 && ($agregar || $modificar)) {
    $no_documento = $no_documento1;
  }
  if (!$no_documento1 && $nombre_nus1  && ($agregar || $modificar)) {
    $nombre_essp = $nombre_nus1;
  }
  if (!$forma) {
  ?>
    <form action='buscarParametro_etqta.php?busq_salida=<?= $busq_salida ?>&krd=<?= $krd ?>&verrad=<?= $verrad ?>&nombreTp1=<?= $nombreTp1 ?>&nombreTp2=<?= $nombreTp2 ?>&nombreTp3=<?= $nombreTp3 ?>&ent=<?= $ent ?>' method="post" name="forma">
    <?
  }
    ...
    <table class="borde_tab" width="100%" cellpadding="0" cellspacing="4" name="datosExpediente">
      <tr class="listado2">
        <TD colspan="10" style="color:#FF0000">
          ...
          <center><b style="font-size:14px" style="color:#00ff00">NOTA: Para pasar el dato al formulario de cambio de etiqueta, por favor haga click en el campo Nombre.</b></center>
        </TD>
      </tr>
      <tr align="center">
        <td class="titulos5">&nbsp;</td>
        <td class="titulos5" width="110">DOCUMENTO O NIT</td>
