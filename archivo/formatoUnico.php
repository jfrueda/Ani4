...
if ($_GET['action'] == "Cosultar") {
	...
	if ($valida == "caja") {
		$isql = "select e.sgd_exp_numero,r.radi_depe_radi,e.sgd_exp_titulo,e.SGD_EXP_FECH_ARCH,e.RADI_NUME_RADI,e.SGD_EXP_ESTADO,r.RADI_NUME_HOJA,e.sgd_exp_carpeta as CARP,e.sgd_exp_caja_bodega 
					...
					,(select sgd_eit_codigo from SGD_EIT_ITEMS where SGD_EIT_COD_PADRE =$id2 order by SGD_EIT_NOMBRE) ent 
					WHERE e.sgd_exp_caja_bodega='$id' 
	...
	fclose($fp);
?>
	El archivo generado lo puede descargar de
	<br>
	<br>
	<a href='<?= $path ?>'>
		<input type="button" class="botones_funcion" value="Descargar" name="descargar">
	</a>
<?
...
?>
	<select id='<?= $id; ?>' onchange='CAJAS("<? echo $next; ?>");'>
		<option value='not'>-- Selecione --</option>
		<?php echo $option; ?>
	</select>
<?php
			...
			function planilla(div, planilla) {
				...
				switch (planilla) {
					case "caja":
						...
						id2 = document.getElementById('entre').value;
						if (caja != "not") {
							var poststr = "id=" + caja + "&div=" + div + "&action=Cosultar&validar=" + planilla + "&id2=" + id2;
				...
				<div class="col-12">
					<div class="card shadow-sm">
						<div class="card-header bg-orfeo text-center fw-bold">
							<h2 class="text-white">Formato Único de Inventario Documental</h2>
						</div>
						...
						<div class="card-body">
							<table class="table table-bordered align-middle mb-0">
								<tbody>
									<tr>
										<td class="fw-semibold">Edificio</td>
										<td>
											<select id="edificio" class="form-select" onchange='CAJAS("combopiso");'>
												<option value="not">-- Seleccione --</option>
												<?php echo $option; ?>
											</select>
										</td>

										<td class="fw-semibold">Piso</td>
										<td>
											<div id="combopiso">
									...
									<tr>
										<td class="fw-semibold">Estante</td>
										<td>
											<div id="comboestante">
												<select id="estante" class="form-select">
													<option value="not">-- Seleccione --</option>
												</select>
											</div>
										</td>
										...
										<td colspan="2" class="text-center">
											<input
												...
												onclick='planilla("estructura","estante");'
												value="Generar Estante"
												name="Buscar">
										</td>
									</tr>
									<tr>
										<td class="fw-semibold">Entrepaño</td>
										<td>
											<div id="comboentre">
												<select class="form-select">
													<option value="not">-- Seleccione --</option>
										...
										<td colspan="2" class="text-center">
											<input
												...
												onclick='planilla("estructura","entre");'
												value="Generar Entrepaño"
												name="Buscar">
										</td>
									</tr>
									<tr>
										<td class="fw-semibold">Caja</td>
										<td>
											<div id="combocaja">
										...
										<td colspan="2" class="text-center">
											<input
												...
												onclick='planilla("estructura","caja");'
												value="Generar Caja"
												name="Buscar">
										</td>
									</tr>
					...
					<div class="mt-4" id="estructura"></div>
				</div>
			</div>
		</div>
	</body>
