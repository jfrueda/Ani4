...
if ($rs_) {
  while (!$rs_->EOF) {
    ...
    if (!$rs_sgd_oem_codigo) {
      $rs_sgd_oem_codigo = "0";
    }
    if (!$rs_sgd_ciu_codigo) {
      $rs_sgd_ciu_codigo = "0";
    }
    if (!$rs_sgd_doc_fun) {
      $rs_sgd_doc_fun = "0";
    }
    if (!$rs_->fields['MUNI_CODI']) {
      $muniCodiP = 1;
    } else {
      $muniCodiP = $rs_->fields['MUNI_CODI'];
    }
    if (!$rs_->fields['DPTO_CODI']) {
      $dptoCodiP = 11;
      $muniCodiP = 1;
    } else {
      $dptoCodiP = $rs_->fields['DPTO_CODI'];
    }
    if (!$rs_->fields['SGD_DIR_DIRECCION']) {
      $direccionP = "";
    } else {
      $direccionP = $rs_->fields['SGD_DIR_DIRECCION'];
    }
    if (!$rs_->fields['SGD_TRD_CODIGO'] || $rs_->fields['SGD_TRD_CODIGO'] == '') {
      $sgdTrdCodigoP = "0";
    } else {
      $sgdTrdCodigoP = $rs_->fields['SGD_TRD_CODIGO'];
    }
  ...
  echo "No se pudo obtener información del destinatario";
  exit;
}
