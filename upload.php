...
if ($usuacrea) {
   if ((!$sololect && !$MAX_FILE_SIZE) || ($sololect && $MAX_FILE_SIZE)) {
      ...
      $isql = "update anexos set anex_solo_lect='$auxsololect',anex_salida=$anex_salida " . "where anex_codigo=$anexo";
   ...
   if ($MAX_FILE_SIZE) {
      ...
      if ($bien) {
         ...
         if ($bien) {
            ...
            $isql = "update anexos set anex_nomb_archivo='$archivo',anex_salida=$anex_salida  " . "where anex_codigo=$anexo";
            $bien = ora_do($handle, $isql);
         }
         if ($bien) {
            $resp2 = "OK";
