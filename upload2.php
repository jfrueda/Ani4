...
if ($_POST['tpradic'] == 0 || $_GET['tpradic'] == 0) {
	$NO_RADICAR = true;
}
...
if (!$cc) {
	...
	if ($bien && $nuevo_archivo && !$esNotificacione && $tpradic != 3) {
		...
		foreach ($destinatarios as $destinatario) {
			...
			if ($envio != '') {
				if ($envio == 'E-mail' || $envio == 'Ambos') {
					...
					$TX_COMENTARIO_ANEXO = "Anexo $codigo medio de envio: $envio " . (($validez == 'true') ? ' certificado' : '') . (($cierre == 1) ? ' .Cierre automatico del radicado de entrada' : '');
					$_numrad[0] = $numrad;
