<?php
/**
* @author Correlibre.org - Jairo Losada   <jlosada@gmail.com>
...
<TABLE width="100%" class="table table-bordered table-hover dataTable no-footer smart-form" cellspacing="5">
  <TR>
    <TD height="30" valign="middle"    align="center">DEVOLUCION DE DOCUMENTOS</td>
  </tr>
</table>
...
<table width="350" class="borde_tab" cellpadding="5">
  <TR>
    <TD width="125" height="21"  >Tipo de Devolucion<br></TD>
  ...
  <TR>
    <TD height="26" >Comentarios</TD>
    <TD valign="top" >
	<input type=text name=comentarios_dev value='<?=$comentarios_dev?>' class=tex_area size=70>
    </TD>
  </TR><tr>
   </tr><tr><td height="26" colspan="2" valign="top" > <center>
     <input type=SUBMIT name='devolver_rad'  value = 'CONFIRMAR DEVOLUCION' class="btn btn-primary btn-xs" ></center></td>
  </tr>
</TABLE>
</center>
<table><tr><td></td></tr></table>
...
if($devolver_rad  and $motivo_devol)
{
 ...
 $condifion_sgd_dir = str_replace("radi_nume_sal", "radi_nume_radi", $radicados_devOrginal);
 ...
 //echo "<br> ****** >>> $condicion <br>";
    ...
    $isqluDir = "update sgd_dir_drecciones
    set
      sgd_dir_enviado=0
    where $condifion_sgd_dir ";
    
	  
	  $rsDir = $db->conn->Execute($isqluDir);
    ...
    $num = count($checkValue);
	   ...
	   $i = 0;
		while ($i < $num)
		 ...
		 $systemDate = $db->conn->OffsetDate(0,$db->conn->sysTimeStamp);
	  	$isql = "update anexos
			...
			sgd_deve_fech=$systemDate,
			sgd_deve_codigo = $motivo_devol
			where sgd_dir_tipo=sgd_dir_tipo
		...
		//select DEPE_CODI   ,HIST_FECH,USUadicA_CODI  ,RADI_NUME_RADI   ,HIST_OBSE         ,USUA_CODI_DEST,USUA_DOC   ,SGD_TTR_CODIGO from hist_eventos where RADI_NUME_RADI
		$isql_hl= "insert
	...
	<table><tr><td></td></tr></table>
	<table><tr><td></td></tr></table>
	<TABLE width="100%" class='borde_tab' cellspacing="5"><TR><TD height="30" valign="middle"    align="center">
