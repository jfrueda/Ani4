...
<BODY>
	...
	<form name="new_product" action='dev_corresp_gestion.php?<?= session_name() . "=" . session_id() . "&krd=$krd&fecha_h=$fechah&$encabezado_i" ?>' method=post>
		<center>
			<?
					...
					<table width="350" class="borde_tab" cellpadding="5">
						<TR>
							...
							<TD width="225" align="right" valign="top" class='listado2'>
								...
								$sql = "select $sqlConcat ,SGD_DEVE_CODIGO from SGD_DEVE_DEV_ENVIO order by SGD_DEVE_CODIGO";
						...
						<tr>
							<td height="26" colspan="2" valign="top" class='titulos2'>
								<center>
									<input type=SUBMIT name='devolver_rad' value='CONFIRMAR DEVOLUCION' class='botones_largo'>
								</center>
							</td>
						</tr>
					</TABLE>
...
*/
		 ...
		 ";
			...
			if ($devolver_rad  and $motivo_devol) {
				if ($motivo_devol != 0) {
					...
					$sinEnvios = function ($rad) {
						return substr($rad, 0, strpos($rad, '+'));
					};
					$pieces = explode(',', $radicados_devOrginal);
					$pieces = array_map($sinEnvios, $pieces);
					$radicados_devOrginal = implode(',', array_unique($pieces));
		  ...
		  where $condicionUp ";
					...
					while ($i < $num) {
						$record_id = key($checkValue);
						//se eliminar el registro de envio.
						$env_dat = $db->conn->getRow("SELECT a.radi_nume_salida as anex_radi_nume, s.tipo as TIPO FROM sgd_rad_envios s JOIN anexos a ON s.id_anexo = a.id WHERE s.id = $record_id");
						$db->conn->Execute("UPDATE sgd_rad_envios SET estado = 0, devuelto = 't' WHERE id = $record_id");
						$radicados_dev .= $env_dat['ANEX_RADI_NUME'] . ",";
						$radicados_sel = $env_dat['ANEX_RADI_NUME'];
						$radicados_lis .= $env_dat['ANEX_RADI_NUME'] . ",";
						$tipo = $env_dat['TIPO'];
						$chkt = $radicados_sel;
						$datos = $db->conn->getAll('SELECT usua_codi_dest, depe_codi_dest FROM hist_eventos WHERE radi_nume_radi = ' . $chkt . ' and sgd_ttr_codigo = 2 LIMIT 1');
		...
		$rs = $db->conn->Execute($isql_hl);*/
						...
						$hist->insertarHistorico(
							...
							$dependencia,
							$codusuario,
							"Devolución $tipo ($motivo). $comentarios_dev",
							28
						);
		...
		where r.radi_nume_radi = $chkt";
				...
				where r.radi_nume_radi = $chkt";
					...
					where depe_codi = " . $rs->fields['RADI_DEPE_ACTU'] . " and usua_codi = " . $rs->fields['RADI_USUA_ACTU'];
							...
							if ($rsUsuAnte->fields['USUA_ESTA'] == 1) {
								$depDestino = $rs->fields['RADI_DEPE_ACTU'];
								$codUsDestino = $rs->fields['RADI_USUA_ACTU'];
								$flagUser = true;
							}
						}
				...
				and hist_fech BETWEEN  (SELECT now() - interval '1 minute') and (SELECT now() + interval '1 minute')";
							...
							if ($rsSqlContaHist->fields['COUNT'] == 0) {
								$tx->reasignar(array($chkt), $krd, $depDestino, $dependencia, $codUsDestino, $codusuario, 1, $observa,  9, 0, false);
							}
						} else {
							$sqlDepeRadi = "select radi_depe_radi from radicado  where radi_nume_radi = $chkt";
							$rsDepeRadi = $db->conn->Execute($sqlDepeRadi);
							...
							$rs = $db->conn->Execute($jefeUsuario);
							$depDestino = $rs->fields['DEPE_CODI'];
							$codUsDestino = $rs->fields['USUA_CODI'];
							$observa = 'Se asigna automáticamente el radicado debido a la devolución y se asigna al jefe del área proyectora por inactividad del último usuario responsable del radicado.';
							$tx->reasignar(array($chkt), $krd, $depDestino, $dependencia, $codUsDestino, $codusuario, 1, $observa, 9, 0, false);
						}

						//$dsql = "DELETE FROM sgd_renv_regenvio WHERE radi_nume_sal = ".$chkt;
						...
						if ($radi_nume_padre != $chkt) {
							/*$isql_hl= "insert
			into hist_eventos(DEPE_CODI   ,HIST_FECH,USUA_CODI  ,RADI_NUME_RADI   ,HIST_OBSE         ,USUA_CODI_DEST,DEPE_CODI_DEST,USUA_DOC   ,SGD_TTR_CODIGO)
			values ($dependencia, $systemDate ,$codusuario,$radi_nume_padre,'Devolucion($chkt, $motivo). $comentarios_dev $tipo','$usua_codi_dest', '$depe_codi_dest', '$usua_doc','28')";
			$rs = $db->conn->Execute($isql_hl);*/

							$radicadosSel[0] = $radi_nume_padre;
							$hist->insertarHistorico(
								$radicadosSel,
								$dependencia,
								$codusuario,
								$dependencia,
								$codusuario,
								"Devolución($chkt, $motivo). $comentarios_dev $tipo",
								28
							);


							//$dsql = "DELETE FROM sgd_renv_regenvio WHERE radi_nume_sal = ".$radi_nume_padre;
							//$rs = $db->conn->Execute($dsql);
						}
		...
		where r.radi_nume_radi = $rad";
				...
				?>
					...
					<TABLE width="100%" class='borde_tab' cellspacing="5">
						<TR>
							<TD height="30" valign="middle" class='listado2' align="center">
			...
			if (!$devolver_rad  or !$motivo_devol) {
				...
				$radicados_devOrginal = str_replace(",)", "", $radicados_devOrginal);
				$radicados_devOrginal = str_replace("-", " and a.SGD_DIR_TIPO=", $radicados_devOrginal);
				$radicados_devOrginal = str_replace("+", " and e.ID=", $radicados_devOrginal);
				$radicados_devOrginal = str_replace(",", ") or (radi_nume_salida=", $radicados_devOrginal);
				$radicados_devOrginal = "and ((radi_nume_salida=$radicados_devOrginal))";
