...
class ConstanciaEjecutoria{
    ...
    */
    ...
    function buscarDataResolucion($resolucion){
        ...
        if($rs && !$rs->EOF){
            ...
            $sql = "SELECT sgd_dir_nombre, sgd_dir_apellido, sgd_dir_doc
            FROM public.sgd_dir_drecciones where radi_nume_radi = '$resolucion'";
            $rs = $this->db->query($sql);
            array_push($data, substr(trim($rs->fields["SGD_DIR_NOMBRE"] . ' ' . $rs->fields["SGD_DIR_APELLIDO"]),0,98));
        ...
        for ($i = 0; $i < count($data); $i++) {
            $sql = "INSERT INTO public.sgd_ce_constancia(
                        ...
                        reso_queja_revoc, tipo_notificacion_final, fecha_notificacion_final, expediente, fecha_notificacion_ultimo, fecha_ejecutoria,
                         ...
                         depe_codi_fir, usua_codi_fir, depe_codi_rev, usua_codi_rev, comentario_proy)
                        VALUES ($count, ($i + 1), '" .$data[$i][0] . "', '" .$data[$i][1] . "', 
                            '" .$data[$i][2] . "', '" .$data[$i][3] . "', '" .$data[$i][4] . "',
                             ...
                             '', '', '" .$data[$i][17] . "', '" .$data[$i][18] . "', '', '', '', 0, 0, 0, 1, (select now()),0,0,0,0, '" .$data[$i][19] . "');";
    ...
    function verSolicitudPorId($idSolicitud){
        ...
        if($rs && !$rs->EOF){
            ...
            $rs->fields["FECHA_SOLICITUD"], $rs->fields["OBSERVACION"], $rs->fields["COMENTARIO_PROY"] );
        }
        return $data;
    }    
    ...
    function editarSolicitud($data){
        ...
        $sql = "UPDATE public.sgd_ce_constancia
                ...
                fecha_notificacion_final='" . $data[16] . "', expediente='" . $data[17] . "', comentario_proy='" . $data[18] . "' 
                    WHERE id = " . $data[0];
    ...
    function editarSolicitudFunNoti($data){
        ...
        $sql = "UPDATE public.sgd_ce_constancia
                ...
                observacion = '" . $data[20] . "' WHERE id = " . $data[0];
     ...
     */
    function envioRolesNoti($idSolicitud, $estado, $depeCodi, $usuaCodi, $comentarioRol){
        $comentario = '';
        if($estado == 3) {
            $sql = "update public.sgd_ce_constancia set id_estado = " . $estado . ", comentario = '" . $comentarioRol . "', 
            ...
            if($estadoAnte == 2 || $estadoAnte == 7){
                $sql = "update public.sgd_ce_constancia set id_estado = " . $estado . ", comentario = '" . $comentarioRol . "', ubicacion = '',  
                alerta = false, depe_codi_rev = '" . $depeCodi . "', usua_codi_rev = '" . $usuaCodi . "' where id = " . $idSolicitud;
        ...
        $this->db->conn->execute($sql); 
        if($comentarioRol != '') {
            $comentarioRol = 'Se envia con el comentario: ' . $comentarioRol;
            $this->agergarHistorial($idSolicitud, $depeCodi, $usuaCodi, $comentarioRol);
        }
    ...
    function crearPDF($idSolicitud, $pdf, $rutaRaiz, $ABSOL_PATH) {
        ...
        $sql = "select grupo, item, resolucion_inicial, fecha_acto, razon_social, presenta_recuro, 
                    ...
                    depe_codi_pro, usua_codi_pro, depe_codi_rev, usua_codi_rev, observacion 
        ...
        if($dia == 1)
            $fecha= "Dada en Bogot&aacute; D.C., el " . $dia . " d&iacute;a del mes de " . $mes . " de " . $anho . ".";
        else
            $fecha= "Dada en Bogot&aacute; D.C., a los " . $dia . " d&iacute;as del mes de " . $mes . " de " . $anho . ".";
        ...
        $pdf->writeHTML($this->plantilla($resoInicial,$fechaActo, $razonSocial, $interpuso, $ultimoActo, 
            $fechaEjecutoria, $fecha, $proyecto, $reviso, $aprobo, $radicado, $observacion), true, false, true, false, '');
        $pdf_result = $pdf->Output($_SESSION['ABSOL_PATH']."/bodega/" . date('Y') . "/" . $rs->fields["DEPE_CODI"] . "/constancia/". $radicado . ".pdf", 'F');
    ...
    function verConstanciasGeneralNoti($inFechaInicio, $inFechaFinal, $seDependencia, $seGrupo, $estadoDife){
        ...
        while($rs && !$rs->EOF){
            ...
            if($rs->fields["DEPE_CODI_PRO"] != 0 && $rs->fields["USUA_CODI_PRO"] != 0)
                ...
                $fechaAnulacion = '';
                if($rs->fields["FECHA_ANULACION"] !== null) {
                    $fechaAnulacion = date('d/m/Y h:i:s', strtotime($rs->fields["FECHA_ANULACION"]));
                }

                $data[$i] = array($rs->fields["ID"], $rs->fields["FECHA_SOLICITUD"], $rs->fields["GRUPO"], $rs->fields["ITEM"], $nombreDependecia, $nombreFuncionario,
                $rs->fields["NO_ID_PLE"], $nombreEstado , $rs->fields["RESOLUCION_INICIAL"], $rs->fields["FECHA_ACTO"],
                $rs->fields["IDENTIFICACION"], $rs->fields["RAZON_SOCIAL"], $rs->fields["TIPO_NOTIFICACION"], $rs->fields["FECHA_ACUSE"],
                $rs->fields["PRESENTA_RECURO"], $rs->fields["RESO_APELACION"], $rs->fields["FECHA_APELACION"], $rs->fields["RESO_REPOSICION"],
                $rs->fields["FECHA_REPOSICION"], $rs->fields["RECURSO_QUEJA_REVOC"], $rs->fields["RESO_QUEJA_REVOC"], $rs->fields["TIPO_NOTIFICACION_FINAL"],
                $rs->fields["FECHA_NOTIFICACION_FINAL"], $rs->fields["EXPEDIENTE"], $rs->fields["FECHA_NOTIFICACION_ULTIMO"], $rs->fields["FECHA_EJECUTORIA"],
                $rs->fields["COMENTARIO"], $rs->fields["FECHA_NOTI_RESP"], $nombreAprobador, $rs->fields["OBSERVACION"], $rs->fields["NUMERO_CONSTANCIA"], $rs->fields["UBICACION"], $rs->fields["COMENTARIO_PROY"],
                $fechaAnulacion, $rs->fields["NOMBRE_ANULADOR"], $rs->fields["OBSERVACION_ANULACION"]);
                $i++;
                $rs->MoveNext();
        }
        return $data;                 
   ...
   /**
    ...
    function firmaDigital($idSolicitud, $ABSOL_PATH, $P12_FILE, $clave, $depeCodi, $usuaCodi, 
        ...
        $commandFirmado='java -jar '.$ABSOL_PATH.'/include/jsignpdf/JSignPdf.jar '.$ABSOL_PATH . $ubicacion .' -kst PKCS12 -ksf '.$P12_FILE.' -ka "superintendencia nacional de salud" -ksp '.$clave.' --font-size 7 -r \'Firmado al Radicar en SuperArgo\' -V -llx 0 -lly 0 -urx 550 -ury 27';
    ...
    function anexarExpediente($idSolicitud, $ABSOL_PATH, $digitosDep, $dependencia, $codUsuario, $usDoc) {
        ...
        for ($i = 0; $i < count($exp); $i++) {
            ...
            if($respAux->fields['COUNT'] > 0) {
                    ...
                    $anextipo = 7;
                    $size = '4645334';
                    $namefile = "CE " . $resoInicial . " " . $idSolicitud;
    ...
    function plantilla($resoInicial, $fechaActo, $razonSocial, $interpuso, $ultimoActo, 
                            $fechaEjecutoria, $fecha, $proyecto, $reviso, $aprobo, $radicado, $observacion){


        if($observacion != '') {
            $observacion = '<tr><td colspan="6"><span style="font-size:8px">Observación: ' .  $observacion . '</span></td></tr>';
        }

        return '<p style="text-align:center"><b>CONSTANCIA DE EJECUTORIA</b></p>

        <p style="text-align:center"><b>EL COORDINADOR(A) DEL GRUPO GESTI&Oacute;N DE NOTIFICACIONES Y<br> COMUNICACIONES DE LA DIRECCI&Oacute;N ADMINISTRATIVA DE LA<br> SUPERINTENDENCIA NACIONAL DE SALUD</b></p>
        
        <p style="text-align:justify">En ejercicio de las funciones consagradas en el numeral 4, artículo 11 de la Resolución 20218000013221-6 del 24 de septiembre de 2021, en concordancia con el artículo 5  de la Resolución 2025910000010505-6 de 2025,</p>
        
        <p style="text-align:center"><b>CERTIFICA QUE:</b></p>
        
        <p style="text-align:justify">El acto administrativo que se identifica a continuación quedó en firme de acuerdo con lo establecido en el art&iacute;culo 87 de la Ley 1437 del 18 de enero de 2011:</p>
        ...
        <table border="1" cellspacing="0" class="Table" style="border-collapse:collapse; border:solid windowtext 1.0pt; margin-left:3.75pt; width:100%">
            <tbody>
                <tr>
                    <td style="background-color:#c6e0b4;width:20%">
                    ...
                    <td style="background-color:#c6e0b4;width:16%">
                ...
                <tr>
                    <td style="text-align:center;width:20%"><span style="font-size:8px">' . $resoInicial . '</span></td>
                    <td style="text-align:center;width:16%"><span style="font-size:8px">' . $fechaActo . '</span></td>
                    <td style="text-align:center;width:16%"><span style="font-size:8px">' . $razonSocial . '</span></td>
                    <td style="text-align:center;width:16%"><span style="font-size:8px">' . $interpuso . '</span></td>
                    <td style="text-align:center;width:16%"><span style="font-size:8px">' . $ultimoActo . '</span></td>
                    <td style="text-align:center;width:16%"><span style="font-size:8px">' . $fechaEjecutoria . '</span></td>
                </tr>' . $observacion . '
            </tbody>
        </table>
        
        <p style="text-align:justify">Para la determinaci&oacute;n de la fecha de ejecutoria del acto administrativo, se tuvo en cuenta la informaci&oacute;n reportada por el &aacute;rea solicitante, mediante solicitud No. <b>' . $radicado . '</b> correspondientes a los soportes del acto y del trámite de notificaci&oacute;n de este, as&iacute; como si existi&oacute; la interposici&oacute;n de recursos o solicitudes de revocatoria directa.&nbsp; ' . $fecha . '</p>
        
        <p style="text-align:center"><em>Firmado electrónicamente por<br>        
            <b>SANDRA PATRICIA CAICEDO BARRERA</b></em></p>
        
        <p style="text-align:center"><b><em>Coordinadora Grupo Gesti&oacute;n de Notificaciones y Comunicaciones</em></b></p>
        
        <p><span style="font-size:8px"><em>Proyect&oacute;: ' . $proyecto . '<br />
        Revis&oacute;: ' . $reviso . '<br />
        Aprob&oacute;: ' . $aprobo . '</em></span></p>';
    }
    ...
    /**
     * Funcion que anula una constancia ejecutoria
     */    
    function anular($idSolicitud, $depeCodi, $usuaCodi, $observacion) {

        $sql = "select 	expediente, resolucion_inicial FROM public.sgd_ce_constancia where id = " . $idSolicitud;
        $rs = $this->db->query($sql);  
        $expediente = $rs->fields["EXPEDIENTE"];    
        $resolucionInicial =  "CE " . $rs->fields["RESOLUCION_INICIAL"]  . " " . $idSolicitud;       

        $nombreFuncionario = $this->obtenerNombreUsuario($usuaCodi, $depeCodi);
        $sql = "update public.sgd_ce_constancia set id_estado = 9, fecha_anulacion = (select now()),
            nombre_anulador = '" . $nombreFuncionario . "', observacion_anulacion = '"  .  $observacion . "',
            ubicacion = '' where id = " . $idSolicitud;
        $this->db->conn->execute($sql); 

        $exp = explode(",", $expediente); 
        for ($i = 0; $i < count($exp); $i++) {
            $sqlExc = "DELETE FROM sgd_exp_anexos where 
                exp_numero = '" . $exp[$i] . "' and exp_anex_desc = '" . $resolucionInicial . "'";
            $this->db->conn->execute($sqlExc);     
        }

        $this->agergarHistorial($idSolicitud, $depeCodi, $usuaCodi, 'Se elimina como anexo al o los expedientes: ' . $expediente);
        $this->agergarHistorial($idSolicitud, $depeCodi, $usuaCodi, 'Se anula constancia ejecutoria.');
    }
