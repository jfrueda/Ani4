   ...
   $( "#cbSolicitudGen" ).click(function() {
        ...
        $.ajax({url: "constanciaController.php", 
						...
						success: function(result){
                            if(result == 'f') {
                                ...
                                divInfo("No se pueden procesar la generación ya que algunas solicitudes no tienen la Fecha notificación último acto y/o Fecha ejecutoria.");
                            } else {
    ...
    $( "#btDevolver" ).click(function() {
        ...
        if($('#taComentarioDev').val() == ""){
            ...
            $.ajax({url: "constanciaController.php", 
					    type: "POST",
						data: {inFuncion : '16', arraySolicitud : arraySolicitudGen, comentario : $('#taComentarioDev').val()},
						success: function(result){
    ...
    $( "#icDescarga" ).click(function() {
        if(tab == 1)
            descargarSolicitudRevisor();
        /*if(tab == 2)
            descargarDevolucionRevisor();*/
        if(tab == 3)
            descargarConstanciaGeneralRevisor();
    });

    function descargarSolicitudRevisor(){
        limpiarDivInfo();
        if($("#inFechaInicio").val() != "" && $("#inFechaFinal").val() !="") {
            let parts = $("#inFechaInicio").val().split('-');
            let dtFechaInicio = new Date(parts[0], parts[1] - 1, parts[2]);
        ...
        $.ajax({url: "constanciaController.php", 
					    type: "POST",
						data: {inFuncion : '33', inFechaInicio : $("#inFechaInicio").val(), inFechaFinal : $("#inFechaFinal").val(), seDependencia : $("#seDependencia").val(), seGrupo : $("#seGrupo").val(), sePrinc : $("#sePrinc").val()},
						success: function(result){
                            ocultarLoading();
                            if(result == "200") {
                                window.location = '../bodega/tmp/workDir/revisorSolicitudes.xlsx';
                            } else {
                                alert("Ocurrió un error, comuníquese con el administrador del sistema.");
                            }
				  		},
