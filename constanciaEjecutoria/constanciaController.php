...
if($funcion == 2) { 
    ...
    $data = array (
        array($_POST['inIdple'], $_POST['inResolucionInicial'], $inFechaActo, $_POST['inNitCC'], trim($_POST['seRazonSocial']), 
        ...
        $_SESSION["dependencia"], $_SESSION["codusuario"], $_POST['taComentarioProy'])
    );
...
if($funcion == 3) {
    ...
    foreach ($sheetData as $t) {
        ...
        if($expAus === "") {
            ...
            $info = $info . "ID/PLE no es numérico " .  $t[1] . " Fila " . $contadorAux . "</br>";
        } else {
            ...
            foreach ($res as $k => $v) {
                if($k == "ok") {
                    ...
                    $t[10], $inFechaReposicion, $t[12], $t[13], $t[14], $inFechaRecursoQueja, $expAus, $_SESSION["dependencia"],  $_SESSION["codusuario"], $t[17]);
                    $i++;
                } else {
                    $info = $info . $v;
                }
...
if($funcion == 6) { 
    ...
    while($info && !$info->EOF){
        ...
        $data[$i] = array($info->fields["ID"], $info->fields["ALERTA"],
                        ...
                        $info->fields["EXPEDIENTE"], $info->fields["COMENTARIO"], $info->fields["COMENTARIO_PROY"],  $info->fields["FECHA_NOTIFICACION_ULTIMO"], $info->fields["FECHA_EJECUTORIA"]);
        $i++;
        $info->MoveNext();
    }
    echo json_encode($data);
...
if($funcion == 9) { 
    ...
    $data = array($_POST['idSolicitudEdit'], $_POST['inIdple'], $_POST['inResolucionInicial'], $inFechaActo, $_POST['inNitCC'], trim($_POST['seRazonSocial2']), 
        $_POST['seTipoNotifiacion'], $inFechaAcuse, $_POST['sePresentaRecurso'], $_POST['inResolucionApleacion'], $inFechaApelacion, $_POST['inResolucionReposicion'],
...
if($funcion == 13) {
    ...
    while($info && !$info->EOF){
        ...
        $data[$i] = array($info->fields["ID"], $info->fields["ALERTA"],
                        ...
                        $info->fields["OBSERVACION"], $info->fields["UBICACION"] , $info->fields["NUMERO_CONSTANCIA"], $info->fields["COMENTARIO_PROY"]);
        $i++;
        $info->MoveNext();
    }
    echo json_encode($data);        
...
if($funcion == 14) {
    ...
    if($editValido == true) {
        $data = array($_POST['idSolicitudEdit'], $_POST['inIdple'], $_POST['inResolucionInicial'], $inFechaActo, $_POST['inNitCC'], trim($_POST['seRazonSocial']), 
        $_POST['seTipoNotifiacion'], $inFechaAcuse, $_POST['sePresentaRecurso'], $_POST['inResolucionApleacion'], $inFechaApelacion, $_POST['inResolucionReposicion'],
        $inFechaReposicion, $_POST['seRecursoQueja'], $_POST['inResolucionRecurosQueja'], $_POST['seNotificacionFinal'], $inFechaRecursoQueja, $_POST['inExpediente'],
        $inFechaUltimoActo2, $inFechaEjecutoria2, $_POST['inObservacion']);
...
if($funcion == 17){
    ...
    foreach ($data as $idSolicitud) {
        $constancia->envioRolesNoti($idSolicitud, $_POST['rol'], $_SESSION["dependencia"], $_SESSION["codusuario"], $_POST['comentario']);
    }
    echo "200";
}

if($funcion == 18) {
    $pdf = new ConstanciaPDF('P', PDF_UNIT, 'LETTER', true, 'UTF-8', false, 3);
...
if($funcion == 23) {
    ...
    while($info && !$info->EOF){
        ...
        $data[$i] = array($info->fields["ID"], $info->fields["ALERTA"],
                        ...
                        $info->fields["RECURSO_QUEJA_REVOC"], $info->fields["RESO_QUEJA_REVOC"], $info->fields["TIPO_NOTIFICACION_FINAL"], $info->fields["FECHA_NOTIFICACION_FINAL"],
                        $info->fields["EXPEDIENTE"], $info->fields["COMENTARIO"], $info->fields["FECHA_NOTIFICACION_ULTIMO"], $info->fields["FECHA_EJECUTORIA"], 
                        $info->fields["OBSERVACION"], $info->fields["UBICACION"] , $info->fields["NUMERO_CONSTANCIA"], $info->fields["APROBACION"], $info->fields["COMENTARIO_PROY"]);
        $i++;
        $info->MoveNext();
    }
    echo json_encode($data);        
...
if($funcion == 31) {
    ...
    foreach ($data as $idSolicitud) {
        ...
        $constancia->firmaDigital($idSolicitud, $ABSOL_PATH, $P12_FILE, $clave, $_SESSION["dependencia"], $_SESSION["codusuario"], $tsUrlTimeStamp, $tsuUserTimeStamp, $tspPasswordTimeStamp);
...
if($funcion == 32) {
    ...
    while($info && !$info->EOF){
        ...
        $data[$i] = array($info->fields["ID"], date('d/m/Y h:i:s', $date), $info->fields["GRUPO"], $info->fields["ITEM"], $info->fields["NO_ID_PLE"], $info->fields["RESOLUCION_INICIAL"],
                        ...
                        $dateRestp, $info->fields["UBICACION"], $info->fields["COMENTARIO_PROY"]);
        $i++;
        $info->MoveNext();
    }
    echo json_encode($data);     
...
if($funcion == 33) {
    ...
    $data[0] = array("FECHA DE SOLICITUD","GRUPO","ITEM","DEPENDENCIA","USUARIO","NO ID_PLE","RESOLUCION INICIAL",
                       ...
                       "FECHA NOTIFICACION ULTIMO","FECHA EJECUTORIA", "OBSERVACION", "NUMERO DE CONSTANCIA", "COMENTARIO PROYECTOR");
    $i = 1;
    while($info && !$info->EOF){
        $nomDependencia = $constancia->obtenerNombreDependencia($info->fields["DEPE_CODI"]);
        $nomUsuario = $constancia->obtenerNombreUsuario($info->fields["USUA_CODI"], $info->fields["DEPE_CODI"]);
        ...
        $data[$i] = array(date('d/m/Y h:i:s', $date), $info->fields["GRUPO"], $info->fields["ITEM"], $nomDependencia, $nomUsuario, $info->fields["NO_ID_PLE"], "*". $info->fields["RESOLUCION_INICIAL"],
                        ...
                        $info->fields["PRESENTA_RECURO"], "*". $info->fields["RESO_APELACION"], $info->fields["FECHA_APELACION"], "*". $info->fields["RESO_REPOSICION"], $info->fields["FECHA_REPOSICION"],
                        $info->fields["RECURSO_QUEJA_REVOC"], "*". $info->fields["RESO_QUEJA_REVOC"], $info->fields["TIPO_NOTIFICACION_FINAL"], $info->fields["FECHA_NOTIFICACION_FINAL"],
                        $info->fields["EXPEDIENTE"], $info->fields["FECHA_NOTIFICACION_ULTIMO"], $info->fields["FECHA_EJECUTORIA"], $info->fields["OBSERVACION"], $info->fields["NUMERO_CONSTANCIA"], $info->fields["COMENTARIO_PROY"]);
        $i++;
        $info->MoveNext();
    }
...
if($funcion == 35) {
    $data =  $constancia->verConstanciasGeneralNoti($_POST['inFechaInicio'], $_POST['inFechaFinal'], 
        $_POST['seDependencia'], $_POST['seGrupo'], 20); 

    $dataHeader = array();
    $dataHeader[0] = array("ID", "FECHA DE SOLICITUD","GRUPO","ITEM","DEPENDENCIA","USUARIO","NO ID_PLE","ESTADO", "RESOLUCION INICIAL",
                        "FECHA ACTO","IDENTIFICACION","RAZON SOCIAL","TIPO DE NOTIFICACION","FECHA ACUSE","PRESENTA RECURSO","RESO APELACION","FECHA APELACION",
                        "RESO REPOSICION","FECHA REPOSICION","RECUROS QUEJA REVO","RESO QUEJA REVOC","TIPO NOTIFICAION FINAL","FECHA NOTIFICACION FINAL","EXPEDIENTE",
                        "FECHA NOTIFICACION ULTIMO","FECHA EJECUTORIA", "COMENTARIO", "FECHA DE RESPUESTA", "APROBADOR", "OBSERVACION","NUMERO DE CONSTANCIA", "UBICACION", "COMENTARIO PROYECTOR",
                        "FECHA ANULACION","NOMBRE ANULADOR", "OBSERVACION ANULACION");
...
if($funcion == 36) {
    ...
    $data[0] = array("FECHA DE SOLICITUD","GRUPO","ITEM","DEPENDENCIA","USUARIO","NO ID_PLE","RESOLUCION INICIAL",
                        "FECHA ACTO","IDENTIFICACION","RAZON SOCIAL","TIPO DE NOTIFICACION","FECHA ACUSE","PRESENTA RECURSO","RESO APELACION","FECHA APELACION",
                        "RESO REPOSICION","FECHA REPOSICION","RECUROS QUEJA REVO","RESO QUEJA REVOC","TIPO NOTIFICAION FINAL","FECHA NOTIFICACION FINAL","EXPEDIENTE",
                        "FECHA NOTIFICACION ULTIMO","FECHA EJECUTORIA", "FECHA DE RESPUESTA", "NUMERO DE CONSTANCIA", "OBSERVACION", "APROBACION", "COMENTARIO PROYECTOR"); 
    $i = 1;
    while($info && !$info->EOF){
        $nomDependencia = $constancia->obtenerNombreDependencia($info->fields["DEPE_CODI"]);
        $nomUsuario = $constancia->obtenerNombreUsuario($info->fields["USUA_CODI"], $info->fields["DEPE_CODI"]);
        ...
        $data[$i] = array(date('d/m/Y h:i:s', $date), $info->fields["GRUPO"], $info->fields["ITEM"], $nomDependencia, $nomUsuario, $info->fields["NO_ID_PLE"], $info->fields["RESOLUCION_INICIAL"],
                        ...
                        $info->fields["NUMERO_CONSTANCIA"], $info->fields["OBSERVACION"], $info->fields["APROBACION"], $info->fields["COMENTARIO_PROY"]);
        $i++;
        $info->MoveNext();
    }        
...
if($funcion == 40) { 
    $constancia->anular($_POST['idSolicitud'],
         $_SESSION["dependencia"], $_SESSION["codusuario"], $_POST['observacion']);
    echo "200";
}
