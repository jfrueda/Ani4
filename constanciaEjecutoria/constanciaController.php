...
if ($funcion == 2) {
    ...
    if ($inFechaActo === '31/12/1969') {
        $inFechaActo = '';
    }
    if ($inFechaAcuse === '31/12/1969') {
        $inFechaAcuse = '';
    }
    if ($inFechaApelacion === '31/12/1969') {
        $inFechaApelacion = '';
    }
    if ($inFechaReposicion === '31/12/1969') {
        $inFechaReposicion = '';
    }
    if ($inFechaRecursoQueja === '31/12/1969') {
        $inFechaRecursoQueja = '';
    }
    ...
    $data = array(
        array(
            $_POST['inIdple'],
            $_POST['inResolucionInicial'],
            $inFechaActo,
            $_POST['inNitCC'],
            trim($_POST['seRazonSocial']),
            $_POST['seTipoNotifiacion'],
            $inFechaAcuse,
            $_POST['sePresentaRecurso'],
            $_POST['inResolucionApleacion'],
            $inFechaApelacion,
            $_POST['inResolucionReposicion'],
            $inFechaReposicion,
            $_POST['seRecursoQueja'],
            $_POST['inResolucionRecurosQueja'],
            $_POST['seNotificacionFinal'],
            $inFechaRecursoQueja,
            trim($_POST['inExpediente']),
            $_SESSION["dependencia"],
            $_SESSION["codusuario"],
            $_POST['taComentarioProy']
        )
    );
...
if ($funcion == 4) {
    ...
    while ($info && !$info->EOF) {
        ...
        $data[$i] = array(
            ...
            $info->fields["GRUPO"],
            $info->fields["ITEM"],
            $info->fields["NO_ID_PLE"],
            $info->fields["RESOLUCION_INICIAL"],
            $info->fields["FECHA_ACTO"],
            $info->fields["IDENTIFICACION"],
            $info->fields["RAZON_SOCIAL"],
            $info->fields["TIPO_NOTIFICACION"],
            $info->fields["FECHA_ACUSE"],
            $info->fields["PRESENTA_RECURO"],
            $info->fields["RESO_APELACION"],
            $info->fields["FECHA_APELACION"],
            $info->fields["RESO_REPOSICION"],
            $info->fields["FECHA_REPOSICION"],
            $info->fields["RECURSO_QUEJA_REVOC"],
            $info->fields["RESO_QUEJA_REVOC"],
            $info->fields["TIPO_NOTIFICACION_FINAL"],
            $info->fields["FECHA_NOTIFICACION_FINAL"],
            $info->fields["EXPEDIENTE"],
...
if ($funcion == 6) {
    ...
    while ($info && !$info->EOF) {
        ...
        $data[$i] = array(
            ...
            $info->fields["FECHA_NOTIFICACION_FINAL"],
            $info->fields["EXPEDIENTE"],
            $info->fields["COMENTARIO"],
            $info->fields["COMENTARIO_PROY"],
            $info->fields["FECHA_NOTIFICACION_ULTIMO"],
            $info->fields["FECHA_EJECUTORIA"]
        );
        $i++;
        $info->MoveNext();
    }
...
if ($funcion == 9) {
    ...
    $data = array(
        $_POST['idSolicitudEdit'],
        $_POST['inIdple'],
        $_POST['inResolucionInicial'],
        $inFechaActo,
        $_POST['inNitCC'],
        trim($_POST['seRazonSocial2']),
        $_POST['seTipoNotifiacion'],
        $inFechaAcuse,
        $_POST['sePresentaRecurso'],
        $_POST['inResolucionApleacion'],
...
if ($funcion == 13) {
    ...
    if ($_POST['sePrinc'] == 1) {
        $info =  $constancia->verSolicitudRolRevisor(
            $_POST['inFechaInicio'],
            $_POST['inFechaFinal'],
            $_POST['seDependencia'],
            $_POST['seGrupo']
        ...
        $info =  $constancia->verSolicitudRolRevisorDevuelta(
            $_POST['inFechaInicio'],
            $_POST['inFechaFinal'],
            $_POST['seDependencia'],
            $_POST['seGrupo']
    ...
    while ($info && !$info->EOF) {
        ...
        $data[$i] = array(
            ...
            $info->fields["FECHA_NOTIFICACION_ULTIMO"],
            $info->fields["FECHA_EJECUTORIA"],
            $info->fields["OBSERVACION"],
            $info->fields["UBICACION"],
            $info->fields["NUMERO_CONSTANCIA"],
            $info->fields["COMENTARIO_PROY"]
        );
        $i++;
        $info->MoveNext();
...
if ($funcion == 14) {
    ...
    if ($inFechaUltimoActo2 === '31/12/1969') {
        $inFechaUltimoActo2 = '';
    }
    if ($inFechaEjecutoria2 === '31/12/1969') {
        $inFechaEjecutoria2 = '';
    }

    $editValido = false;
    $infoInvalido = '';
    $res =  $constancia->evaluarExpediente(
        $_POST['inResolucionInicial'],
        trim($_POST['inExpediente'])
    );
...
if ($funcion == 20) {
    $info =  $constancia->verSolicitudDevueltaGeneral(
        $_POST['inFechaInicio'],
        $_POST['inFechaFinal'],
        $_POST['seDependencia'],
        $_POST['seGrupo']
...
if ($funcion == 21) {
    $info =  $constancia->verConstanciasGeneralNoti(
        $_POST['inFechaInicio'],
        $_POST['inFechaFinal'],
        $_POST['seDependencia'],
        $_POST['seGrupo'],
...
if ($funcion == 23) {
    ...
    if ($_POST['sePrinc'] == 1) {
        $info =  $constancia->verSolicitudRolAprobador(
            $_POST['inFechaInicio'],
            $_POST['inFechaFinal'],
            $_POST['seDependencia'],
            $_POST['seGrupo']
        ...
        $info =  $constancia->verSolicitudRolAprobadorDevuelta(
            $_POST['inFechaInicio'],
            $_POST['inFechaFinal'],
            $_POST['seDependencia'],
            $_POST['seGrupo']
...
if ($funcion == 29) {
    $info =  $constancia->verSolicitudRolFirmante(
        $_POST['inFechaInicio'],
        $_POST['inFechaFinal'],
        $_POST['seDependencia'],
        $_POST['seGrupo']
...
if ($funcion == 31) {
    ...
    foreach ($data as $idSolicitud) {
        ...
        $constancia->anexarExpediente(
            $idSolicitud,
            $ABSOL_PATH,
            $_SESSION['digitosDependencia'],
            $_SESSION["dependencia"],
            $_SESSION["codusuario"],
            $_SESSION["usua_doc"]
        );
...
if ($funcion == 33) {
    ...
    $data[0] = array(
        ...
        "RECUROS QUEJA REVO",
        "RESO QUEJA REVOC",
        "TIPO NOTIFICAION FINAL",
        "FECHA NOTIFICACION FINAL",
        "EXPEDIENTE",
        "FECHA NOTIFICACION ULTIMO",
        "FECHA EJECUTORIA",
        "OBSERVACION",
        "NUMERO DE CONSTANCIA",
        "COMENTARIO PROYECTOR"
    );
    $i = 1;
    while ($info && !$info->EOF) {
        $nomDependencia = $constancia->obtenerNombreDependencia($info->fields["DEPE_CODI"]);
...
if ($funcion == 34) {
    ...
    $data =  $constancia->verSolicitudDevueltaGeneral(
        $_POST['inFechaInicio'],
        $_POST['inFechaFinal'],
        $_POST['seDependencia'],
        $_POST['seGrupo']
...
if ($funcion == 35) {
    $data =  $constancia->verConstanciasGeneralNoti(
        $_POST['inFechaInicio'],
        $_POST['inFechaFinal'],
        $_POST['seDependencia'],
        $_POST['seGrupo'],
...
if ($funcion == 37) {
    $data =  $constancia->evaluarExpediente(
        $_POST['resolucion'],
        trim($_POST['inExpediente'])
    );
    echo json_encode($data);
