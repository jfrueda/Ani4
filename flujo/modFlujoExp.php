...
<?
 $verradEntra=$verrad;
  ...
  include_once ("$ruta_raiz/process/class/Flujo.php");
...
//			$ordenFlujo = $rs->fields["SGD_FEXP_ORDEN"];
			...
			?>
			<option value=<?=$value?>-<?=$aristaS?>  <?=$datoss?>> <?=$codigoEtapa?> - <?=$nombre_flujo?> -><?=$desc_flujo?> </option>
			<?
			$k++;
		}
	?>
...
if($grabarFlujo)
{
  ...
  if(!$ddca_causal)
	...
	list ($estadoNuevo, $aristaActual) = explode('-', $flujo);
