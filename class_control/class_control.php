...
function CONTROL_ORFEO(& $db)
{
	...
	$this->digitosSecRad = intval($this->db->conn->getOne('SELECT conf_valor FROM sgd_config WHERE conf_nombre = \'digitosSecRad\'')) ?? 7;
	$this->digitosDependencia = intval($this->db->conn->getOne('SELECT conf_valor FROM sgd_config WHERE conf_nombre = \'digitosDependencia\'')) ?? 5;
}
...
function consulta_per_arch($nombusuario){
	...
	function consulta_exp($radicado)
	{
	 ...
	 {
                $sqlbe="select sgd_cerrado from sgd_sexp_secexpedientes where  SGD_EXP_NUMERO = '$expediente'";
		$rsb=$this->cursor->conn->Execute($sqlbe);
                if($rsb->fields['SGD_CERRADO']==0 or $rsb->fields['SGD_CERRADO']==''){
			$estado_expediente =0;
			...
			if ($rs==false){
                    ...
                    echo '<br>Lo siento no se pudo agregar el raidcado al expediente se encuentra cerrado<br>';
                }
   }
   /**
    * Fecha de modificacion: 29-Junio-2006
    ...
    */
	 function modificar_expediente($radicado,$num_expediente,$exp_titulo,$exp_asunto,$exp_item1,$exp_item2,$exp_caja,$exp_estante,$exp_carpeta,$exp_subexpediente,$exp_archivo,$exp_unicon,$exp_fechaIni,$exp_fechaFin,$exp_folio,$exp_rete,$exp_entrepa,$exp_edificio,$usua_arch,$carro)
	 {
	    ...
	    if($exp_asunto)$record["SGD_EXP_ASUNTO"]="".$exp_asunto."";
	    if($exp_item1)$record["SGD_EXP_UFISICA"]="".$exp_item1."";
	    if($exp_item2)$record["SGD_EXP_ISLA"]="".$exp_item2."";
	    if($exp_caja) $record["SGD_EXP_CAJA"]="".$exp_caja."";
	    $record["SGD_EXP_ESTANTE"]="'".$exp_estante."'";
	    $record["SGD_EXP_CARPETA"]="'".$exp_carpeta."'";
	    $record["SGD_EXP_ESTADO"]='1';
	    $fechaHoy = $this->db->conn->OffsetDate(0,$this->db->conn->sysTimeStamp);
	    if($this->db->driver=="postgres") $fechaHoy = "now()";
	    if($this->db->driver=="oci8") $fechaHoy = "sysdate";
	    $record["SGD_EXP_FECH_ARCH"]=$fechaHoy;
	    if(trim($exp_archivo))  $record["SGD_EXP_ARCHIVO"]="".$exp_archivo."";
	    if($exp_unicon)   $record["SGD_EXP_UNICON"]="".$exp_unicon."";
	    if($exp_fechaIni) $record["SGD_EXP_FECH"]="'".$exp_fechaIni."'";
	    ...
	    $record1["RADI_NUME_RADI"] = $radicado;
            ...
            if($exp_subexpediente) $record["SGD_EXP_SUBEXPEDIENTE"]="'".$exp_subexpediente."'";
            if($exp_rete)$record["SGD_EXP_RETE"]="".$exp_rete."";
            if($exp_entrepa) $record["SGD_EXP_ENTREPA"]="'".$exp_entrepa."'";
            if($usua_arch)$record["RADI_USUA_ARCH"]="'".$usua_arch."'";
            if($carro) $record["SGD_EXP_CARRO"]="'".$carro."'";
...
function radicar_salida($tipoanexo,$cuentai,$documento_us3 ,$med ,$fec
					,$radicadopadre,$codusuario,$tip_doc ,$ane,$pais
					,$asu,$coddepe,$carp_codi,$tip_rem,$numdoc,$tdoc
					,$muni_codi,$archivo,$usua_doc,$depe_codi_territorial){

						$sec=$this->cursor->nextId("sec_$depe_codi_territorial");
						if ($sec!=-1)
						{	
							$depe_part = str_pad($coddepe,$this->digitosDependencia, "0",STR_PAD_LEFT);
							$sec_part = str_pad($sec, $this->digitosSecRad, "0", STR_PAD_LEFT);
							$nurad = date("Y") . $depe_part . $sec_part ."1";
							$carp_codi = 5;
							echo '<pre>'.	$query="insert into radicado (    radi_tipo_deri
								, radi_cuentai
								, eesp_codi
								, mrec_codi
								, radi_fech_ofic
...
function radicar_salida_masiva($tipoanexo,$cuentai,$documento_us3 
							 ,$med ,$fec,$radicadopadre,$codusuario
							 ,$tip_doc ,$ane,$pais,$asu,$coddepe
							 ,$tip_rem,$numdoc,$tdoc,$muni_codi
							 ,$archivo,$usua_doc,$depe_codi_territorial
                             ,$secRadicacion,$numeroExpediente='',$pais_codi
                             ,$dpto_codi
                             ,$cont_codi){
...
function grabar_usuario($no_documento,$nombre_us1,$direccion_us1,$prim_apell_us1,$seg_apell_us1,$telefono_us1,$mail_nus1,$muni_codi,$dpto_codi,$pais_codi,$cont_codi)
{
		...
		$query = "INSERT INTO SGD_CIU_CIUDADANO(SGD_CIU_CEDULA, TDID_CODI, SGD_CIU_CODIGO, ".
				...
				"'$telefono_us1','$mail_us1' ,$cont_codi,$pais_codi,$dpto_codi,$muni_codi)";
			$rs=$this->cursor->conn->Execute($query);

			if (!$rs){
				$this->cursor->conn->RollbackTrans();
				die ("<span class='etextomenu'>No se ha podido insertar la informacion del nuevo usuario ($query)");
			}
	...
	return $nextval;
	 ...
	 // Fin de funcion que consulta envio de radicado.
       ...
       /**
          ...
          *Extrae los datos de la modicion Fisica de los documentos.
          ...
          *
          * @param $itemHijo Numeric  Item del cual se inicia la recuperacion de los datos.
    */
