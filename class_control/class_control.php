...
class CONTROL_ORFEO
{
	...
	function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc)
	{
		...
		if ($rsb->fields['SGD_CERRADO'] == 0 || $rsb->fields['SGD_CERRADO'] == '') {
			$estado_expediente = 0;
	...
	function radicar_salida_masiva(
		$tipoanexo,
		$cuentai,
		$documento_us3,
		$med,
		...
		if ($jh_db) {
			...
			while ($this->cursor->next_record()) {
				...
				foreach ($this->campos_tabla as $campos_d) {
					...
					if (!$this->cursor->f($campos_d)) $dato = "-";
					else $dato = substr($this->cursor->f($campos_d), 0, $width_max);
