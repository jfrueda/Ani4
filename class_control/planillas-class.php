...
class panillasClass
{
	...
	function generarSipos()
	{
		...
		if (is_writable($nombre_archivo2)) {
			...
			$tmp = chr(255) . chr(254) . mb_convert_encoding($contenidotxt1 . $contenidotxt, 'UTF-8');

			$tmp = $contenidotxt1 . $contenidotxt;
			$tmp = mb_convert_encoding($contenidotxt1 . $contenidotxt, 'UTF-16LE', 'UTF-8');
	...
	function limpiarDato($dato)
	{
		...
		$dato = str_replace($tildes, $sinTildes, $dato);
