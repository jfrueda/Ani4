...
// Bloque de atributos que corresponden a los campos de la tabla anexos
   ...
   * @return   string
   */
  function obtenerMaximoNumeroAnexo(int $radicacion)
  {
    $this->cursor->conn->SetFetchMode(ADODB_FETCH_ASSOC);
    ...
    while ($sw == 0) {
      ...
      $isql = "SELECT 
                        ...
                        FROM ANEXOS
                    WHERE 
                        ANEX_RADI_NUME =" . (empty($radicacion) ? 0 : $radicacion) . " AND
  ...
  function anexarFilaRadicado($codMax = 0)
  {
    ...
    $codigoTipoAnexo = $this->obtenerTipoExtension(strtolower($this->anex_nomb_archivo));
