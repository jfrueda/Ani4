			...
			<tr>
				<td class="titulos2">
					...
					if (($grabar_causal == "yes") && (($causal_new != $causal_grb) || ($deta_causal != $deta_causal_grb) || ($ddca_causal != $ddca_causal_grb))) {
						...
						if ($causal_new != null) {
							$recordSet["SGD_DCAU_CODIGO"] = "$causal_new";
						}
						if ($ddca_causal != null) {
							$recordSet["SGD_DDCA_DDSGRGDO"] = "$ddca_causal";
						}
						if ($deta_causal != null) {
							$recordSet["SGD_DDCA_CODIGO"] = "$deta_causal";
						}
						...
						if (isset($actualizo) && ($actualizo > 0)) {
							...
							$actualizoFlag = true;
						} elseif (!isset($actualizo) || ($actualizo == 0)) {
