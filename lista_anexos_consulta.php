...
$queryPadre = "select ANEX_RADI_NUME
                from ANEXOS
                where RADI_NUME_SALIDA IN( $verrad ) OR ANEX_RADI_NUME IN ( $verrad ) LIMIT 1";

$rsPadre = $db->conn->query($queryPadre);

if ($rsPadre) {
    $verrad = $rsPadre->fields["ANEX_RADI_NUME"];
}
...
<style type="text/css">
    ...
    .form-pqrs-contInt input:invalid,
    select:invalid,
    textarea:invalid {
        border: 1px solid red;
    }

    .form-pqrs-contInt input[type="text"]:valid,
    select:valid,
    textarea:valid {
                                    ...
                                    <td>
                                        <font size=1>
                                            ...
                                            if ($origen != 1 && $linkarchivo  && $verradPermisos == "Full") {
                                                ...
                                                if ($no_esta_enviado && $anexEnvEmail != 1) {
...
<style type="text/css">
    ...
    @-webkit-keyframes mostrarAnimadoMensaje {
        from {
            //height:0px; background:rgba(255, 255, 255, 0.1);} to{//height:800px; background:rgba(255, 255, 255, 1);}}

            @-webkit-keyframes mostrarAnimadoMensaje {
                from {
                    //height:0px; background:rgba(255, 255, 255, 0.1);} to{//height:800px; background:rgba(255, 255, 255, 1);}}

                    @keyframes cerrarAnimadoMensaje {
                        from {
