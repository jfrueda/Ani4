...
<script src="../js/hideURL.js"></script>
...
while(!$rs->EOF)
{
  ...
  $pathImagen = str_replace("\\","/",$pathImagen);
    ...
    $val = array($radEnviado);
    $visto = $db->conn->GetOne($sql, $val);
    if ($visto >= 3) {
        $verImagen = '<a href="#" onclick="alert(\'Por favor comuniquese con la entidad para poder realizar la descarga\')">Ver Imagen</a>';
    } else {

     $verImagen = "<a target='_blank' href='ver_anexo.php?rad=$radEnviado'>Ver Imagen</a>";}
	}
	$pathImagen = "";
	if($radDev)
	{
