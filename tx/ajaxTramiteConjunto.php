...
$nombTx        = "Envio tramite conjunto";
if($enviarMailInformados==1) $sendMail = true; else $sendMail = false;

if(is_array($usuarios)){
   while (list(,$var)=each($usuarios)) {
       $data     = explode("_", $var);
       $result[] = $tx->tramiteConjunto($radicado, $krd, $data[0], $dependencia, $data[1], $codusuario, $observa, $usua_doc,'', $trGrupo,$sendMail);
   }
}

if(!empty($result)){
    if ($result[0] === "YA_INFORMADO") {
        echo json_encode(array("false" => "Este usuario ya está informado"));
    } elseif ($result[0] === "YA_DESTINATARIO") {
        echo json_encode(array("false" => "El usuario es un destinatario"));
    } else {
        echo json_encode(array("true" => "Se informo a los usuarios seleccionados $result[0]"));
    }
}