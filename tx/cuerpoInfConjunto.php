...
if($_GET["filtroSelec"]) $filtroSelec=$_GET["filtroSelec"];
...
if($busq_radicados)
{
	...
	{	$item = trim ( $item );
		...
		{	if(strlen($item)<=6)
			...
			{}
			$busq_radicados_tmp .= " (cast(a.radi_nume_radi as varchar(14)) like '%$item%') or";
		}
	}
	...
	<td>
		...
		<TR>
			<td width='33%' >
      			...
      			<tr class="info">
      				<td height="20">Tramite Conjunto</td>
      			</tr>
      			</table>
      		</td>
			<td width='33%' >
		...
		<tr class="tablas">
			<TD>
			<FORM name=form_busq_rad action='./cuerpoInfConjunto.php?krd=<?=$krd?>&<?=session_name()."=".trim(session_id()).$encabezado?>' method=GET>
				Buscar radicado(s) de Tramite Conjunto(s) (Separados por coma)<input name="busq_radicados" type="text" size="70" class="tex_area" value="<?=$busq_radicados?>">
		...
		<form name='form1' action='<?=$ruta_raiz?>/tx/formEnvio.php' method='GET'>
	    <input type='hidden' name='<?=session_name()?>' value='<?=session_id()?>'> 
...
?>
					<span class='etitulos'><b>
						...
						<option value=7>Borrar Documento De Tramite Conjunto</option>
...
<?
include "$ruta_raiz/include/query/tx/queryCuerpoInfConjunto.php";
$a = new combo($db);
