...
if (isset($_POST['depuser'])) {
    ...
    $hist->insertarHistorico([$_POST['radicados']],
        ...
        103);
    $texto = 'Estimado usuario, le informamos que se ha radicado a trav&eacute;s de superargo el documento *RAD_S*, <span style="font-size:130%;font-weight:bold;background-color:yellow;">que contiene un enlace de descarga que vence en 24 horas.</span><br><br>Por lo expuesto, se recomienda revisar el radicado y efectuar la descarga respectiva para no perder la informaci&oacute;n all&iacute; contenida.';
...
if (($codTx == 9 && $depsel == 93001) || $codTx == 13) {
...
if($codTx==8 || $codTx==9 || $codTx==14  || $validarTRD){
 
    //Si soy jefe de grupo o tramitador y envio a mi misma dependencia y todo es entradas omito valicion de TRD
    $omititRegla = false;
    if($_SESSION["USUA_JEFE_DE_GRUPO"] == true || $_SESSION["USUA_TRAMITADOR"]==true) {
        if(($depsel == $dependencia || in_array($depsel, $arrayAdscritas)) &&  $validarTRDJefe == false) {
            $omititRegla = true;
        }
    }

    /**Permite omitit regla si es una entrada y es una jefe de grupo o un tramitador */
    if( !$omititRegla) {
        if($_SESSION["USUA_JEFE_DE_GRUPO"] == true || $_SESSION["USUA_TRAMITADOR"]==true) {
            if($validarTRDJefe == false) {
                $omititRegla = true;
            }
        }    
    }
...
if($checkValue) {
    ...
    while (list($recordid,$tmp) = each($checkValue)){
        ...
        case  9:
            ...
            if((
                $indicaBorrador > 3000 && 
                ($indicadorTipoRadi >= 4 and $indicadorTipoRadi <= 7)) 
                || (
                    ($_SESSION["USUA_JEFE_DE_GRUPO"] == true || $_SESSION["USUA_TRAMITADOR"]== true) 
                    && ($indicadorTipoRadi < 4)
                    
                )
                && in_array($indicadorTipoRadi, array(1,2,3)) && $_SESSION["USUA_JEFE_DE_GRUPO"] != true
            ) {
        ...
        case 12:
            if($codTx == 12){
                ...
                $usuInUsuCod=$UsuIn->fields["USUA_CODI"];
                $usuInDepeCod=$UsuIn->fields["DEPE_CODI"];

                $pasaFiltro3 = "";
                if($usuInAct == '' && substr($record_id, -1) == ENTRADA){
                    $pasaFiltro2 = "No";
                    $pasaFiltro3 = "Si";//Para mostrar mensaje de envio automatico
                    $isqlJefeArea = "SELECT d.depe_codi, u.usua_codi FROM usuario u
                        ...
                        WHERE gr.id = 2 and u.depe_codi = 93004";
                    $jefeArea  = $db->query($isqlJefeArea);  
                    $usuJfUsuCod=$jefeArea->fields["USUA_CODI"];
                    $usuJfDepeCod=$jefeArea->fields["DEPE_CODI"];                

                    $radicadosSel[0] = $record_id;  
                    $tx->reasignar( $radicadosSel, $krd, $usuJfDepeCod, $dependencia, $usuJfUsuCod, $codusuario, "si", "Se realiza devolución al Cordinador actual del Grupo de Correspondencia para su respectiva reasignación", 12, 0);                  
                } elseif($usuaEsta != 1 || $usuInAct === 'ADMON') {
                    $pasaFiltro2 = "No";
                } else {
                    $pasaFiltro2 = "Si";
                }
        ...
        case 13:
        {
            ...
            if (($codTx == 9   and (!$esjefe and $codusuario != $usuarioEnrutador)) or $codTx == 16 or $codTx == 13){
                $aux_tiene_exp = false;
        ...
        case 16:
        {
             ...
             */
            $anoRad = substr($record_id,0,4);
            $tiporadicado = substr($record_id, -1);
            ...
            if (!in_array($anoRad, ["2005", "2004", "2003"]) && strlen(trim($radiNumero)) === 0) {
                $pasaFiltro = "No";
                $setFiltroSinTRD .= $record_id;
                if ($i <= $num) {
                    $setFiltroSinTRD .= ",";
	...
	if($setFiltroSinTRD && $pasaFiltro=="No" && !$omititRegla)
	{
	...
	if ($pasaFiltro2 == "No") {
        if ($pasaFiltro3 == "Si") {
            $mensaje_error = "SE ENVÍA AUTOMATICAMENTE AL JEFE DE CORRESPONDENCIA POR NO SER DE TU COMPETENCIA Y PARA SU RESPECTIVA REASIGNACIÓN";
        } else {
            if($usuInAct != '') {
                if($usuInAct == 'ADMON')
                    $mensaje_error = "NO SE PERMITE ESTA OPERACION YA QUE VIENE DEL ADMINISTRADOR";
                else
                    $mensaje_error = "NO SE PERMITE ESTA OPERACION El USUARIO <BR> < $usuInAct > <BR> SE ENCUENTRA INACTIVO.";
            } else {
                $mensaje_error = "NO SE PERMITE ESTA OPERACION YA QUE EL USUARIO ANTERIOR NO SE PUEDE ENCONTRAR";
            }
        }
	}

	if ( $setFiltroSinEXP &&  !$omititRegla) {
...
$borrVal = implode(",",$radBorr);
$sqlValBorr = "SELECT is_borrador from radicado where radi_nume_radi in({$borrVal})";
$rs = $db->conn->execute($sqlValBorr);

foreach ($rs as $key => $value) 
{
	if ($value['IS_BORRADOR'] == 't' && $no_entrada && ($mensaje_errorEXP || $mensaje_error))
	{
			die("
			<div class='well well-sm well-light' style='opacity: 1; width=80%;'>
				<div class='alert alert-warning'>
	...
	else if($value['IS_BORRADOR'] == 't' && ($_GET['codTx'] == 13 || $_GET['codTx'] == 16))
	{
		// Obtén el nombre del host
		$urlCompleta = $_SERVER['REQUEST_URI'];
		// Divide el host en partes usando el punto como delimitador
		$parts = explode('/', $urlCompleta);
		// El subdominio estará en el primer elemento del array resultante
		$subdomain = $parts[1];
		echo "<div class=\"container mt-5\">
				<div class=\"alert alert-warning\" role=\"alert\">
					 <h4 class=\"alert-heading\">Finalización de radicados</h4>
					 <p>Los borradores no se pueden finalizar ni aplicar NRR(No requiere respuesta), estos se deben eliminar o gestionar.</p>
					 <hr>
				<p class=\"mb-0\">Por favor, intenta  regresar a la <a href=\"./../../{$subdomain}/cuerpo.php\">página de inicio</a>.</p>
					</div>
			</div>";
		return;
	}
	else if($value['IS_BORRADOR'] == 't' && ($_GET['AccionCaliope'] == 13 || $_GET['AccionCaliope'] == 16))
	{
		// Obtén el nombre del host
		$urlCompleta = $_SERVER['REQUEST_URI'];
		// Divide el host en partes usando el punto como delimitador
		...
		echo "<div class=\"container mt-5\">
			<div class=\"alert alert-warning\" role=\"alert\">
				 ...
				 <p>Los borradores no se pueden finalizar ni aplicar NRR(No requiere respuesta), estos se deben eliminar o gestionar.</p>
				 <hr>
				<p class=\"mb-0\">Por favor, intenta  regresar a la <a href=\"./../../{$subdomain}/verradicado.php?verrad={$borrVal}\">página de inicio</a>.</p>
				</div>
			</div>";
		return;
	}
...
<script>
    ...
    function okTx() {
        $("#REALIZAR").prop( "disabled", true );
        valCheck = Anular(0);
        if(valCheck==0) {
            $("#REALIZAR").prop( "disabled", false );
            return 0;
        }
    ...
    function informarUsuario(arrRadicados,dependencia,codigoUsuario,comentario){
        $.post("../tx/ajaxInformarUs.php", { 
            arrRadicados: arrRadicados, 
        ...
        .done(function(data) {
            if (data && data.true) {
                // Solo mostrar el alert si ya estaba informado
                $("#msgInformar").html(
                    "<div class='alert alert-success fade in'>" +
                    "<button class='close' data-dismiss='alert'> × </button>" +
                    "<i class='fa-fw fa fa-check'></i>" +
                    "<strong>" + data.true + "</strong>" +
                    "</div>"
                );
            } else if (data && data.false) {
                // Mostrar error si data.false está presente
                $("#msgInformar").html(
                    "<div class='alert alert-danger fade in'>" +
                    "<button class='close' data-dismiss='alert'> × </button>" +
                    "<i class='fa-fw fa fa-times'></i>" +
                    "<strong>" + data.false + "</strong>" +
                    "</div>"
                );
            } else {
                // Limpiar el mensaje si no aplica
                $("#msgInformar").html('');
            }
		...
		<tr bgcolor="White">
            ...
            <td colspan=3>
                ...
                <label class="textarea">
                    ...
                    <textarea name="observa" maxlength="2000" id="observa" placeholder="Escriba un Comentario" rows="3"></textarea>
                </label>
                <?php
