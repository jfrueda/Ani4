...
<script language="javascript">
  ...
  function clickTx() {
    ...
    if (varNom == "Informados") {
      ...
      document.getElementById('lbDependenciaMsx').style.display = 'none';
      document.getElementById('lbDependenciaMsx').innerHTML = '';
      document.getElementById('depsel').style.display = 'none';
      $("#AccionCaliope").prop("selectedIndex", 0).val();
      ...
      if (sw == 0) {
        ...
        for (i = 0; i < document.form1.elements.length; i++) {
          if (document.form1.elements[i].name.includes("checkValue")) {
            if (document.form1.elements[i].checked) {
              let radicado = document.form1.elements[i].name.replace('checkValue[', '');
              radicado = radicado.replace(']', '');
              radicadoArray.push(radicado);
            }
          }
        }

        radicadoArray.forEach(element => {
          if (element.substring(0, 4) >= 3000) {
            canBorrador++;
          }
        });

        if (canBorrador > 0) {
          document.getElementById('lbDependenciaMsx').style.display = '';
          document.getElementById('lbDependenciaMsx').innerHTML = 'LOS BORRADORES  SELECCIONADOS NO SON VALIDOS PARA TRÁMITE';
        }
      }
    }
  }
    ...
    function changedepesel(enviara) {
      ...
      if (enviara == 19) {
        ...
        if ($("#dt_basic input:checked").length == 1 && (depe == 420 || depe == 410 || depe == 900) && entidad == 'CRA') {
          ...
          if (observaBorrarInf.trim().length > 4) {
            $.post("./tx/ajaxBorrarInformado.php", {
              "rads": rads,
              "observa": observaBorrarInf
            }).done(
              function(data) {
            ...
            alert("Debe escribir un motivo (al menos 5 caracteres) de eliminacion del Documento Informado....!");
          }
        }
      }
      //Borrar conjunto
      if (enviara == 101) {
        ...
        if ($("#dt_basic input:checked").length == 1 && (depe == 420 || depe == 410 || depe == 900) && entidad == 'CRA') {
          ...
          if (observaBorrarInf) {
            $.post("./tx/ajaxBorrarTramiteConjunto.php", {
              "rads": rads,
              "observa": observaBorrarInf
            }).done(
              function(data) {
...
// Revisar si existe la variable hasChkAnular
function esVerradicado()
{
  $backtrace = debug_backtrace();
  if (isset($backtrace[1]['file'])) {
    if (strpos($backtrace[1]['file'], 'verradicado.php') !== false) {
      return true;
    }
  }
  return false;
...
function puedeTramitar($depe_actu, $dependencia, $codusuario, $usuacodi)
{
  return (
    (isset($_SESSION["USUA_TRAMITADOR"]) && $_SESSION["USUA_TRAMITADOR"] == true && $depe_actu == $dependencia)
    ||
    ($codusuario == $usuacodi && $depe_actu == $dependencia)
  );
}
...
if ($mostrarMoverCarpeta) {
  $opciones_listado_accion .= '<option value="10">Mover a Carpeta...</option>';
}

$opciones_listado_accion .= '<option value="8" >a Informar...</option>
...
if (
  ...
  $verradPermisos == "Full" &&
  !$tieneAsignacion && !esVerradicado()
) {
  echo $opciones_listado_accion;
}
if (
  isset($verradPermisos) &&
  $verradPermisos == "Full" &&
  !$tieneAsignacion && esVerradicado() && puedeTramitar($depe_actu, $dependencia, $codusuario, $usuacodi)
) {
  echo $opciones_listado_accion;
}
echo $sCarpeta = isset($sCarpeta) ? $sCarpeta : '';
