...
/**
 ...
 /*
   ...
   if(empty($radicadosSel)){
     $whereFiltro = " and b.radi_nume_radi in($selectradicados)";
   }
...
switch ($codTx){
	...
	case 9:
	    ...
	    if(empty($radicadosSel))
	    {
	    	$radicadosSel = explode(',',$selectradicados);
    	}

		$usCodDestino = $rs->reasignarNoArchivado($radicadosSel, $krd, $depsel, $dependencia, $usCodSelect, $codusuario, $tomarNivel, $observa, $codTx, $carp_codi);

		$asignador_pqrd = new Asignador($db->conn);

		foreach($radicadosSel as $radicado) {
			if($asignador_pqrd->validarDependenciaRadicado($radicado, $depeRadicaFormularioWeb))
			{
				try {
					$documento = $db->conn->getOne('SELECT usua_doc FROM usuario WHERE depe_codi = ? AND usua_codi = ?', [$depsel, $usCodSelect]);
					$asignador_pqrd->borrarAsignaciones($radicado);
					$asignador_pqrd->asignarRadicado($radicado, $documento);
				} catch (UsuarioDesconocido $e) {
					error_log('No se pudo reasignar el radicado '.$radicado.' en el sistema PQRD');
				}
			}	
		}
	...
	case 16:
		...
		break;
	case 17:
		$nombTx = "Archivados en Fisico";
		$txSql = $rs->archifisico( $radicadosSel, $krd,$dependencia,$codusuario,$observa);
		break;
	case 18:
		$nombTx = "Cierre por Vencimiento";
		$txSql = $rs->cierre_exp( $expedienteSel, $krd,$dependencia,$codusuario,$observa);
		break;

	case 19:
		$nombTx = "Cierre por asunto Particular";
		$txSql = $rs->cierre_exp( $expedienteSel, $krd,$dependencia,$codusuario,$observa);
		break;

         //Hecho Juan carlos Villalba Villalba 16/03/2013
        //case 62:
		//$nombTx = "Abrir por asunto Particular";
		//$txSql = $rs->abrir_exp( $expedienteSel, $krd,$dependencia,$codusuario,$observa);
		//break;

	case 20:
		$nombTx = "Exluir en Fisico";
		$txSql = $rs->excluirfisico( $radicadosSel, $krd,$dependencia,$codusuario,$observa);
		break;
...
<table WIDTH=50%  class="table table-striped table-bordered table-hover dataTable" id=tb_general align="center">
	<tr>
	<td colspan="2" >ACCION REQUERIDA <?=$accionCompletada?>  <?=$okTxDesc?> COMPLETADA <?=$causaAccion ?> <?if ($_POST["codTx"]==17){?><div align="right"><a class="btn" href="../CRA.cuerpoarch.php">Continuar</a></div><?}?></td>
	</tr>
	<tr>
...
<?php
$_SESSION["enviarMailMovimientos"];
