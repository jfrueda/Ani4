...
$(function () {
    ...
    $('body').on('click', '.link_reporte', function (e) {
...
function busquedaValidacion() {
    let fromDateobj = Date.parse($('#fecha_ini').val());
    let toDateobj = Date.parse($('#fecha_fin').val());
    if (fromDateobj > toDateobj) {
        $('.INFOalert').html("La fecha 'Desde' no puede ser mayor que la fecha 'Hasta'");
        $('#resulEstdatos').hide();
        return false;
    } else {
        let futureDateobj = new Date(fromDateobj);
        futureDateobj.setMonth(futureDateobj.getMonth() + 3);
        futureDateobj = Date.parse(futureDateobj);
        if (futureDateobj < toDateobj && $('#tipoEstadistica').val() != 17) {
            $('.INFOalert').html("La búsqueda no puede tener una longitud de más de 3 meses.");
            $('#resulEstdatos').hide();
            return false;
        } else {
            $('.INFOalert').html("");
...
function reportExpediente() {
    var dependencia = $("#dependencia_busq").val();
    var adscritas = $("#CHKseldep").is(':checked') ? '1' : '0';
    var serie = $("#selSerie").val();
    var subserie = $("#selSubSerie").val();
    var fecha_inicial = $("#fecha_ini").val();
    var fecha_final = $("#fecha_fin").val();

    var request = $.get('reportes/reporte17_html.php');

    request.done(function (html) {
        $('#resultado').html(html);
        $('#reporte_17').DataTable({
            searching: false,
            processing: true,
            serverSide: true,
            scrollX: true,
            ajax: {
                url: 'reportes/reporte17_table.php',
                type: 'POST',
                data: {
                    dependencia,
                    adscritas,
            ...
            columns: [
                { title: 'Año', data: 'ANIO' },
                { title: 'Código Dependencia Antecesora de la creadora', data: 'CODIGO DEPENDENCIA ANTECESORA DE LA CREADORA' },
                { title: 'Nombre Dependencia Antecesora de la creadora', data: 'NOMBRE DEPENDENCIA ANTECESORA DE LA CREADORA' },
                { title: 'Código Dependencia creadora', data: 'CODIGO DEPENDENCIA CREADORA' },
                { title: 'Dependencia creadora', data: 'DEPENDENCIA CREADORA' },
                { title: 'Código Serie Documental', data: 'CODIGO SERIE DOCUMENTAL' },
                { title: 'Nombre Serie Documental', data: 'NOMBRE SERIE DOCUMENTAL' },
                { title: 'Código Subserie Documental', data: 'CODIGO SUBSERIE DOCUMENTAL' },
                { title: 'Nombre Subserie Documental', data: 'NOMBRE SUBSERIE DOCUMENTAL' },
                { title: 'Número de expediente', data: 'NUMERO DE EXPEDIENTE' },
                { title: 'Fecha de creación', data: 'FECHA DE CREACION' },
                { title: 'Nombre del expediente', data: 'NOMBRE DEL EXPEDIENTE' },
                { title: 'Responsable del expediente', data: 'RESPONSABLE DEL EXPEDIENTE' },
                { title: 'Creador del expediente', data: 'CREADOR DEL EXPEDIENTE' },
                { title: 'Estado', data: 'ESTADO' },
                { title: 'Fecha de cierre', data: 'FECHA DE CIERRE' }
            ],
            pageLength: 25
        });
        $('#generar').removeAttr('disabled');
        $('#reporte_17_export').on('click', function (e) {
            e.preventDefault();
            ...
            window.open('reportes/reporte17_export.php?' + params.toString(), '_blank');
        });

        $('#reporte_17_excel').on('click', function (e) {
            e.preventDefault();
            const params = new URLSearchParams({
                dependencia,
                adscritas,
            ...
            window.open('reportes/reporte17_excel.php?' + params.toString(), '_blank');
        });
    });
}
...
function report() {
    ...
    var tpUs = $("#CHKselUsuario").is(':checked') ? '1' : '0';
    datosbtnextra = "data-depe='" + depe + "'  data-tpRad='" + tpRad + "'  data-titulo='" + titulo + "' data-serie='" + serie + "' data-subserie='" + subserie + "' data-tpdoc='" + tpdoc + "' data-tpRad='" + tpRad + "' data-usua='" + usuA + "' data-tpads='" + tpAds + "' data-fini='" + fini + "' data-ffin='" + ffin + "'  data-tpUs='" + tpUs + "' ";
    //$('#animationload').show();
    ...
    axios({
        method: 'post',
        baseURL: 'rest-est.php',
        data: 'fn=rp&reporte=' + rp + '&depe=' + depe + '&serie=' + serie + '&subserie=' + subserie + '&tpdoc=' + tpdoc + '&usu=' + usuA + '&fini=' + fini + '&ffin=' + ffin + '&tpAds=' + tpAds + '&tpRad=' + tpRad + '&tpUs=' + tpUs
    })
        .then(function (response) {
            //   console.log(rp);
            total = 0;
            ...
            if (rp == 30) {
                ...
                htmls = '<div class="col-4 float-right"><div class="input-group input-group-sm">' +
                    '<div class="input-group-prepend"><span class="input-group-text tamCab text-left" id="basic-addon1">Total Radicados</span></div>' +
                    '  <input type="text" class="form-control" id="tb_total" readonly aria-describedby="basic-addon3"> <div class="input-group-append">' +
                    '<button class="btn btn-outline-primary btn-rp1det" data-rep="30" data-toggle="modal" data-target="DetEsta" data-btns=""  data-id="T"  type="button" data-toggle="tooltip"  data-placement="top"  title="Ver detalles Total"><i class="fa fa-align-justify"></i></button>' +
                    '<button class="btn btn-outline-success btn-rp1detXLS" type="button" data-rep="30" ' + datosbtnextra + '  data-tit="" data-btns="99999" data-id="T" ><i class="fa fa-table"></i></button></div><div id="detXLST" class="float-right"></div>' +
                    '</div></div>';
                ...
                $.each(data, function (index, value) {
                    ...
                    radi = value['RADICADOS'];
                    usua = value['USUARIO'];
                    btnA = '<button class="btn btn-xs btn-primary btn-rp1det" data-rep="30" data-toggle="modal" data-target="DetEsta" data-btns=' + depe + '  data-id=' + codigo2 + ' type="button" data-toggle="tooltip"  data-placement="top"  title="Ver detalles" ><i class="fa fa-align-justify"></i></button> ';
                    btnToS = '<button class="btn btn-xs btn-success btn-rp1detXLS" data-rep="30" data-toggle="modal" data-target="DetEsta"  data-tit="' + usua + '" data-btns="' + depe + '" ' + datosbtnextra + ' data-id=' + codigo2 + '  data-btns=2 type="button" ><i class="fa fa-table" data-toggle="tooltip"  data-placement="top"  title="Descargar detalles en excel"></i></button> <div id="detXLST" class="float-right"></div>';
                    $('#tb_rp').append('<tr> <td class = "text-right" > ' + numr + ' </td><td >' + usua + '</td><td class = "text-right">' + radi + '</td><td class="text-center" >' + btnA + btnToS + '</td></tr>');
                    total = total + parseInt(radi);
                });
...
function report14() {
    var request = $.get('reportes/reporte14.php');
            ...
            <div class="col-4 float-right">
                <div class="input-group input-group-sm">
                    <div class="input-group-prepend">
                        <span class="input-group-text tamCab text-left" id="basic-addon1">Total registros</span>
                    </div>  
                    <input type="text" class="form-control" id="tb_total" value="${data.length}" readonly="" aria-describedby="basic-addon3"> 
                    <div class="input-group-append">
                        <button class="btn btn-outline-success link_reporte" data-href="reportes/reporte14.php?formato=xls"><i class="fa fa-table"></i></button>
                    </div>
                    <div id="detXLST" class="float-right">
                    </div>
                </div>
            </div>
            <table id="tb_rp" class="table table-bordered table-striped table-hover">
...
function report9() {
    ...
    var ffin = $("#fecha_fin").val();
    var tpUs = $("#CHKselUsuario").is(':checked') ? '1' : '0';
    datosbtnextra = "data-depe='" + depe + "' data-serie='" + serie + "' data-subserie='" + subserie + "' data-tpdoc='" + tpdoc + "' data-tpRad='" + tpRad + "' data-usua='" + usuA + "' data-tpads='" + tpAds + "' data-fini='" + fini + "' data-ffin='" + ffin + "' data-btns=2 data-tpus='" + tpUs + "'";
    //$('#animationload').show();
    $('#processing-modal').modal('show');
    //console.log('fn=usuarios&depe='+depe+'&tpus='+tpUs);
    axios({
        ...
        .then(function (response) {
            ...
            switch (reporte) {
                ...
                case 3:
                    ...
                    htmls += '<thead class="thead-dark"><tr><th>#</th><th>Radicado Padre</th><th>Fecha de radicado</th><th>Radicado Hijo</th><th>Fecha de Radicado</th><th>E-mail certificado</th><th>E-mail no certificado</th><th>Fecha de envio</th>' +
                        '<th>Usuario que envio</th><th>Email Destinos</th><th>Direccion destinos</th><th>Planillas</th><th>Observacion</th>' +
                        '</tr></thead>';
                    htmls += '<tbody></tbody></table>';//<th>#</th>
                    $("#mdRespiues").html(htmls);
                    $('#processing-modal').modal('hide');
                    $(document).ready(function () {
                        $('#tb_rpDT').DataTable({
                            ...
                            "columns": [
                                ...
                                { "data": "NUM" }, { "data": "RADI_P" }, { "data": "RPFECHA" }, { "data": "RADI_SAL" }, { "data": "RHFECHA" },
                                { "data": "MEDIO_ENV_IN" }, { "data": "MEDIO_ENVIO_HIST" }, { "data": "FECH_ENV" }, { "data": "USUARIO" },
                                { "data": "EMAIL" }, { "data": "DIRECCION" }, { "data": "PLANILLA" }, { "data": "OBSERVACION" }
                            ],
                            'language': esvar, pageLength: 25,
                        });
                    });
                ...
                case 30:
                    $('#processing-modal').modal('hide');
                    htmls = '<table id="tb_rpDT" class="table table-sm table-bordered table-striped table-hover " style="font-size:10px">'
                    htmls += '<thead class="thead-dark"><tr><th>#</th><th>Borrador</th><th>Radicado</th><th>Estado</th><th>Fecha Radicacion</th> <th>Fecha Borrador</th><th>Tipo Documento</th><th>Asunto</th>' +
                        '<th>Usuario</th><th>Proyectado por</th><th>Remitente</th><th>Dignatario</th><th>Dependencia Inicial</th><th>Dependencia Actual</th><th>Usuario Actual</th>' +
                        '<th>Medio de Recpción</th><th>Departamento</th><th>Municipio</th><th>Referenciado o Radicado Padre</th><th>Correo electrónico</th><th>Fecha Digitalización</th><th>Usuario Digitalizador</th><th>Num. Folios</th>' + '</tr></thead>';
                    htmls += '<tbody></tbody></table>';//<th>#</th>
                    $('#processing-modal').modal('hide');
                    ...
                    $(document).ready(function () {
                        $('#tb_rpDT').DataTable({
                            data: data.data,
                            "columns": [
                                { "data": "NUM" }, { "data": "BORRA" }, { "data": "RADI" }, { "data": "ESTADO" }, { "data": "RFECH" }, { "data": "BFECH" }, { "data": "TPNOMB" }, { "data": "ASUNTO" }, { "data": "USUAR" }, { "data": "USUAR" },
                                { "data": "REM" }, { "data": "DIG" }, { "data": "DEPEI" }, { "data": "DPA" }, { "data": "USUAA" }, { "data": "MREC" }, { "data": "DPTO" }, { "data": "MUNI" }, { "data": "ASOCIADO" }, { "data": "EMAIL" }, { "data": "FECHADIG" }, { "data": "USUD" },
                                { "data": "FOL" }
                            ],
                            'language': esvar, pageLength: 10,
...
function report10() {
    ...
    var ffin = $("#fecha_fin").val();
    var tpUs = $("#CHKselUsuario").is(':checked') ? '1' : '0';
    datosbtnextra = "data-depe='" + depe + "' data-serie='" + serie + "' data-subserie='" + subserie + "' data-tpdoc='" + tpdoc + "' data-tpRad='" + tpRad + "' data-usua='" + usuA + "' data-tpads='" + tpAds + "' data-fini='" + fini + "' data-ffin='" + ffin + "' data-btns=1 data-tpus='" + tpUs + "' ";
    //$('#animationload').show();
    $('#processing-modal').modal('show');
    //console.log('fn=usuarios&depe='+depe+'&tpus='+tpUs);
    axios({
