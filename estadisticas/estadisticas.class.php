...
class estadisiticas
{
    ...
    public function rp1($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
        ...
        if ($usu && $usu != 0) {
            ...
            $whereU = " and b.usua_codi = $usu ";
        }

        $whereAct = '';
        if ($tpUs == 0) {
            $whereAct = " and b.usua_esta='1'";
        }
        ...
        $iSql = "
            SELECT
                ...
                MIN(b.USUA_CODI) AS COD_USU,
                MIN(b.depe_codi) AS DEPE_USUA,
                b.usua_doc
            FROM
                usuario b
            JOIN
                ...
                ) htev ON htev.usua_doc = b.usua_doc
            JOIN
                radicado r ON r.radi_nume_radi = htev.radi_nume_radi
            WHERE
                $where
                AND r.radi_fech_radi BETWEEN '$fini 00:00:00' AND '$ffin 23:59:59'
                AND (r.sgd_eanu_codigo NOT IN (1, 2) OR r.sgd_eanu_codigo IS NULL)
                $whereTipoRadicado
                $whereU
            GROUP BY
                b.USUA_NOMB, b.usua_doc;
        ";
    ...
    public function rp2($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
        if ($tpAds == 1) {
            $depe = $this->depahijas($depe);
        }
    ...
    public function rp3($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
        ...
        $iSql = "   select sgd_fenv_descrip nomb,m.sgd_fenv_codigo cod , count(2) num
                        from  SGD_RENV_REGENVIO b
                        $whereI
                        left join SGD_FENV_FRMENVIO m on b.sgd_fenv_codigo=m.sgd_fenv_codigo
                        --left join anexos a on a.radi_nume_salida=b.radi_nume_sal
                        --left join sgd_rad_envios renvio on renvio.id_anexo=a.id
                    where b.sgd_fenv_codigo=m.sgd_fenv_codigo and b.sgd_renv_fech BETWEEN ('$fini 00:00:00') and ('$ffin 23:59:59')
                    AND	(b.sgd_renv_planilla != '00' or b.sgd_renv_planilla is null)
        ...
        $Sql2 = "select m.sgd_fenv_codigo cod , count(2) dev
                        ...
                        $whereI
                        left join SGD_FENV_FRMENVIO m on b.sgd_fenv_codigo=m.sgd_fenv_codigo
                        left join hist_eventos h on h.radi_nume_radi=b.radi_nume_sal
                where  b.sgd_renv_fech BETWEEN  ('$fini 00:00:00') and ('$ffin 23:59:59')
                AND	(b.sgd_renv_planilla != '00' or b.sgd_renv_planilla is null) and h.radi_nume_radi=b.radi_nume_sal and sgd_ttr_codigo=28
                AND (m.sgd_fenv_codigo<>106 OR m.sgd_fenv_codigo=106 and radi_nume_grupo is null and h.hist_obse='Devolución E-mail (DOCUMENTO MAL RADICADO). Devolución por gestión de Rebote')
                group by m.sgd_fenv_codigo ";
    ...
    public function rp4($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
        ...
        AND r.radi_fech_radi between ('$fini 00:00:00') and ('$ffin 23:59:59') 
        $whereTipoRadicado  $whereUsu $whereAct
    ...
    public function rp6($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
        $whereTipoRadicado = $tpRad ? " and r.sgd_trad_codigo = $tpRad " : '';
    ...
    public function rp9($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpUs)
    {
        ...
        and r.radi_depe_actu = 999  $where2 $whereADD $whereUH $whereAct";
        $iSql2 = "select
        count(distinct r.radi_nume_radi) num
        from radicado r
        left join sgd_tpr_tpdcumento td on r.TDOC_CODI=td.SGD_TPR_CODIGO
    ...
    public function dtrp3($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpbusq)
    {
        ...
        $where2TB = '';
        $resp['depe'] = $depe;
        //$where = "and rh.radi_usua_radi=$tpbusq and rh.radi_depe_radi='$depe' ";
        if ($tpbusq == 'T') {
            $where = '';
            if ($depe != 99999) {
        ...
        if ($serie != 0) {
            $whereI .= "INNER JOIN  sgd_rdf_retdocf rdf on rdf.radi_nume_radi=b.radi_nume_sal
      INNER JOIN sgd_mrd_matrird mrd on mrd.sgd_mrd_codigo=rdf.sgd_mrd_codigo and  mrd.sgd_srd_id=$serie $ddsubserie";
        }
		...
		to_char(rh.radi_fech_radi, 'YYYY-MM-DD HH24:MI') rhfecha,
		--mc.mrec_desc medio_env_in,
		CASE
			WHEN renvio.certificado  then renvio.tipo 
			ELSE ''
		END medio_env_in,
		m.sgd_fenv_descrip medio_envio_hist,
		to_char( b.sgd_renv_fech, 'YYYY-MM-DD HH24:MI') fech_env,
		u.usua_login usuario,
		b.sgd_renv_mail email,
		b.sgd_renv_dir direccion,
		b.sgd_renv_planilla planilla,
		b.sgd_renv_observa observacion
	    from  SGD_RENV_REGENVIO b
		$whereI
		left join SGD_FENV_FRMENVIO m on b.sgd_fenv_codigo=m.sgd_fenv_codigo
		left join anexos a on a.radi_nume_salida=b.radi_nume_sal
		left join radicado rp on a.anex_radi_nume=rp.radi_nume_radi
		left join sgd_rad_envios renvio on renvio.id_anexo=a.id
		--left join medio_recepcion mc on mc.mrec_codi=rp.mrec_codi
		left join radicado rh on b.radi_nume_sal=rh.radi_nume_radi
		left join usuario u on u.usua_doc=b.usua_doc::varchar
	    where b.sgd_renv_fech BETWEEN ('$fini 00:00:00') and ('$ffin 23:59:59')
		AND b.usua_doc::varchar <> ''
	    AND	(b.sgd_renv_planilla != '00' or b.sgd_renv_planilla is null)
	    AND (b.sgd_renv_observa not like 'Masiva%' or  b.sgd_renv_observa is null) 
    ...
    public function dtrpCons($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpbusq, $tpRAD = 1)
    {
        ...
        if (!$rs || $rs->EOF) {
            return $datos; // Retorna el array vacío []
        }
        ...
        if (is_array($datoss) && !empty($datoss)) {
            foreach ($datoss as $key => $value) {
    ...
    public function rp30($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpRad, $tpUs)
    {
    ...
    public function dtrp30($depe, $tpAds, $tpdoc, $serie, $subserie, $usu, $fini, $ffin, $tpbusq, $tpRad)
    {
        ...
        select  distinct 
            ...
            CASE 
            WHEN r.radi_nume_radi = r.radi_nume_borrador and radi_depe_actu != 999 THEN 'En Tramite'
            WHEN r.is_borrador is true and radi_depe_actu = 999 THEN 'Eliminado'
            ELSE 'Radicado' 
            END AS estado, 
            r.radi_depe_radi, 
            CASE WHEN r.is_borrador IS FALSE THEN to_char(r.radi_fech_radi, 'DD-MM-YYYY HH24:MI') ELSE NULL END AS rfech,
            (SELECT to_char(hh.hist_fech, 'DD-MM-YYYY HH24:MI')
             FROM hist_eventos hh
             WHERE hh.radi_nume_radi = r.radi_nume_radi
               AND hh.sgd_ttr_codigo = 104
             ORDER BY hh.id ASC
             LIMIT 1) AS bfech,
            td.sgd_tpr_descrip tpnomb, 
            r.ra_asun asunto, 
            b.USUA_NOMB as USUAR,
            b.depe_codi as DEPEI, 
            r.radi_depe_actu DPA, 
            u.usua_nomb usuaa, 
            radi_nume_folio FOL,  
            '' dig , 
            mrec_codi mrec,
            '' FECHADig , 
            '' usud,
            r.radi_cuentai  ||' '||r.radi_nume_deri ||' ' ASOCIADO,
            '' dpto,
            '' proyecto,
            '' muni,
            '' EMAIL,
           ...
           FROM
           radicado r INNER JOIN ( select h1.radi_nume_radi as radi_nume_radi, h1.id, h1.sgd_ttr_codigo as sgd_ttr_codigo, h1.usua_doc as usua_doc, h1.depe_codi as depe_codi from hist_eventos h1 
          INNER JOIN (select distinct hist_eventos.radi_nume_radi as radi_nume_radi, min(hist_eventos.id) as id from hist_eventos where hist_eventos.sgd_ttr_codigo = 104 GROUP BY hist_eventos.radi_nume_radi) h2 on h1.id = h2.id $where) htev ON r.radi_nume_radi = htev.radi_nume_radi 
          LEFT JOIN usuario b ON htev.usua_doc = b.usua_doc 
           left join sgd_tpr_tpdcumento td on r.TDOC_CODI=td.SGD_TPR_CODIGO  left join usuario u on u.usua_codi = r.radi_usua_actu and u.depe_codi=r.radi_depe_actu 
           where  r.radi_fech_radi between  ('$fini 00:00:00') and ('$ffin 23:59:59')   $whereTipoRadicado order by rfech desc ";
        //trae info de total
