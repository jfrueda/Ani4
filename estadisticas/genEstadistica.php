...
if($_GET["tipoRadicado"])  $codus=$_GET["tipoRadicado"];
...
switch($db->driver){
	...
	case 'postgres':
		...
		if($tipoRadicado and ($tipoEstadistica==1 or $tipoEstadistica==2 or $tipoEstadistica==6 or $tipoEstadistica==7))
		{
			$whereTipoRadicado=" AND r.SGD_TRAD_CODIGO = $tipoRadicado";
		}
		break;
	default:
		...
		if($tipoRadicado and ($tipoEstadistica==1 or $tipoEstadistica==6 or $tipoEstadistica==7))
		{
			$whereTipoRadicado=" AND r.SGD_TRAD_CODIGO = $tipoRadicado";
		}
}
...
switch($tipoEstadistica){
	case "1";
		if($dependencia_busq != "99999") { /// Diferente a todas
	...
	case "2";
		if($dependencia_busq != "99999") { /// Diferente a todas
			$whereDependencia = "AND r.radi_depe_radi=$dependencia_busq";
		}else{
			$whereDependencia = "";
		}
	...
	case "6";
        ...
        if($_GET["codus"]){
            $whereTipoRadicado =" AND r.radi_usua_actu = $codus ";
        }

		include "$ruta_raiz/include/query/estadisticas/consulta006.php";
		$titulo="Radicados Actuales en la Dendencia";
		$generar = "ok";
	break;
	case "7";
	...
	case "17";
		...
		$titulo="Estadisticas radicados y sus respuestas.";
		$generar = "ok";
	break;
	case "27";
		if($dependencia_busq != "99999") { /// Diferente a todas
		...
		include "$ruta_raiz/include/query/estadisticas/consulta027.php";
		$titulo="Estadisticas de Digitalizacion";
		$generar = "ok";
	break;
	case "28";
		...
		include "$ruta_raiz/include/query/estadisticas/consulta028.php";
		$titulo="Estadisticas de Digitalizacion";
		$generar = "ok";
	break;
	case "29";
		...
		include "$ruta_raiz/include/query/estadisticas/consulta029.php";
		$titulo="Estadisticas de Digitalizacion";
		$generar = "ok";
	break;
  default;
	include "$ruta_raiz/include/query/estadisticas/consulta".str_pad($tipoEstadistica,3,"0", STR_PAD_LEFT).".php";
	$generar = "ok";
  break;
...
//$db->conn->debug = true;
if($tipoReporte==1 || $tipoReporte==7){
