...
<body onLoad="comboUsuarioDependencia(document.formulario,document.formulario.elements['dependencia_busq'].value,'codus');" topmargin="0">
	...
	<div id="app">
		<Transition name="slide-fade">
			<form v-if="showForm" id="form-reporte" name="formulario" method="GET" action='./vistaFormConsulta.php?<?= session_name() . "=" . trim(session_id()) . "&fechah=$fechah" ?>'>
				<div class="container-fluid py-3">
					<div class="card shadow-sm border-0">
						...
						<div class="card-body">
							...
							<div class="row mb-3">
								<div class="col-12 d-flex justify-content-between align-items-center">
									<div>
										POR RADICADOS –
										<a href='vistaFormProc.php?<?= session_name() . "=" . trim(session_id()) . "&fechah=$fechah" ?>'>
											POR PROCESOS
										</a>
									</div>

									<a class='btn btn-outline-primary btn-sm'
										href='vistaFormConsultaNew.php?<?= session_name() . "=" . trim(session_id()) . "&fechah=$fechah" ?>'>
										Nueva Versión
									</a>
								</div>
							</div>
							...
							<div class="alert alert-info small">
								<?= $helpE[$tipoEstadistica] ?>
							</div>

							<!-- TIPO DE ESTADISTICA -->
							<div class="mb-3">
								<label class="form-label fw-bold">Tipo de Consulta / Estadística</label>
								<select name="tipoEstadistica" class="form-select"
									onChange="formulario.submit();">
									<?php
									$num = 1;
									foreach ($tituloE as $key => $value) {
										$selectE = ($tipoEstadistica == $key) ? "selected" : "";
									?>
										<option value="<?= $key ?>" <?= $selectE ?>>
											<?= $num ?> - <?= $tituloE[$key] ?>
										</option>
									<?php
										$num++;
							...
							<!-- AQUI SIGUEN TUS BLOQUES DINÁMICOS EXACTAMENTE IGUALES -->
							<!-- SOLO SE MEJORA UI, NO SE CAMBIA NADA DE PHP -->
										...
										<div>
                                            ...
                                            order by depe_codi";
												...
												print $rsDep->GetMenu2(
													"dependencia_busq",
													$coddepe,
													'99999:-- Todas las Dependencias --',
													false,
													"",
													" onChange='formulario.submit();' class=\"form-select\""
												);
											} else {
												$sql = "select $sqlConcat ,depe_codi from dependencia 
                                            where DEPE_CODI=$dependencia order by depe_codi";
												$rsDep = $db->conn->Execute($sql);
												print $rsDep->GetMenu2(
													"dependencia_busq",
													"$coddepe",
							...
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">SUBSERIE</label>
									<label class="form-select">
										<?
										...
										$querySub = "select distinct (su.sgd_sbrd_descrip||'-'||su.sgd_sbrd_codigo) as detalle, su.sgd_sbrd_codigo
						...
						<?php if ($tipoEstadistica == 18) { ?>
							<!-- Dependencia ORIGEN -->
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Dependencia ORIGEN</label>
									<select name=dependencia_busqOri class="form-select" onChange="formulario.submit();">
										<?php
										...
										if ($usua_perm_estadistica == 2) {
											...
											$isqlusOri = "select a.DEPE_CODI,a.DEPE_NOMB,a.DEPE_CODI_PADRE from DEPENDENCIA a where $whereDepSelectOri ";
										}
						...
						?>
							<div class="card mb-3">
								<div class="card-body">
									<!-- LABEL PRINCIPAL -->
									<label class="form-label fw-bold d-block mb-2">Usuario</label>

									<!-- CHECKBOX: Incluir usuarios inactivos -->
									...
									<div class="form-check mb-2">
										...
										<label class="form-check-label" for="chkUsActivos">
											Incluir Usuarios Inactivos
										</label>
									</div>

									<!-- CHECKBOX: Limitar respuesta (solo para ciertos tipos) -->
									<?php if ($tipoEstadistica == 17 || $tipoEstadistica == 18 || $tipoEstadistica == 19) { ?>
										<?php $datoss = (isset($conSinRep) && $conSinRep) ? " checked " : ""; ?>
										<div class="form-check mb-3">
											...
											<label class="form-check-label" for="chkConSinRep">
									...
									<select name="codus"
										...
										<?php if (($tipoEstadistica == 5) && ($usua_perm_estadistica > 0)) { ?>
											<option value="0">-- NO AGRUPAR POR USUARIOS --</option>
										<?php } ?>

										<?php if (($usua_perm_estadistica > 0) && ($tipoEstadistica != 5)) { ?>
											<option value="0">-- AGRUPAR POR TODOS LOS USUARIOS --</option>
										<?php } ?>

										<?php
										// Filtros dinámicos
										$whereDep = ($dependencia_busq != 99999)
											? " u.DEPE_CODI = " . $dependencia_busq
											: '';

										$whereUsSelect = (!isset($_GET['usActivos']))
											? " u.USUA_ESTA = '1' "
											: "";
										...
										if ($dependencia_busq != 99999) {
											$whereUsSelect = ($whereUsSelect == "")
												? $whereDep
												: $whereUsSelect . " AND " . $whereDep;

											$isqlus = "SELECT u.USUA_NOMB, u.USUA_CODI, u.USUA_ESTA
                                            FROM USUARIO u
                                            WHERE $whereUsSelect
                                            ORDER BY u.USUA_NOMB";

											// TEMP: Printed for debugging in original code
											echo "<!-- SQL: $isqlus -->";
											...
											while (!$rs1->EOF) {
												...
												$selected = ($codus == $codigo) ? " selected " : "";

												echo "<option value='$codigo' $selected>$usNombre</option>";
						...
						if (
							$tipoEstadistica == 1 or $tipoEstadistica == 2 or $tipoEstadistica == 3 or
						...
						?>
							...
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Tipo de Radicado</label>
									<div>
										<?php
										...
										print $rs->GetMenu2($nmenu, $default_str, $blank1stItem = "$valor:$itemBlanco", false, 0, "class=\"form-select\"");
										?>&nbsp;
									</div>
								</div>
							</div>
						...
						<?php
						if (
							$tipoEstadistica == 1 or $tipoEstadistica == 6 or $tipoEstadistica == 7 or $tipoEstadistica == 10 or
						...
						?>
							...
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Agrupar por Tipo de Documento</label>
									<select name="tipoDocumento" class="form-select">
										<?php
										// -------------------------------------
										// Consulta para obtener tipos documentales
										// -------------------------------------
										...
										if ($tipoEstadistica == 6) {
											$datoss = ($tipoDocumento == '9998') ? 'selected' : '';
										?>
										...
										$datoss = ($tipoDocumento == '9997') ? 'selected' : '';
										?>
										...
										while (!$rs1->EOF) {
											...
											$nombre = $rs1->fields["SGD_TPR_DESCRIP"];
											$vecDeps[] = $codigo;

											$datoss = ($tipoDocumento == $codigo) ? 'selected' : '';

											echo "<option value=\"$codigo\" $datoss>$nombre</option>";
						...
						?>
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Desde fecha (aaaa/mm/dd)</label>
									<div class="input-group">
										...
										<input
											...
											id="startdate1"
											class="form-control"
											placeholder="Fecha de búsqueda inicial"
											value="<?= $fecha_ini ?>">
									</div>
								</div>
							</div>
							...
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Hasta fecha (aaaa/mm/dd)</label>
									<div class="input-group">
										<span class="input-group-text">
											<i class="fa fa-calendar"></i>
										</span>
										...
										<input
											...
											id="finishdate1"
											class="form-control"
											placeholder="Fecha de búsqueda final"
											value="<?= $fecha_fin ?>">
									</div>
								</div>
							</div>
						...
						?>
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">Año de secuencia</label>
									<select class="form-select" name="ano_secuencia">
										<?
										$agnoactual = Date('Y');
						...
						?>
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">REASIGNADO Desde fecha (aaaa/mm/dd)</label>
									<div class="input-group">
										<span class="input-group-text">
											<i class="fa fa-calendar"></i>
										</span>
										...
										<input
											...
											id="startdate2"
											class="form-control"
											placeholder="Fecha de busqueda inicial"
											value="<?= $fecha_ini2 ?>">
									</div>
								</div>
							</div>
							...
							<div class="card mb-3">
								<div class="card-body">
									<label class="form-label fw-bold">REASIGNADO Hasta fecha (aaaa/mm/dd)</label>
									<div class="input-group">
										<span class="input-group-text">
											<i class="fa fa-calendar"></i>
										</span>
										...
										<input
											...
											id="finishdate2"
											class="form-control"
											placeholder="Fecha de busqueda final"
											value="<?= $fecha_fin2 ?>">
									</div>
								</div>
							</div>
						...
						<div class="card margin-botton-table">
		...
		$(document).ready(function() {
			...
			$('#form-reporte').on('submit', function(e) {
				...
				if (start && end) {
					...
					if (diffMonths > 3 || (diffMonths === 3 && endDate.getDate() > startDate.getDate())) {
