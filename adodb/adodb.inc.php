...
*/
	...
	*/
	...
	/**
	* 	DESCOMPONE EL SQL, LO ANALIZA E INGRESA LOS DATOS CORRESPONDIENTES A LA TABLA DE AUDITORIA
	*/

	function create_trail($sql, $inputarr = false) {
		$finalSql = $sql;
		if (is_array($inputarr)) {
			foreach ($inputarr as $value) {
				...
				// Reemplaza solo la primera ocurrencia de ?
				$finalSql = preg_replace('/\?/', $escaped, $finalSql, 1);
			}
		}
		$sql=$finalSql;
		// descompongo el sql y verifico si es un insert, update o delete
		$arrayQuery=explode(" ", $sql);
		if($arrayQuery[0]!="SELECT"){
			// OBTENEMOS LA IP REAL DEL USUARIO SALTANDO EL PROXY
			if ($_SERVER) {  
			...
			// Compruebo si es un insert, update o delete		
			$pos = 0;
			$table = "";	
			if(strstr($sql,"INSERT")!=FALSE){				
				$type="i";
				$pos=2;				
			}elseif(strstr($sql,"UPDATE")!=FALSE){
				$type="u";
				$pos=1;
			}elseif(strstr($sql,"DELETE")!=FALSE){ 
				$type="d";
				$pos=2;
			}	
			// SACO EL NOMBRE DE LA TABLA
			if ($pos != 0) {
				$table=$arrayQuery[$pos];
				$table=explode("(", $table);
				$table=$table[0];
				$table=explode(" ", $table);
				$table=$table[0];
			}
			if(strcmp($table,"SGD_AUDITORIA")!=0 && isset($type) && $type!=''){
				$sql=str_replace("'","^", $sql);
				$query="INSERT INTO SGD_AUDITORIA (FECHA, USUA_DOC, IP, TIPO, TABLA, ISQL, MOMENTO) VALUES ('".time()."', '".$_SESSION['usua_doc']."', '$realip', '$type', '$table','$sql','".date('Y-m-d H:i:s')."')";
				$this->Execute($query);
			}			
		}				 
	}
	...
	*/
	...
	public function Execute($sql, $inputarr = false) {
		$this->create_trail($sql, $inputarr);
		if ($this->fnExecute) {
			$fn = $this->fnExecute;
	...
	*/
	...
	function AutoExecute($table, $fields_values, $mode = 'INSERT', $where = false, $forceUpdate = true, $magicq = false) {
		if (empty($fields_values)) {
			$this->outp_throw('AutoExecute: Empty fields array '.$table.' '.json_encode($fields_values), 'AutoExecute');
			return false;
		}
