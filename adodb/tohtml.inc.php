...
/*			$hdr .= "<TH class=titulos2 width=1%>$fname</TH>"; */
        ...
        if ($moreColumns){
            foreach($moreColumns as $columns => $column){
                $hdr .= "<TH>".$columns."</TH>";
            }
        } 
			...
			switch($fname)
			{
			 case 'CHU_';
				...
				case 97:
					...
					//validar codigo de devolucion cambios
	...
	break;
		...
		case 'DAT_';
			...
			$v = "<a class='radNumClass' href='".$rutaRaiz."/verradicado.php?verrad=".$verNumRadicado."&".$encabezado."'><span class='$radFileClass'>".$v."</span></a>";
			$special = "si";
			break;
		}
		$type = $typearr[$i];
