...
if($pais_afectado == 'Colombia')
{
    ...
    $muni_afectado = "\nMunicipio: ".$db->conn->getOne("SELECT MUNI_NOMB FROM municipio WHERE muni_codi = '$ciudad_afectado' and dpto_codi = '$departamento_afectado' ");
} else {
    $depto_afectado = '';
...
//$comentario = "\n\nDetalles del caso: ".$comentarios.$nombre_afect.$edad.$rango_edad.$sexo.$gestante.$poblacion_especial.$grupo_etnico.$p_afectado.$depto_afectado.$muni_afectado.$tel_afectado.$cel_afectado.$email_afectado;

$comentario = "\n\nDetalles del caso: ".$comentarios;
...
if($errorFormulario==0){
    ...
    $ins_rad.="'/$anoRad/".intval($depeRadicaFormularioWeb)."/$numeroRadicado".".pdf'
        ...
        ,".$_SESSION['depeRadicaFormularioWeb'].",1,3,1,0,0,1,0,0,0,0,$tipoPoblacion,'".$_SESSION['codigoverificacion']."',".$_SESSION['depeRadicaFormularioWeb'].",2)";
    ...
    if($uploader->tieneArchivos){
        for($i=0; $i < count($uploader->subidos);$i++)
        {
            ...
            $rs_tipoAnexo = $db->conn->getOne($sql_tipoAnex,[$extension]);
            $tipoCodigo = 24;
            if($rs_tipoAnexo){
