...
$result = $uploader->handleUpload('../bodega/tmp/');
echo htmlspecialchars(json_encode($result), ENT_NOQUOTES);
...
class qqFileUploader
{
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE)
    {
        if (!is_writable($uploadDirectory)) {
            return array('error' => "Error del Servidor. Directorio de subidas no es accesible.");
        }
        ...
        if ($size == 0) {
            return array('error' => 'El archivo está vacio');
        }
        ...
        if ($this->allowedExtensions && !in_array(strtolower($ext), $this->allowedExtensions)) {
            ...
            return array('error' => 'El archivo tiene una extensión inválida, la extensión debe ser una de éstas ' . $these . '.');
        }
        ...
        if ($this->file->save($uploadDirectory . DIRECTORY_SEPARATOR . $filename . $ext)) {
            ...
            return array('error' => 'No se pudo subir el archivo.' .
                'La subida fue cancelada, u ocurrió un error en el servidor');
        }
    }
}
