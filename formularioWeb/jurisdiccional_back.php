...
if (!verify_captcha($_POST['altcha']) || invalidateArray($_POST, ['tipo_demanda', 'apoderado_o_oficioso', 'tipo_identificacion_demandante', 'id_demandante'])) {
    header("Location: jurisdiccional.php");
    die();
}
...
// datos demandante
$num_id = $id_demandante;
$tipoDocumento = $tipo_identificacion_demandante;
$depto = isset($departamento_demandante) && $departamento_demandante != ''  ? $departamento_demandante : '11';
$muni = isset($ciudad_demandante) && $ciudad_demandante != ''  ? $ciudad_demandante : '1';
if($tipo_demandante == '1')
{
    $nombre_afectado = trim($nombre_demandante_1.' '.$nombre_demandante_2);
    $apellidos_afectado = trim($apellidos_demandante_1.' '.$apellidos_demandante_2);
} else {
    $nombre_afectado = trim($rs_demandante);
    $apellidos_afectado = '';
}
$email = $correo_demandante.'@'.$dominio_demandante;

if($pais_demandante == 'Colombia')
{
    $depto_afectado = "\nDepartamento: ".$db->conn->getOne("SELECT DPTO_NOMB FROM departamento WHERE dpto_codi = ?", [$departamento_demandante]);
    $muni_afectado = "\nMunicipio: ".$db->conn->getOne("SELECT MUNI_NOMB FROM municipio WHERE muni_codi = ? and dpto_codi = ? ", [$ciudad_demandante, $departamento_demandante]);
} else {
    $depto_afectado = '';
    $muni_afectado = "\nCiudad / Estado / Provincia: ".$provincia_demandante;
}

$tipo_doc_direccion = $db->conn->getOne("SELECT TDID_DESC FROM tipo_doc_identificacion WHERE tdid_codi = ?", [$tipo_identificacion_demandante]);
$nombre_direccion = $nombre_afectado;
$apellido_direccion = $apellidos_afectado;  
$email_direccion = $email;
$depto_direccion = $depto;
$muni_direccion = $muni;
$dir_direccion = $direccion_demandante;
$celular_direccion = $celular_demandante;
$telefono_direccion = $telefono_demandante;

$nombre_afect = "\nDemandante: ".$nombre_afectado.' '.$apellidos_afectado;
$tip_afectado = "\nTipo persona: ".($tipo_demandante == '1' ? "Natural" : "Juridico");
$p_afectado = "\nPaís: ".$pais_demandante;
$tel_afectado = "\nTeléfono: ".$telefono_demandante;
$cel_afectado = "\nCelular: ".$celular_demandante;
$email_afectado = "\nCorreo: ".$email;
$doc_afectado = "\nDocumento: ".$tipo_doc_direccion." ".$num_id;
$dir_afectado = "\nDirección: ".$dir_direccion;

//comentario con datos del demandante
$asunto = $tipo_demanda.($tipo_demanda == 'Conflictos derivados de las devoluciones o glosas a las facturas, reclamaciones y recobros de servicios No PBS' ? ' - '.$subtipos_demandas : '');
$comentario .= "\n\nDetalles del caso: ".$tip_afectado.$nombre_afect.$doc_afectado.$p_afectado.$depto_afectado.$muni_afectado.$dir_afectado.$tel_afectado.$cel_afectado.$email_afectado;

//datos apoderado
if($_POST['apoderado_o_oficioso'] == "Si")
{
    switch($tipo_apoderado)
    {
        case '1':
        case '2':
            $anonimo = 0;
            $nombre_peticionario = $tipo_apoderado == '1' ? trim($nombre_apoderado_1.' '.$nombre_apoderado_2) : $rs_apoderado;
            $apellidos_peticionario = $tipo_apoderado == '1' ? trim($apellidos_apoderado_1.' '.$apellidos_apoderado_2) : '';
            
            $num_id = $id_apoderado;
            $tipoDocumento = $tipo_identificacion_apoderado;
            $tipo_doc_direccion = $db->conn->getOne("SELECT TDID_DESC FROM tipo_doc_identificacion WHERE tdid_codi = ?", [$tipo_identificacion_apoderado]);
            $nombre_direccion = $nombre_peticionario;
            $apellido_direccion = $apellidos_peticionario;
            $email_direccion = $correo_apoderado.'@'.$dominio_apoderado;
            $depto_direccion = isset($departamento_apoderado) && $departamento_apoderado != ''  ? $departamento_apoderado : '11';
            $muni_direccion = isset($ciudad_apoderado) && $ciudad_apoderado != ''  ? $ciudad_apoderado : '1';
            $dir_direccion = $direccion_apoderado;
            $celular_direccion = $celular_apoderado;
            $telefono_direccion = $telefono_apoderado;
            $doc_peticionario = "\nDocumento: ".$tipo_doc_direccion." ".$num_id;
        break;
    }

    if($pais_apoderado == 'Colombia')
    {
        $depto_peticionario = "\nDepartamento: ".$db->conn->getOne("SELECT DPTO_NOMB FROM departamento WHERE dpto_codi = ?", [$departamento_apoderado]);
        $mpio_peticionario = "\nMunicipio: ".$db->conn->getOne("SELECT MUNI_NOMB FROM municipio WHERE muni_codi = ?  and dpto_codi = ? ", [$ciudad_apoderado, $departamento_apoderado]);
    } else {
        $depto_peticionario = '';
        $mpio_peticionario = "\nCiudad / Estado / Provincia: ".$provincia_apoderado;
    }

    $p_peticionario = "\nPaís: ".$pais_apoderado;
    $dreccion_peticionario = isset($direccion_apoderado) ? "\nDirección: ".$direccion_apoderado : '';
    $dreccion_peticionario_2 = isset($direccion_apoderado_2) ? "\nDirección comercial: ".$direccion_apoderado_2 : '';
    $tel_peticionario = isset($telefono_apoderado) ? "\nTeléfono: ".$telefono_apoderado : '';
    $cel_peticionario = isset($celular_apoderado) ? "\nCelular: ".$celular_apoderado : '';
    $email_peticionario = "\nCorreo: ".$correo_apoderado.'@'.$dominio_apoderado;

    $comentario .= "\n\nApoderado o agente oficioso";
    
    $comentario .= "\nTipo persona: ".($tipo_apoderado == '1' ? "Natural" : "Juridico");
...
if($errorFormulario==0){
        ...
        trim($res['RA_ASUN']) == 'ADJUNTO SOLICITUD DE CONCILIACIÓN:')
    {
        ...
        $errorFormulario = 0;
        $codTx = 1983;
        $mensaje_tipo = "Se ha registrado en el Sistema de Información de la Superintendencia Nacional de Salud la demanda jurisdiccional con el número de radicado ".$_SESSION['radcom'].". Por favor conserve el número para consultar el estado de su demanda.";
        $mail_origen = "funcionjurisdiccional@supersalud.gov.co";
