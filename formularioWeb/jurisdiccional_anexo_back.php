...
if (!verify_captcha($_POST['altcha']) || invalidateArray($_POST, ['calidad_sujeto_procesal', 'tipo_sujeto', 'tipo_identificacion_sujeto', 'id_sujeto', 'radicado'])) {
...
$num_id = $id_sujeto;
$tipoDocumento = $tipo_identificacion_sujeto;
$depto = isset($departamento_sujeto) && $departamento_sujeto != ''  ? $departamento_sujeto : '11';
$muni = isset($ciudad_sujeto) && $ciudad_sujeto != ''  ? $ciudad_sujeto : '1';
$tipo_doc_direccion = $db->conn->getOne("SELECT TDID_DESC FROM tipo_doc_identificacion WHERE tdid_codi = ?", [$tipo_identificacion_sujeto]);
$nombre_direccion = ($tipo_sujeto == '1') ? trim($nombre_sujeto_1.' '.$nombre_sujeto_2) : trim($rs_sujeto);
$apellido_direccion =($tipo_sujeto == '1') ? trim($apellidos_sujeto_1.' '.$apellidos_sujeto_2) : '';
$email_direccion = $correo_sujeto.'@'.$dominio_sujeto;
$depto_direccion = $depto;
$muni_direccion = $muni;
$dir_direccion = $direccion_sujeto;
$celular_direccion = $celular_sujeto;
$telefono_direccion = $telefono_demandante;

// datos sujeto procesal
$comentario = "Sujeto procesal ".
            "\nTipo: ".$calidad_sujeto_procesal.
            "\nTipo persona: ".($tipo_sujeto == '1' ? "Natural" : "Juridico").
            "\nDocumento: ".$db->conn->getOne("SELECT TDID_DESC FROM tipo_doc_identificacion WHERE tdid_codi = ?", [$tipo_identificacion_sujeto]).' '.$id_sujeto.
            "\nNombre: ".($tipo_sujeto == '1' ? trim($nombre_sujeto_1.' '.$nombre_sujeto_2).' '.trim($apellidos_sujeto_1.' '.$apellidos_sujeto_2) : trim($rs_sujeto)).
            "\nPaís: ".$pais_sujeto.
            "\nDepartamento: ".$db->conn->getOne("SELECT DPTO_NOMB FROM departamento WHERE dpto_codi = ?", [$departamento_sujeto]).
            "\nMunicipio: ".($pais !== 'Colombia' ? $db->conn->getOne("SELECT MUNI_NOMB FROM municipio WHERE muni_codi = ? and dpto_codi = ? ", [$departamento_sujeto, $ciudad_sujeto]) : $provincia_sujeto).
            "\nDirección: ".$direccion_sujeto.
            "\nCelular: ".$celular_sujeto.
            "\nTeléfono: ".$telefono_sujeto.
            "\nCorreo: ".$correo_sujeto.'@'.$dominio_sujeto;

//comentario con datos del demandante
$asunto = 'Adjunto proceso jurisdiccional: '.$radicado;
$comentario .= "\n\nDetalles del anexo: ".$comentarios;
$pqrsFacebook = 0;
$autoriza_respuesta = '';
...
if($errorFormulario==0){
    ...
    //Insertar expediente
    $exp = $db->conn->getAll('SELECT * FROM sgd_exp_expediente WHERE sgd_exp_numero = ?', [$expediente])[0];
    $ins_exp = "INSERT INTO sgd_exp_expediente(sgd_exp_numero, radi_nume_radi, sgd_exp_fech, depe_codi, usua_codi, usua_doc, sgd_exp_estado) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $db->conn->Execute($ins_exp, [$exp['SGD_EXP_NUMERO'], $numeroRadicado, date('Y-m-d H:i:s'), $exp['DEPE_CODI'], $exp['USUA_CODI'], $exp['USUA_DOC'], "0"]);

    //$ins_exp = "INSERT INTO sgd_exp_expediente (sgd_exp_numero, radi_nume_radi, sgd_exp_fech) "
        ...
        trim($res['RA_ASUN']) == 'ADJUNTO SOLICITUD DE CONCILIACIÓN:')
    {
        ...
        $errorFormulario = 0;
        $codTx = 1983;
        $mensaje_tipo = "Se ha registrado en el Sistema de Información de la Superintendencia Nacional de Salud el anexo con número de radicado ".$_SESSION['radcom']." al expediente ".$exp['SGD_EXP_NUMERO'].".";
