...
<head>
  ...
  $hidden = "true";
  $swBusqDep  = "si";
  $dependencia_busq2 = " and c.depe_codi  = " . $dep_sel;
  ...
  <script>
    $(document).ready(function() {
      ...
      function verificarRestricciones(objRadicado) {
        ...
        $.post("../include/ajax/envios/ajaxValEnviar.php", {
          radicadoEnviar: objRadicado
        }, function(res) {
        ...
        <div class="card-body">
          ...
          <div class="table-responsive mb-4">
            <table class="table table-bordered table-striped align-middle">
              <tr>
                <td>
                  <!-- SUBCONTENIDO (incluye PHP, NO TOCADO) -->
                  <div class="p-3 bg-light border rounded">
                    <?php include "../envios/paEncabeza.php"; ?>
                    <?php include "../envios/paBuscar.php"; ?>

                    <!-- Grupo de filtros y botón -->
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 gap-3">
                      <div>
                        <strong class="me-2">Listar por:</strong>
                        <a href="<?= $pagina_actual ?>?<?= $encabezado ?>98&ordcambio=1" class="me-2 text-success fw-semibold">Enviados</a>
                        <?= $img7 ?>
                        <a href="<?= $pagina_actual ?>?<?= $encabezado ?>99&ordcambio=1" class="ms-2 text-danger fw-semibold">Por Enviar</a>
                      </div>

                      <input type=submit value="<?= $accion_sal ?>" id="botonEnviar" name="Enviar" id="Enviar" valign="middle" class="btn btn-primary px-4" onclick="marcar();">
                    </div>
                  </div>
                  ...
                  <!-- Solicitar firma -->
                  <?php if ($usua_perm_firma == 2 || $usua_perm_firma == 3) : ?>
                    <div class="my-4 p-3 bg-warning-subtle border rounded d-flex justify-content-end">
          ...
          <form name="formEnviar" method="post"
            action="<?= $pagina_sig ?>?&fechah=<?= $fechah ?>&dep_sel=<?= $dep_sel ?>&estado_sal=<?= $estado_sal ?>&usua_perm_impresion=<?= $usua_perm_impresion ?>&estado_sal_max=<?= $estado_sal_max ?>"
            onsubmit="myFunction()">
            ...
            <div class="table-responsive">
              <table class="table table-hover table-striped align-middle">
                <thead class="table-primary">
                  <tr>
                    <th class="text-center"><img src='<?= $ruta_raiz ?>/imagenes/estadoDoc.gif'></th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=1"><?= $img1 ?> Radicado Salida</a></th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=2"><?= $img2 ?> Copia</a></th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=3"><?= $img3 ?> Radicado Padre</a></th>
                    <th>Tipo de envío</th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=4"><?= $img4 ?> Fecha Radicado</a></th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=5"><?= $img5 ?> Descripción</a></th>
                    <th><a ><?= $img6 ?> Fecha marca</a></th>
                    <th><a  href="<?= $PHP_SELF . '?' . $encabezado ?>1&ordcambio=1&orno=7"><?= $img7 ?> Generado Por</a></th>
                    <th class="text-center"><input id="checkAll" onclick="markAll();" type="checkbox"></th>
                  </tr>
                  <tr>
                    Marcar Todos &nbsp;<input name="checkAll" value="checkAll" id="checkAll" onclick="markAll();" type="checkbox">
                  </tr>
                </thead>
                <tbody>
                  <?
                  ...
                  while ($rs && !$rs->EOF) {
                    if ($rs->fields['E_ESTADO'] == 0) {
                      if ($ki >= $registro and $ki < ($registro + $registrosXpagina)) {
                        ...
                        $imgEstado = "<img src=$ruta_raiz/imagenes/docDevueltoActualizado.gif  border=0>";
                  ...
                  ?>
                        <tr class='<?= $formato ?>'>
                          <td class='img-estdos'>
                            <img src='<?= $ruta_raiz ?>/imagenes/estadoDoc.gif' border=0>
                          </td>
                          <td class='<?= $leido ?>' align="center" width="14%">
                            <?= $imgEstado ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Radicado Salida:" width="8%">
                            <?
                            /**
                          ...
                          <td class='<?= $leido ?>' data-titulo="Copia" width="5%">
                            <span class='<?= $leido ?>'>
                              <?= $copia ?>
                            </span>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Radicado Padre" width="9%">
                            <?= $rad_padre  ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Tipo de envio" width="9%">
                            <?= $tipo  ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Fecha de radicado" width="9%">
                            <?= $fech_radicado ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Descripción" width="30%">
                            <?= $descripcion ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Fecha Impresion" width="12%"> &nbsp;
                            <?= $fecha_impre; ?>
                          </td>
                          <td class='<?= $leido ?>' data-titulo="Generado Por" width="10%">
                            <?= $generadoPor ?>
                          </td>
                          <td align='center' class='<?= $leido ?>' data-titulo="Seleccione tramite" width="3%">
                            <? if ($swEsperaFirma) { ?>
                              <script>
            ...
            <div class="container my-3">
              <div class="row align-items-center">
                <!-- Columna izquierda (vacía para ahora, preservando tu estructura original) -->
                <div class="col-md-6 mb-2">
                  <div class="fw-bold"></div>
                </div>

                <!-- Columna derecha con el botón -->
                <div class="col-md-6 mb-2 text-center">
                  <a href="<?= $pagina_actual ?>?<?= $encabezado ?>" class="d-none"></a>
                  <input
                    type="submit"
                    value="Generar listado para Estos radicados"
                    name="Enviar"
                    id="Enviar"
              ...
              <div class="col-auto">
                <nav aria-label="Paginación de resultados">
                  <ul class="pagination pagination-sm">
                    ...
                    echo "<span class='me-2 fw-semibold text-secondary'>Páginas:</span>";
                    ...
                    for ($ii = 0; $ii < $paginas; $ii++) {
                      ...
                      echo "
                                    <li class='page-item $active'>
                                        <a class='page-link'
                                            href='$PHP_SELF?porEnviar=$porEnviar&dep_sel=$dep_sel&pagina=$ii&$encabezado&orno=$orno&usuaCodiEnvio=$usuaCodiEnvio'>
                                            " . ($ii + 1) . "
                                        </a>
                                    </li>";
                    }
