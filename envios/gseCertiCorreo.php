        ...
        if ($rs && !$rs->EOF && $rs->fields["NUM"] > 0 && file_exists($saveFiles.$namePath)) {
			...
			if($rs_per_cierre->fields["DEP_CIERRE"]==1)
				
				{
								...
								switch ($mensaje) {
										...
										default:
												...
												break;
															...
															/*captura estado del acuse*/

															$sql_update_est="UPDATE sgd_renv_regenvio SET sgd_renv_observa='".$mensaje."' WHERE sgd_renv_nombre='".$id_envio."'";
															$db->conn->query($sql_update_est);


															/*numero de envios*/

															$sql_num_envios="SELECT COUNT(*) k FROM sgd_renv_regenvio WHERE radi_nume_sal=".$output[0]." AND sgd_fenv_codigo=106 
															AND sgd_renv_destino IS NOT NULL";

															$rs_num_envios=$db->conn->query($sql_num_envios);

															$kenvios=$rs_num_envios->fields['K'];

															/*numero de acuses*/


															$sql_num_acuses="SELECT COUNT(*) k FROM sgd_renv_regenvio WHERE radi_nume_sal=".$output[0]." AND sgd_fenv_codigo=106 
															AND sgd_renv_dir like '%target%' AND (sgd_renv_observa like '%Open%' OR sgd_renv_observa like '%Click%')";

															$rs_num_acuses=$db->conn->query($sql_num_acuses);

															$kacuses=$rs_num_acuses->fields['K'];

															/*verifica que no haya rebotes*/
						...
						$sql_exp="SELECT COUNT(*) k  FROM sgd_exp_expediente WHERE radi_nume_radi=".$output[0]." or radi_nume_radi in (select anex_radi_nume from anexos where radi_nume_salida=".$output[0].")";									
						...
						/*verifica que todas las salidas tengan envio*/
						...
						$sql_sse="select
								...
								and anex_radi_nume in (
									...
									anexos a
									where
									radi_nume_salida = ".$output[0].")";

						$rs_sse=$db->conn->query($sql_sse);
						if($rs_sse->fields["K"]>0)
							$error_envio=1;						
						...
						if($error_envio==0)
							...
							$sql_ins_h="
								...
								$db->conn->query($sql_ins_h);
																		...
																		/*cierre automatico padre*/

																		$sql_ce="SELECT anex_radi_nume radicado,anex_env_email cierre FROM anexos WHERE radi_nume_salida=".$output[0];
																		$rs_ce=$db->conn->query($sql_ce);
																		
																		if($rs_ce->fields["CIERRE"]==1)
																		{
																			$sql_update="UPDATE radicado SET radi_depe_actu=999,radi_usua_actu=15 WHERE radi_nume_radi =".$rs_ce->fields["RADICADO"];
																			...
																			insert
																				...
																				now(),
																				1,
																				".$rs_ce->fields["RADICADO"].",
																				'Cierre automático con el radicado ".$output[0].", Acuse de recibido con estado: ".$mensaje."',
																				15,
																				'10153900001',
																				null,
																				13,
