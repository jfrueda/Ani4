...
$depencias_h = array(93004,93001,900);
if (in_array($_SESSION['dependencia'], $depencias_h))
  {
  //echo "Match found";
  }
else
  {
	die('No tiene permisos para acceder a esta función');
  }
...
from
    radicado r
    join anexos a on a.radi_nume_salida = r.radi_nume_radi
    join sgd_dir_drecciones d on r.radi_nume_radi = d.radi_nume_radi
    left join sgd_rad_envios e on d.id = e.id_direccion
where
    a.anex_estado = 3
    and r.mrec_codi = 4
    and e.id_direccion is null
    ...
    <script type="text/javascript">
		...
		$('.glyphicon-forward, .glyphicon-chevron-right, .glyphicon-chevron-left, .glyphicon-backward').on('click', function() {
			run_waitMe('bounce');
		});

		function digitalSending(codigo, envio) {
			if ($("#btn-" + codigo + "_" + envio)[0].classList.value.indexOf("btn-danger") >= 0) {
				alert(
					"Error al enviar correo electronico favor devolver, para evitar el reenvio de este mismo radicado error: Este correo ya fue enviado"
					);
			} else {
				$.ajax({
