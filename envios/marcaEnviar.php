...
<form name='forma' id="forma"
      action='marcaEnviar.php?<?= session_name() . "=" . session_id() . "&fecha_h=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&estado_sal_max=$estado_sal_max&no_planilla=$no_planilla" ?>'
      method=post>
...
<? if ($checkValue) {
    ...
    while ($i < $num) {
        ...
        $envio_id = explode("_",key($checkValue))[2];
        if($envio_id_list == '') {
            $envio_id_list =  $envio_id;
        } else {
            $envio_id_list =  $envio_id_list . ',' . $envio_id;
        }
        // echo $record_id. ">>>";
        ...
        if ($estadoFirma == "COMPLETA") {
            ...
            if ($swValido == false) {
                ...
                echo("</span>");
                die;
            } else { //Archivo donde se le indica al servidor de Open Office como llevar a cabo la combinaci�n
        ...
        $db->conn->execute("UPDATE sgd_rad_envios SET estado = 1 WHERE id = $envio_id");

        $sqlTipoDireccion = "select e.tipo, d.sgd_dir_mail, d.sgd_dir_direccion, d.radi_nume_radi
                from sgd_rad_envios e
                join sgd_dir_drecciones d on d.id = e.id_direccion
                WHERE   e.id= $envio_id limit 1";
        $rsSqlTipoDireccion = $db->query($sqlTipoDireccion);      
        
        $hisPorEnviar = 'Se marca por enviar ';
        if($rsSqlTipoDireccion->fields['TIPO'] == 'E-mail') {
            $hisPorEnviar .= 'E-mail ' . $rsSqlTipoDireccion->fields['SGD_DIR_MAIL'];
        } else {
            $hisPorEnviar .= 'Físico ' . $rsSqlTipoDireccion->fields['SGD_DIR_DIRECCION'];
        }

        $radicadosSel[0] = $rsSqlTipoDireccion->fields['RADI_NUME_RADI'];
    ...
    if ($radicadosSel)
        $whereFiltro = " and cast(b.radi_nume_radi as varchar(20) ) in($setFiltroSelect)";
} // FIN  if ($checkValue)
...
if (!$verRegistros) {
    ...
    while (!$rsMarcar->EOF) {
        ...
        $dirRemDestino = $rsMarcar->fields["SGD_DIR_NOMREMDES"];
        $dirDireccion = $rsMarcar->fields["SGD_DIR_DIRECCION"];
        $dirDptoCodi = $rsMarcar->fields["DPTO_CODI"];
        $dirMuniCodi = $rsMarcar->fields["MUNI_CODI"];
        $dirMail = $rsMarcar->fields["SGD_DIR_MAIL"];
        $dirTelefono = $rsMarcar->fields["SGD_DIR_TELEFONO"];
        $dirIdPais = $rsMarcar->fields["ID_PAIS"];
        $dirIdCont = $rsMarcar->fields["ID_CONT"];
        ...
        $nombre_us = substr($dirRemDestino, 0 , 35);
        ...
        $pais = $a->GET_NOMBRE_PAIS($dirIdPais, $db);
        ...
        */
        ...
        if ($rem_destino == 3) {
            ...
            $destino = $muni_nomlistaMabre_us3;
        ...
        $email_us = $dirEmail;
        $telefono_us = $dirTelefono;
        $nombre_us = $dirRemDestino;
        $destino = $muni_nombre_us;
        $departamento = $dpto_nombre_us;
        $direccion_us = $dirDireccion;
        $dir_codigo = $dirCodigo;
        $dir_tipo = $sgd_dir_tipo;
        /*
        * Fin modificacion
        */
        if (!$mensaje) {
...
<?php	echo("<center><a href='$ruta_raiz/envios/cuerpoMarcaEnviar.php?usua_perm_impresion=$usua_perm_impresion&carpeta=8&nomcarpeta=Documentos Para Impresion&orderTipo=desc&orderNo=3&porEnviar=$porEnviar' target='mainFrame' class='vinculos'>Volver a listado.</a></center>");?>
</form>
</div>
</div>
</div>
