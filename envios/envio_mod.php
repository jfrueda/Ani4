...
$depe_municipio  = $_SESSION["depe_municipio"];
...
<link href="<?=$ruta_raiz."/estilos/".$_SESSION["ESTILOS_PATH"]?>/orfeo.css" rel="stylesheet" type="text/css">
<script>
function back1() {
    history.go(-1);
}
  ...
  <table border=0 width=100% class=borde_tab cellspacing="5">
    ...
    <?
  	 if ($valRadio) 
	 {
			...
			$radSelec = $valRadio;
			$radicado_mat = explode("-",$radSelec,4);
    ...
    <?
			...
			print $rsEnv->GetMenu2("empresa_envio","$empresa_envio",false, false, 0," id=empresa_envio class='select' onClick='calcular_precio();'");
   ?>
   </td>
...
//	echo "<hr>Radicados";
   ...
   if (($reg_envio) AND (!$envio_peso) ) die ("<p><hr><b><center><span class=etexto>Debe Colocar el Peso para poder Enviar el Radicado  Archivo anexado correctamente<br></br></span><hr>");
   if(!$reg_envio)
   {
    $i = $iii;
   ?>
