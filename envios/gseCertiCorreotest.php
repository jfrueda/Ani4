...
$params   = http_build_query(array('startDate' => date('Y-m-d', time()) ,
								...
								if($output[0]==$_GET[rad])
									
									{
														...
														if ($rs && !$rs->EOF && $rs->fields["NUM"] > 0 && file_exists($saveFiles.$namePath)) {
															...
															/***************************************************************/
															/************** CERITIFICACION ESTADO DE CADA CORREO ***********/
															/***************************************************************/
															...
															AND sgd_renv_dir like '%target%' AND (sgd_renv_observa like '%Delivery%' OR sgd_renv_observa like '%Open%' OR sgd_renv_observa like '%Click%')";
															...
															$sql_exp="SELECT COUNT(*) k  FROM sgd_exp_expediente
															WHERE radi_nume_radi=".$output[0];
															...
															$sql_trd="SELECT COUNT(*) k FROM sgd_rdf_retdocf
															WHERE radi_nume_radi=".$output[0];
															...
															if($error_envio==0)
																...
																{	
																		...
																		/*$sql_update="UPDATE radicado SET radi_depe_actu=999,radi_usua_actu=15 WHERE radi_nume_radi in (select anex_radi_nume
																			from anexos
																			where radi_nume_salida=".$output[0].")";
																		...
																		insert
																			...
																			1,
																			".$output[0].",
																			'Cierre automático, Acuse de recibido con estado: ".$mensaje."',
																			15,
																			'10153900001',
																			null,
																			13,
