  ...
  <table width="100%" align="center" class="table table-striped table-hover table-bordered">
    ...
    $isql = "select $sqlFecha AS HIST_FECH1
      , a.DEPE_CODI
			...
			from hist_eventos a
		 where
			a.radi_nume_radi =$verrad and
      a.SGD_TTR_CODIGO not in (111,110)
...
$isql = "select $sqlFechaEnvio AS SGD_RENV_FECH,
    ...
    AND a.SGD_RENV_OBSERVA not like '%Masiva grupo%'
  ...
  <table width="100%" align="center" class="table table-bordered">
    ...
    ?>
        <tr> <? $i = 1;
              ...
              ?>
        <td>
          <small><?= $imgRadDev ?><?= $radEnviado ?></small>
        </td>
        <td>
          <small><?= $rs->fields["DEPE_NOMB"] ?></small>
        </td>
        <td><small>
            <?
        ...
        <td>
          <small><?= $rs->fields["SGD_RENV_DEPTO"] ?> </small>
        </td>
        <td>
          <small><?= $rs->fields["SGD_RENV_MPIO"] ?> </small>
        </td>
        <td>
          <small><?= $rs->fields["SGD_FENV_DESCRIP"] ?> </small>
        </td>
        <td>
          <small><?= $rs->fields["SGD_RENV_PLANILLA"] ?> </small>
        </td>
        <td>
          <small><?= $rs->fields["SGD_RENV_OBSERVA"] ?> </small>
        </td>
        <td>
          <small><?= $rs->fields["USUA_LOGIN"] ?> </small>
        </td>
