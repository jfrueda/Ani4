...
<div class="alert alert-success" role="alert">
 <h3>REPORTE RADICADOS DEVUELTOS</h3>
</div>
...
$generar_informe = 'generar_informe';
  ...
  <strong>Listado de documentos Devueltos <?=$fecha_ini?> a <?=$fecha_fin?></strong> 
  </div>
	<?php
...
select
    distinct 
    r.radi_nume_sal radicado_salida,
    r.sgd_renv_fech fecha_envio,
    r.sgd_renv_dir direccion,
    concat(r.sgd_renv_nombre, ' ', r.sgd_renv_destino) destinatario
    ,
    r2.ra_asun asunto
    ,
    u.usua_nomb responsable_envio,
    case
        when r.sgd_renv_tipo = 0 then 'EMAIL'
        when r.sgd_renv_tipo = 1 then 'FISICO'
    end medio_envio,
    h.hist_obse motivo_devolucion,
    h.hist_fech fecha_devolucion,
    f.sgd_fenv_descrip empresa_mensajeria
from
    sgd_renv_regenvio r,
    radicado r2,
    usuario u,
    ...
    sgd_fenv_frmenvio f
where
    r.radi_nume_sal = r2.radi_nume_radi
    and cast(r.usua_doc as varchar) = u.usua_doc
    and h.radi_nume_radi = r.radi_nume_sal
    and h.sgd_ttr_codigo = 28
    and r.sgd_fenv_codigo =f.sgd_fenv_codigo
    and r.sgd_renv_fech between '".$fecha_ini." 00:00:01' and '".$fecha_fin." 23:59:59' 
    and r.radi_nume_sal in (
    select
        radi_nume_radi
    from
        hist_eventos
    where
        sgd_ttr_codigo in (28
, 68
, 90))
order by 2";


	$pager = new ADODB_Pager($db,$query_t,'adodb', true,1,$orderTipo);
