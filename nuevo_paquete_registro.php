<?php
// Programa que actualiza el paquete de documentos de numeraci�n y fechado
session_start();
//print("ENTRA");
if (!$ruta_raiz) {
	$ruta_raiz = ".";
...
//Trae los datos relacionados con el paquete de documentos de numeraci�n y fechado
...
//print ($isql);
$directorio="./bodega/".substr(trim($archivo),0,4)."/".substr(trim($archivo),4,$digitosDependencia)."/docs/";
...
if ($doctoPrincipal=="1") {
		...
		while ($i < $num) {			
			 ...
			 if (!$rs3){
			 	$db->conn->RollbackTrans();
		 	 	die ("<span class='etextomenu'>No se ha podido Grabar la informaci�n seleccionada");
			 }
