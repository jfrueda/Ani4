...
////  Validaciones /////
if ($orden_cambio == 1) (!$orderTipo) ? $orderTipo = "desc" : $orderTipo = "";
...
if (
	...
	$tipoDocumento ||
	$dependenciaSel ||
	$adodb_next_page || $orderNo || $orderTipo || $orden_cambio
) {
	if (!isset($orderNo)) {
		$orderNo = 0;
	}
	...
	$sql = 'SELECT R.RADI_NUME_RADI AS "IMG_Numero Radicado",
		...
		$db->conn->concat('R.RADI_DEPE_ACTU', "'-'", 'R.RADI_USUA_ACTU', "'-'", 'R.SGD_SPUB_CODIGO', "'-'", 'R.CODI_NIVEL') . ' AS "HID_priv"
			FROM RADICADO R INNER JOIN
				USUARIO U ON R.RADI_USUA_ACTU = U.USUA_CODI AND R.RADI_DEPE_ACTU = U.DEPE_CODI INNER JOIN
				SGD_DIR_DRECCIONES A ON R.RADI_NUME_RADI = A.RADI_NUME_RADI 
			WHERE ' .
		$db->conn->SQLDate('Y/m/d', 'R.RADI_FECH_RADI') . " BETWEEN '" . $fecha_ini . "' and '" . $fecha_fin . "' " . $sWhere .
		" GROUP BY R.RADI_NUME_RADI, R.RADI_PATH,  " . $sqlFecha . ", R.RADI_NUME_RADI, A.SGD_DIR_NOMREMDES, A.SGD_DIR_DOC, 
					U.USUA_NOMB, " . $db->conn->concat('R.RADI_DEPE_ACTU', "'-'", 'R.RADI_USUA_ACTU', "'-'", 'R.SGD_SPUB_CODIGO', "'-'", 'R.CODI_NIVEL') . $c_order;
...
<body class="PageBODY" topmargin="0" onLoad="window_onload();">
	...
	<form method="post" action="<?= $encabezadol ?>" name="Search">
			...
			<tr>
				<td valign="top">
					<table class="FormTABLE">
						<tr>
							<td colspan="2" class="titulos4">B&uacute;squeda por Usuario</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="titulos5">
								<INPUT type="radio" NAME="s_solo_nomb" CHECKED value="All" <? if ($s_solo_nomb == "All") {
																								echo "CHECKED";
																							} ?>>Todas las palabras (y)<br>
								<INPUT type="radio" NAME="s_solo_nomb" value="Any" <? if ($s_solo_nomb == "Any") {
																						echo "CHECKED";
				...
				<td valign="top">
					<a class="vinculos" href="../busqueda/busquedaPiloto.php?<?= session_name() . "=" . session_id() . "&fechah=$fechah&primera=1&ent=2&s_Listado=VerListado"; ?>">B&uacute;squeda Cl&aacute;sica</a><br>
					<a class="vinculos" href="../busqueda/busquedaHist.php?<?= session_name() . "=" . session_id() . "&fechah=$fechah&primera=1&ent=2"; ?>">B&uacute;squeda Hist&oacute;ricos</a><br>
					<a class="vinculos" href="../busqueda/busquedaExp.php?<?= session_name() . "=" . session_id() . "&fechah=$fechah&primera=1&ent=2"; ?>">B&uacute;squeda Expediente</a>
				</td>
			</tr>
		</table>
		<?php
		if (
			...
			echo "<table width=100%><tr><td class='titulos5'>Reporte por usuarios.</td></tr>";
