...
<body onload="<? if ($param->PARAM_VALOR == '1') { ?> showInBox();<? } ?>">
								...
								<tr>
									<td valign="top">
										<table width="150" border="0" cellpadding="0" cellspacing="3">
											...
											$isql = "select count(1) as CONTADOR from SGD_AGEN_AGENDADOS agen where  usua_doc='$usua_doc' and agen.SGD_AGEN_ACTIVO=1 $sqlAgendado";
											$rs = $db->query($isql);
											...
											($carpeta == $numdata && $tipo_carp = 0) ? $imagen = "folder_open.gif" : $imagen = "folder_cerrado.gif";
											$rs1 = $db->query($isql);
											if ($rs1) {
												$numerot = $rs1->fields["CONTADOR"];
											} else {
											...
											?>
												<tr valign="middle">
													<!-- <td width="25"><img src="imagenes/menu.gif" width="15" height="18" alt='<?= $data ?> ' title='<?= $data ?>' name="plus<?= $i ?>"></td> -->
													<td width="125">
														<a onclick="cambioMenu(<?= $i ?>);" href='tx/cuerpoInfConjunto.php?<?= $phpsession ?>&<?= "mostrar_opc_envio=1&orderNo=2&fechaf=$fechah&carpeta=66&nomcarpeta=Informados&orderTipo=desc&adodb_next_page=1"; ?>' class="menu_princ" target="mainFrame" alt='<?= $data ?>' title="Memorando Multiple">
															Memorando múltiple (<?= $numerot ?>) <? $i++; ?>
														</a>
													</td>
												</tr>
											<?php
											...
											<tr valign="middle">
												...
												if ($numeroP >= 1) {
													$clasePrioritarios = "titulosError";
												} else {
													$clasePrioritarios = "menu_princ";
												}
												?>
