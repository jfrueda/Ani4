...
if ($rs_trad->RecordCount() >= 0) {
	...
	while (!$rsC->EOF) {
		...
		$val .= "<td align='center' >" . $rsC->fields["CODIGO"] . "</td>";
		$val .= "<td align='left'>" . $rsC->fields["TIPOD"] . "</td>";
		$val .= "<td align='center'>" . $rsC->fields["TERMINO"] . "</td>";
		$conteo = count($matriz);
		for ($j = 0; $j < $conteo; $j++) {
			$val .= "<td>" . $rsC->fields[$matriz[$j]] . "</td>";
		}
		$val .= '<td align="center" >' . $rsC->fields["ESTADO"] . '</td>';
		$val .= '<td align="center">' . $rsC->fields["SGD_TPR_RADICA"] . '</td></tr>';
		$rsC->MoveNext();
	}
}
	...
	<table class="table table-striped table-hover align-middle">
		<thead class="table-primary">
			<tr>
				...
				<th class="text-center">Form Radica</th>
			</tr>
		</thead>
