...
<body bgcolor="#ffffff">
		...
		<tr>
			<td class="titulos2" ALIGN=CENTER>
				<FONT SIZE=2>INFORMACION DEL DOCUMENTO CON NUMERO DE RADICADO <?= $verrad ?>
					<a href='<?= $ruta_raiz ?>/bodega/<?= $radi_path ?>'>
				</FONT>
				<FONT SIZE=1>(Ver Imagen del documento)</a></FONT>
			</td>
		</tr>
	</table>
	<table width="100%" border="0" cellspacing="5" cellpadding="0">
		<tr>
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="borde_tab">
					<tr>
						<td>
							<table width="100%" border="0" cellspacing="5" cellpadding="0">
								<tr>
		...
		<tr>
			<td colspan="2">
				<table width="99%" border="0" cellpadding="0" cellspacing="0" class="borde_tab">
					<tr>
						<td>
							<?
	...
	<?
	$isql = "select RADI_NUME_SALIDA from anexos a where a.anex_radi_nume='$verrad' and a.anex_estado>=4";
	$rs = $db->query($isql);
	$radicado_d = "";
	while (!$rs->EOF) {
		$valor = $rs->fields["RADI_NUME_SALIDA"];
	...
	$isql = "select 
		...
		and a.anex_estado>=4
		and a.anex_radi_nume = $verrad
		AND (r.radi_nume_radi like '%5' or r.radi_nume_radi like '%1')
		";
	$rs = $db->query($isql);
	$i = 1;
	if (!$rs->EOF) {
	?>
		...
		<CENTER>
			...
			<tr align="center">
				<td class=borde_tab align="center">
					<?
