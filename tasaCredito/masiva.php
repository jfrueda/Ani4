...
function generarDocumento($t, $dia, $mes, $anho, $num, $pathAux, $complementDireccion, $asunto) {
    ...
    $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('ResolucionCredito.docx');
    ...
    $templateProcessor->setValue('RES_OSA_NO', $t[35]);
    $templateProcessor->setValue('FECHA_OSA_NO', $t[36]);
    $templateProcessor->setValue('NO_DOC', $t[33]);
    $templateProcessor->setValue('FECHA_ NOT_OSA', $t[37]);
    $templateProcessor->setValue('RADICADO_NOT_OSA', $t[38]);
    $templateProcessor->setValue('FECHA_LIQUIDACION', $t[39]);
    ...
    $templateProcessor->setValue('OBLIGACION_NO_5', $t[29] != '' ? $t[29] : '------------');
    ...
    $templateProcessor->setValue('FECHA_FIRMEZA_NO_5', $t[30] != '' ?  $t[30] : '------------');

    $templateProcessor->setValue('SALDO_CAPITAL',   $t[15] != '' ? '$ ' . number_format($t[15], 2, ',', '.') : '------------');
    $templateProcessor->setValue('SALDO_CAPITAL_2', $t[19] != '' ? '$ ' . number_format($t[19], 2, ',', '.') : '------------');
    $templateProcessor->setValue('SALDO_CAPITAL_3', $t[23] != '' ? '$ ' . number_format($t[23], 2, ',', '.') : '------------');
    $templateProcessor->setValue('SALDO_CAPITAL_4', $t[27] != '' ? '$ ' . number_format($t[27], 2, ',', '.') : '------------');
    $templateProcessor->setValue('SALDO_CAPITAL_5', $t[31] != '' ? '$ ' . number_format($t[31], 2, ',', '.') : '------------');
    ...
    $templateProcessor->setValue('MULTA_5', $t[32] != '' ? $t[32] : '------------');

    $templateProcessor->setValue('TASA_APLICABLE_NO', $t[40] != '' ? $t[40] : '------------');
    $templateProcessor->setValue('TASA_APLICABLE_N1', $t[45] != '' ? $t[45] : '------------');
    $templateProcessor->setValue('TASA_APLICABLE_N2', $t[50] != '' ? $t[50] : '------------');
    $templateProcessor->setValue('TASA_APLICABLE_N3', $t[55] != '' ? $t[55] : '------------');
    $templateProcessor->setValue('TASA_APLICABLE_N4', $t[60] != '' ? $t[60] : '------------');


    $templateProcessor->setValue('FECHA INICIO_ COBRO_NO', $t[41] != '' ? $t[41] : '------------');
    $templateProcessor->setValue('FECHA INICIO_ COBRO_N1', $t[46] != '' ? $t[46] : '------------');
    $templateProcessor->setValue('FECHA INICIO_ COBRO_N2', $t[51] != '' ? $t[51] : '------------');
    $templateProcessor->setValue('FECHA INICIO_ COBRO_N3', $t[56] != '' ? $t[56] : '------------');
    $templateProcessor->setValue('FECHA INICIO_ COBRO_N4', $t[61] != '' ? $t[61] : '------------');   

    $templateProcessor->setValue('VALOR_CAPITAL_0',   $t[42] != '' ? '$ ' . number_format($t[42], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_CAPITAL_1', $t[47] != '' ? '$ ' . number_format($t[47], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_CAPITAL_2', $t[52] != '' ? '$ ' . number_format($t[52], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_CAPITAL_3', $t[57] != '' ? '$ ' . number_format($t[57], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_CAPITAL_4', $t[62] != '' ? '$ ' . number_format($t[62], 2, ',', '.') : '------------');

    $templateProcessor->setValue('VALOR_INTERES_0',   $t[43] != '' ? '$ ' . number_format($t[43], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_INTERES_1', $t[48] != '' ? '$ ' . number_format($t[48], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_INTERES_2', $t[53] != '' ? '$ ' . number_format($t[53], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_INTERES_3', $t[58] != '' ? '$ ' . number_format($t[58], 2, ',', '.') : '------------');
    $templateProcessor->setValue('VALOR_INTERES_4', $t[63] != '' ? '$ ' . number_format($t[63], 2, ',', '.') : '------------');

    $templateProcessor->setValue('TOTAL_LIQUIDADO_0', $t[44] != '' ? '$ ' . number_format($t[44], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_LIQUIDADO_1', $t[49] != '' ? '$ ' . number_format($t[49], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_LIQUIDADO_2', $t[54] != '' ? '$ ' . number_format($t[54], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_LIQUIDADO_3', $t[59] != '' ? '$ ' . number_format($t[59], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_LIQUIDADO_4', $t[64] != '' ? '$ ' . number_format($t[64], 2, ',', '.') : '------------');


    $direccion = $t[5] != '' ? $t[5] : '--';
    $templateProcessor->setValue('TOTAL_LIQUIDACIÓN_CAPITAL', $t[65] != '' ? '$ ' . number_format($t[65], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_LIQUIDACIÓN_INTERESES', $t[66] != '' ? '$ ' . number_format($t[66], 2, ',', '.') : '------------');
    $templateProcessor->setValue('TOTAL_CAPITAL_MAS_INTERESES', $t[67] != '' ? '$ ' . number_format($t[67], 2, ',', '.') : '------------');
    ...
    $templateProcessor->setValue('EXP_SP', $t[34]);
    $templateProcessor->setValue('USUA_PROYECTO', ' Indrys Milena Guerrero Martinez – Profesional Universitario');
...
$asunto = 'Por la cual se liquida el crédito en el proceso de cobro coactivo';
...
foreach ($sheetData as $t) {
    ...
    if($t[0] != '') {
       ...
       if ($nurad=="-1"){
            ...
            */  
            ...
            $hist->insertarHistorico($radicadosSel,
                ...
                $commandFirmado='java -jar '.$ABSOL_PATH.'/include/jsignpdf/JSignPdf.jar ' . str_replace('docx','pdf',$pathAux) . ' -kst PKCS12 -ksf ' . $P12_FILE . ' -ksp ' 
                    . $clave . ' --font-size 7 -r \'Firmado al Radicar en CADET\' -V -llx 0 -lly 0 -urx 550 -ury 27 -d ' . $pathFolderAux . ' 2>&1';
            ...
            */            
            $errorExp = '';
            $sqlValidarExpediente = "SELECT count(*) FROM public.sgd_sexp_secexpedientes WHERE sgd_exp_numero =  '$t[34]'";
            ...
            if($existeExp > 0) {
                ...
                $sqlExpedienteEstado = "SELECT sgd_sexp_estado FROM public.sgd_sexp_secexpedientes  where sgd_exp_numero = '$t[34]'";
                ...
                if($estadoExp == 0) {
                    ...
                    $historialExpediente="INSERT INTO sgd_hfld_histflujodoc(
                        ...
                        usua_doc, usua_codi, depe_codi, sgd_ttr_codigo, sgd_fexp_observa, sgd_hfld_observa, 
                        sgd_fars_codigo, sgd_hfld_automatico) values
                        (0,null, CURRENT_TIMESTAMP,'$t[34]' ,$nurad, '" . $usua_doc_radica . "',$usuRadica,$depeRadica, 53, 
                            null,'Incluir radicado en Expediente', null,null)";
                     $db->conn->Execute($historialExpediente);                  
