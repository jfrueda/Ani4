...
if (
    ...
    if ($_SESSION["usua_perm_envios"] >= 1 || $_SESSION['envios_general'] >= 1 || $_SESSION['envios_dependencia'] >= 1) {
        $envios = array(
            'subMenu' => 0,
    ...
    if ($_SESSION["usr_adm_paz_salvo"] >= 1) {
        $pazSalvo = array(
            'subMenu' => 0,
            'url' => "estadisticas/reportePz/ReportePazSalvo.php",
            'nombre' => "Reporte Paz y Salvo por Usuario"
        );
...
/***********************************************************************/
if ($_SESSION['subir_acuses'] >= 1) {
    $cargaAcuses = array(
        'subMenu' => 0,
        'url' => "carga_acuses/subirAcuses.php",
        'nombre' => "Cargue de acuses"
    );
    $acciones['cargAcuses'] = $cargaAcuses;
}
/***********************************************************************/
...
if ($_SESSION["usua_admin_sistema"] >= 1 || $tiene_acceso_admin) {
    ...
    */
    ...
    if (!UsuarioFiltro::tieneFiltro()) {
        ...
        $administracion["auditoria"] = array(
            'subMenu' => 0,
            'url' => "./auditoria/cuerpoAuditoria.php?$sendSession",
            'nombre' => "Auditoria de usuario"
        );
        $administracion["auditoria-sesion"] = array(
            'subMenu' => 0,
            'url' => "./auditoria/cuerpoAuditoriaSesion.php?$sendSession",
            'nombre' => "Auditoria de sesión"
        );
        $administracion["auditoria-administracion"] = array(
            'subMenu' => 0,
            'url' => "./auditoria/cuerpoLog.php?$sendSession",
            'nombre' => "Auditoria de administración"
        );
    }
    $administracion["notificaciones-sistema"] = array(
        'subMenu' => 0,
        'url' => "./alertas/notificacionesArgo.php?$sendSession",
    ...
    if (!UsuarioFiltro::tieneFiltro()) {
        $administracion['seguridadRadicado'] = [
            'subMenu' => 0,
            'url' => "./Administracion/adm_seguridad_radicado.php?$sendSession",
            'nombre' => "Seguridad radicado"
        ];
    }
} else {
...
while (!$rs->EOF) {
    ...
    $usuario["usuario$cambioDepeCodi" . $i] = array(
        'subMenu' => 0,
...
if ($_SESSION["USUA_PRAD_TP6"]) {
    ...
    $radicacion["radica6"] = array('subMenu' => 0, 'url' => "$enlace6", 'nombre' => "Resoluciones");
}
...
$bandeja->usuaDoc = $_SESSION['usua_doc'];
...
?>
<? include_once "$ruta_raiz/alertas/alertasModal.php"; ?>
