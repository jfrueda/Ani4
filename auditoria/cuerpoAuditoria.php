...
<body onLoad="window_onload();" class="waitMe_body">
  ...
  if ($busqUsuario || $busqRadicado || $busExpediente) {
    ...
    if (strlen($busqUsuario) > 4) {
      $whereUsuario .= " AND us.usua_nomb like '%$busqUsuario%'";
      $whereUsuarioExp .= " AND us.usua_nomb like '%$busqUsuario%'";
    }
    if (strlen($busqRadicado) > 4) {
      $whereUsuario .= " AND (b.RADI_NUME_RADI  = '$busqRadicado' or cast(b.radi_nume_borrador as varchar(20)) = '$busqRadicado')  ";
      $whereUsuarioExp .= " AND t.RADI_NUME_RADI  = '$busqRadicado'";
    }
...
<script>
  ...
  function validateForm() {
    ...
    if (busqUsuario.trim() === '' && busqRadicado.trim() === '' && busExpediente.trim() === '') {
      notie.alert({
        type: 'error',
        text: 'Se debe diligenciar al menos un campo'
      })
      return false;
    }
