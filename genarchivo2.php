...
//print ("***ocho***($numrad)($nom_recurso)".$radObjeto->getTdocCodi());  
...
<body>
	...
	if (strlen(trim($radicar_a)) == 13 || strlen(trim($radicar_a)) == 18) {
		$no_digitos = 5;
	} else {
		$no_digitos = 6;
	}
		...
		if (($ent != 2) && $nurad && $vpppp == "ddd") {
			...
			if ($vp == "n" && $radicar_a == "si") {
				if ($generar_numero == "no") {
					...
					if (!$radicado_salida) { //print ("Genera secuencia antigua");
